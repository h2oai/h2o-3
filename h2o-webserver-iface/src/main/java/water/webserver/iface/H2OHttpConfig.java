...
public class H2OHttpConfig {
  ...
  /**
  * Creates new instance of {@link H2OHttpConfig} with API permanently enabled
  */
  public H2OHttpConfig() {
    this.apiEnabledSupplier = (Supplier) () -> true;
  }

  /**
  * Creates a new intance of {@link H2OHttpConfig} with API states indicated by a dynamically called supplier.
  * @param apiEnabledSupplier A {@link Supplier} which returns true if API on this node should be enabled/accessible, otherwise
  *                           returns false. This supplier is called with each request.
  */
  public H2OHttpConfig(Supplier<Boolean> apiEnabledSupplier) {
    this.apiEnabledSupplier = apiEnabledSupplier;
  }
  ...
  // Supplier enables users of this API to manipulate the API status dynamically, e.g. as resources become avaialble
  // during H2O clustering process.
  private final Supplier<Boolean> apiEnabledSupplier;

  /**
  * @return True if API services on this H2O node should be accessible, otherwise false.
  */
  public boolean isNodeApiEnabled(){
    return apiEnabledSupplier.get();  
  }
