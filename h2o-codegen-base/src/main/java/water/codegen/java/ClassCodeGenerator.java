 ...
 *   - and list of field and method generators
 */
public class ClassCodeGenerator extends CodeGeneratorPipeline<ClassCodeGenerator, CodeGeneratorB> {
  ...
  // FIXME: source is the most cases model itself, we can get rid of the source parameter
  public ClassCodeGenerator withMixin(Object source, Class... mixins) {
    return withMixin(source, false, false, mixins);
  }

  public ClassCodeGenerator withMixin(Class mixin, Class... mixins) {
    return withMixin(null, false, false, ArrayUtils.append(mixins, mixin));
  }

  public ClassCodeGenerator withMixin(Object source, boolean includeParent, boolean onlyMethodDefs, Class... mixins) {
    ...
    for (Class mixin : mixins) {
      ...
      if (onlyMethodDefs) {
        for (Method decMethod : declaredMethods) {
          MethodCodeGenerator mcg = method(decMethod.getName());
          if (mcg == null) {
            withMethod(mcg);
          }
        }
        ...
        assert is != null : "The mixin for " + mixin.getSimpleName() + " generator is missing! Did you have resources generated by gradle task `copyMixinsToResources`?";
        // Extract methods and redefine them at code generator level
        try {
  ...
  protected JCodeSB genClassHeader(JCodeSB sb) {
    ...
    sb.p(Modifier.toString(modifiers)).p(modifiers > 0 ? " " : "").p("class ").p(name).p(" ");
    if (extendClass != null) {
