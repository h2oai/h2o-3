...
then
    ...
    COVERAGE="-javaagent:$AGENT=destfile=build/jacoco/h2o-ext-steam.exec"
...
# On this h2o-ext-steam testMultiNode.sh only, force the tests.txt to be in the same order for all machines.
...
(cd src/test/java; /usr/bin/find . -name '*.java' | cut -c3- | sed 's/.....$//' | sed -e 's/\//./g') | sort | grep -v AAA_PreCloudLock > $OUTDIR/tests.txt
