...
function cleanup () {
  ...
  if [ "$RC" != "0" ]; then
    cat $OUTDIR/out.*
    echo h2o-automl junit tests FAILED
    exit 1
  else
    echo h2o-automl junit tests PASSED
    exit 0
  fi
}
...
then
    ...
    COVERAGE="-javaagent:$AGENT=destfile=build/jacoco/h2o-automl.exec"
...
# On this h2o-automl testMultiNode.sh only, force the tests.txt to be in the same order for all machines.
...
# All output redir'd at the OS level to sandbox files.
...
echo Running h2o-automl junit tests...
