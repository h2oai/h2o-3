...
public class GridSearchVsSMBOBenchmark extends water.TestUtil {
  ...
  public void benchmark() {
    int numberOfSearchIterations = 180; //377; 
    ...
    for (int seedAttempt = 0; seedAttempt < numberOfRuns; seedAttempt++) {
      ...
      try {
        ...
        // If SMBO prediction is in top 3 then it is considered as a success
        if(bestParamsFromSMBO.getScore() >= bestParamsFromRGS[2].getScore()) successCount++;

        System.out.println("Index of best found params for " + seedAttempt + "st run : " + bestParamsFromSMBO.getIndex() );
