...
public class TargetEncodingHyperparamsEvaluator_V2_Inner_EMaps_on_OOF extends Iced {
  public TargetEncodingHyperparamsEvaluator_V2_Inner_EMaps_on_OOF() {
  ...
  public double evaluateForCVMode(TargetEncodingParams teParams, ModelBuilder modelBuilder, String[] columnsToEncode, long seedForFoldColumn) {
    int numberOfFolds = modelBuilder._parms._nfolds;
    ...
    try {
      ...
      //In case user has not not provided custom fold column we don't have autogenerated fold column yet and `_parms._fold_column` = null as well
      // We will use the same seed as will be used in model builder to autogenerate folds.
      if(foldColumnName == null){
        final long seedForAutoGeneratedFolds = modelBuilder._parms.getOrMakeRealSeed();
        foldColumnName = modelBuilder._job._key.toString() + "_autogenerated_fold";
        
        addKFoldColumn(modelBuilder._parms._fold_assignment, trainCopy, foldColumnName, numberOfFolds, responseColumn, seedForAutoGeneratedFolds);
      }

      // To make evaluation representative with regard to the way we are going to validate the final model we need to use same folds as will be used during validation.
      for (int fold = 0; fold < numberOfFolds; fold++) {

        Frame[] splitsByCurrentFold = TargetEncoderFrameHelper.splitByValue(trainCopy, foldColumnName, fold);
        Frame currentFold = splitsByCurrentFold[0];
        Frame otherFolds = splitsByCurrentFold[1];

        //For each cv model we need copy of the builder. Original one will not be used here ( but we actually used it read some properties before for loop)
        ...
        switch (holdoutType) {
          case KFold:
            ...
            //Note default value for KFOLD target encoding. We might want to search for this value but it is quite expensive and is ok to be magically fixed.
            int nfolds = 5;
            addKFoldColumn(otherFolds, foldColumnForTE, nfolds, seedForFoldColumn);

            encodingMap = tec.prepareEncodingMap(otherFolds, responseColumn, foldColumnForTE, true);
            trainEncoded = tec.applyTargetEncoding(otherFolds, responseColumn, encodingMap, KFold, foldColumnForTE, teParams.isWithBlendedAvg(), teParams.getNoiseLevel(), true, seedForFoldColumn);
            testEncoded = tec.applyTargetEncoding(currentFold, responseColumn, encodingMap, None, foldColumnForTE, teParams.isWithBlendedAvg(), 0.0, true, seedForFoldColumn);
            break;
          case LeaveOneOut:
          case None:
            default: 
        ...
        clonedModelBuilder._parms._ignored_columns = concat(concat(originalIgnoredColumns, teColumnsToExclude), new String[]{foldColumnName, foldColumnForTE});

        clonedModelBuilder.setTrain(trainEncoded);
        clonedModelBuilder._parms.setTrain(trainEncoded._key);

        try {
          cumulativeScore += scoreOnTest(clonedModelBuilder, testEncoded);
      ...
      modelBuilder._parms._ignored_columns = originalIgnoredColumns; // we dont need it here as we were changing clonedMB
    ...
    averageScore = cumulativeScore / numberOfFolds;
    return averageScore;
  }
  ...
  private double scoreOnTest(ModelBuilder modelBuilder, Frame testEncodedFrame) {
    ...
    ModelMetricsBinomial mmb = ModelMetricsBinomial.getFromDKV(retrievedModel, testEncodedFrame);
