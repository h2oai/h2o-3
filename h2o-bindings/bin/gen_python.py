...
def gen_module(schema, algo):
    ...
    if algo in ['stackedensemble']:
        yield "    def model_summary(self, base_model_detail=False):"
        yield "        \"\"\""
        yield "        Summary of stacked ensemble model"
        yield "        "
        yield "        :param base_model_detail: A string specifying boolean indicating whether or not to include"
        yield "            base model summary information"
        yield "        \"\"\""
        yield "        from collections import Counter"
        yield "        from h2o import api, no_progress, get_model"
        yield "        import pandas as pd"
        yield "        "
        yield "        assert_is_type(base_model_detail, bool)"
        yield "        "
        yield "        no_progress()"
        yield ""
        yield "        models = dict()"
        yield "        for model in  self._parms.get('base_models'):"
        yield "            model_json = api('GET /%d/Models/%s' % (3, model))['models'][0]"
        yield "            models[model] = model_json['algo']"
        yield ""
        yield "        # base model information"
        yield "        print('Base Model Information: \\n')"
        yield "        print('Number of Base Models: {0}'.format(len(models.values())))"
        yield "        "
        yield "        print('Base Models (count by algorithm type):')"
        yield "        c = Counter(models.values())"
        yield "        print(H2OFrame(c))"
        yield "        "
        yield "        if base_model_detail:"
        yield "            print('Base model details:')"
        yield "            for model_id, algo in models.items():"
        yield "                print('{0}:'.format(algo))"
        yield "                model_summary = get_model(model_id).summary().as_data_frame()"
        yield "                # convert model_summary to H2OFrame and remove '0' key if it exists"
        yield "                model_summary = model_summary.to_dict(orient='list')"
        yield "                if '' in model_summary.keys():"
        yield "                    del model_summary['']"
        yield "                print(H2OFrame(model_summary))"
        yield ""
        yield "        print('\\nMetalearner Information:\\n')"
        yield "        "
        yield "        metalearner_algo = self._parms.get('metalearner_algorithm')"
        yield "        meta_algo = 'glm' if metalearner_algo is None or len(metalearner_algo) > 1 else metalearner_algo"
        yield "        if meta_algo is not None:"
        yield "            print('Metalearner Algorithm: {0}'.format(meta_algo))"
        yield ""
        yield "        metalearner_nfolds = self._parms.get('metalearner_nfolds')"
        yield "        if metalearner_nfolds is not None and metalearner_nfolds is not 0:"
        yield "            fold_assignment = 'Random' if len(metalearner_nfolds) > 1 else self._parms.get('metalearner_fold_assignment')"
        yield "            print('Metalearner cross-validation fold assignment:')"
        yield "            print('    Fold assignment scheme: {0}'.format(fold_assignment))"
        yield "            print('\\n    Number of folds: {0}'.format(metalearner_nfolds))"
        yield "            print('\\n    Fold column: {0}'.format(self._parms.get('metalearner_fold_column')))"
        yield ""
        yield "        metalearner_params = self._parms.get('metalearner_params')"
        yield "        if metalearner_params is not None:"
        yield "            print('Metalearner parameters: {0}'.format(metalearner_params))"
