...
def generate_main_class(endpoints):
    ...
    yield "import com.google.gson.reflect.TypeToken;"
    yield "import com.google.gson.stream.JsonReader;"
    yield "import com.google.gson.stream.JsonWriter;"
    ...
    yield "   */"
    yield "  private static class KeySerializer implements JsonSerializer<KeyV3>, JsonDeserializer<KeyV3> {"
    yield "    @Override"
    ...
    yield "    }"
    yield "    @Override"
    yield "    public KeyV3 deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) {"
    yield "      if (json.isJsonNull()) return null;"
    yield "      JsonObject jobj = json.getAsJsonObject();"
    yield "      String type = jobj.get(\"type\").getAsString();"
    yield "      switch (type) {"
    # TODO: dynamically generate all possible cases
    yield "        case \"Key<Model>\": return context.deserialize(jobj, ModelKeyV3.class);"
    yield "        case \"Key<Job>\":   return context.deserialize(jobj, JobKeyV3.class);"
    yield "        case \"Key<Grid>\":  return context.deserialize(jobj, GridKeyV3.class);"
    yield "        case \"Key<Frame>\": return context.deserialize(jobj, FrameKeyV3.class);"
    yield "        default: throw new JsonParseException(\"Unable to deserialize key of type \" + type);"
    yield "      }"
    yield "    }"
    yield "  }"
    yield ""
    ...
    yield "   */"
    yield "  public static <T extends KeyV3> T[] stringArrayToKeyArray(String[] ids, Class<T> clz) {"
