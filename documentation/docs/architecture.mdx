...
> Python scripts use the H2O package for Python (`import h2o`), which communicates with H2O-3 through the REST API.
...
> MRTask stands for MapReduce Task. This is an H2O in-memory MapReduce task.
...
![r_hdfs_read_step3](images/r_hdfs_read_step3.png)
...
The left side shows the steps that run the R process and the right side shows the steps that run in the H2O cluster. The top layer is the TCP/IP network code that enables the two processes to communicate with each other.
...
![Sequence diagram: starting GLM from R](images/start_glm_from_r.png)
...
![Sequence diagram: retrieving GLM result from R](images/retrieve_glm_result_from_r.png)
...
![End-to-end sequence diagram: running GLM from R](images/run_glm_from_r.png)
