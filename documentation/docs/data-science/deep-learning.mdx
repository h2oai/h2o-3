...
A feedforward artificial neural network (ANN) model, also known as deep neural network (DNN) or multi-layer perceptron (MLP), is the most common type of Deep Neural Network and the only type that is supported natively in H2O-3. Several other types of DNNs are popular as well, such as Convolutional Neural Networks (CNNs) and Recurrent Neural Networks (RNNs). MLPs work well on transactional (tabular) data; however if you have image data, then CNNs are a great choice. If you have sequential data (for example, text, audio, time-series), then RNNs are a good choice.

### MOJO support

Deep Learning supports importing and exporting [MOJOs](../save-and-load-model#supported-mojos).

## Quick start and additional resources

- Deep Learning in H2O Tutorial (R language): [\[GitHub\]](https://htmlpreview.github.io/?https://github.com/ledell/sldm4-h2o/blob/master/sldm4-deeplearning-h2o.html)
- Deep learning in H2O with Arno Candel (Overview) [\[Youtube\]](https://www.youtube.com/watch?v=zGdXaRug7LI/)
- Top 10 tips and tricks [\[Youtube\]](https://www.youtube.com/watch?v=LM255qs8Zsk/)
- NYC Tour Deep Learning Panel: Tensorflow, Mxnet, Caffe [\[Youtube\]](https://www.youtube.com/watch?v=KWdkVoKJG3U/)
...
- **hidden**: Specify the hidden layer sizes (for example, `(100,100)`). The value must be positive. This option defaults to `(200,200)`.

- **hidden_dropout_ratios**: (Applicable only if the activation type is `tanh_with_dropout`, `rectifier_with_dropout`, or `maxout_with_dropout`) Specify the hidden layer dropout ratio to improve generalization. Specify one value per hidden layer. The range is $\geq$ 0 to \< 1, and the default is `0.5`.

- [huber_alpha](algo-params/huber_alpha): Specify the desired quantile for Huber/M-regression (the threshold between quadratic and linear loss). This value must be between 0 and 1, and the default is `0.9`.
...
- **max_w2**: Specify the constraint for the squared sum of the incoming weights per unit (for example, for `rectifier`). Defaults to `3.4028235e+38`.
...
- **nesterov_accelerated_gradient**: (Applicable only if `adaptive_rate=False`) Enables the [Nesterov Accelerated Gradient](http://premolab.ru/pub_files/pub88/qhkDNEyp8.pdf). This option defaults to `True` (enabled).
...
- [quantile_alpha](algo-params/quantile_alpha): (Only applicable if `distribution="quantile"`) Specify the quantile to be used for Quantile Regression. This option defaults to `0.5`.

- [rate](algo-params/rate): (Applicable only if `adaptive_rate=False`) Specify the learning rate. Higher values result in a less stable model, while lower values lead to slower convergence. This option defaults to `0.005`.

- [rate_annealing](algo-params/rate_annealing): (Applicable only if `adaptive_rate=False`) Specify the rate annealing value. $\frac{\text{rate}}{(1+ \text{ rate_annealing } \times \text{ samples})}$. This option defaults to `1e-06`.

- [rate_decay](algo-params/rate_decay): (Applicable only if `adaptive_rate=False`) Specify the rate decay factor between layers. $\text{N-th layer: } \text{rate } \times \text{ rate_decay}^{(n-1)}$. This options defaults to `1`.
...
- **score_validation_samples**: (Applicable only if a `validation_frame` is specified) Specify the number of validation set samples for scoring. The value must be $\geq$ 0. To use all validation samples, enter `0` (default).
...
- **train_samples_per_iteration**: Specify the number of global training samples per MapReduce iteration. To specify one epoch, enter `0`. To specify all available data (for example, replicated training data), enter `-1`. To use the automatic (default) values, enter `-2`.
...
- **sparsity_beta**: (Applicable only if `autoencoder=True`) Specify the sparsity-based regularization optimization. [Read more about sparsity-based regularization](http://www.mit.edu/~9.520/spring09/Classes/class11_sparsity.pdf). This option defaults to `0`.
...
- [checkpoint](algo-params/checkpoint): Enter a model key associated with a previously trained Deep Learning model. Use this option to build a new model as a continuation of a previously generated model.

  > **Note**: Cross-validation is not supported during checkpoint restarts.

- [class_sampling_factors](algo-params/class_sampling_factors): (Applicable only for classification and when `balance_classes=True`) Specify the per-class (in lexicographical order) over/under-sampling ratios. By default, these ratios are automatically computed during training to obtain the class balance.
...
- [keep_cross_validation_fold_assignment](algo-params/keep_cross_validation_fold_assignment): Enable this option to preserve the cross-validation fold assignment. This option defaults to `False` (default).
...
- [max_after_balance_size](algo-params/max_after_balance_size): (Applicable only when `balance_classes=True`)Specify the maximum relative size of the training data after balancing class counts. The value can be \> 1.0. This option defaults to `5.0`.
...
- [missing_values_handling](algo-params/missing_values_handling): Specify how to handle missing values. One of: `Skip` or `MeanImputation` (default).
...
- [nfolds](algo-params/nfolds): Specify the number of folds for cross-validation. This option defaults to `0` (no cross-validation).

  > **Note**: Cross-validation is not supported when `autoencoder` is enabled.

- [offset_column](algo-params/offset_column): (Applicable for regression only) Specify a column to use as the offset.
...
- [score_each_iteration](algo-params/score_each_iteration): Specify whether to score during each iteration of model training. This option defaults to `False` (disabled).

- [seed](algo-params/seed): Specify the random number generator (RNG) seed for algorithm components dependent on randomization. The seed is consistent for each H2O instance so that you can create models with the same starting conditions in alternative configurations. This option defaults to `-1` (time-based random number).

- [single_node_mode](algo-params/single_node_mode): Specify whether to run on a single node for fine-tuning of model parameters. This option defaults to `False` (disabled).

- [standardize](algo-params/standardize): If enabled, automatically standardize the data (mean 0, variance 1). If disabled, the user must provide properly scaled input data. This option defaults to `True` (enabled).
...
- [stopping_tolerance](algo-params/stopping_tolerance): Specify the relative tolerance for metric-based stopping to stop training if the improvement is less than this value. This option defaults to `0`.
...
- [tweedie_power](algo-params/tweedie_power): (Only applicable if `distribution="tweedie"`) Specify the Tweedie power. You can tune over this option with values \> 1.0 and \< 2.0, and the default is `1.5`. For more information, refer to [Tweedie distribution](https://en.wikipedia.org/wiki/Tweedie_distribution).
...
This code example demonstrates how to build a Deep Learning model using H2O-3 in R and Python. It uses the insurance dataset, sets categorical features, and trains a model with Tweedie distribution. Predictions and performance evaluations are also shown for both languages.
...
> Neither; `reduce()` calls occur after every two `map()` calls, between threads and ultimately between nodes. Many `reduce()` calls occur—much more than one per MapReduce step (also known as an "iteration"). Epochs are not related to MR iterations, unless you specify `train_samples_per_iteration` as `0` or `-1` (or to number of rows/nodes). Otherwise, one MR iteration can train with an arbitrary number of training samples (as specified by `train_samples_per_iteration`).
...
>
> Of course, using more training or validation samples will increase the time for scoring, as well as scoring more frequently. For more information about how this affects runtime, refer to the [Deep Learning Performance Guide](https://www.h2o.ai/blog/deep-learning-performance/).
...
>
> A few options exist for managing checkpoint restarts:
>
...
- [R](https://github.com/h2oai/h2o-3/blob/master/h2o-docs/src/product/tutorials/dl/dlperf.Rmd)
- [Blog](https://www.h2o.ai/blog/deep-learning-performance-august/)

## References

["Deep Learning." \*Wikipedia: The free encyclopedia\*. Wikimedia Foundation, Inc. May 1, 2015. Web. May 4, 2015.](http://en.wikipedia.org/wiki/Deep_learning)

["Artificial Neural Network." \*Wikipedia: The free encyclopedia\*. Wikimedia Foundation, Inc. April 22, 2015. Web. May 4, 2015.](http://en.wikipedia.org/wiki/Artificial_neural_network)

[Zeiler, Matthew D. 'ADADELTA: An Adaptive Learning Rate Method'. Arxiv.org. N.p., 2012. Web. May 4, 2015.](http://arxiv.org/abs/1212.5701)
...
[Candel, Arno. "The Definitive Performance Tuning Guide for H2O Deep Learning." H2O.ai, Inc. (2015).](http://www.h2o.ai/blog/deep-learning-performance/)
