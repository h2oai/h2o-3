...
`lambda_min_ratio` works in conjunction with `nlambdas`. The sequence of the $\lambda$ values is automatically generated as an exponentially decreasing sequence. It ranges from $\lambda_{max}$ (the smallest $\lambda$ so that the solution is a model with all 0s) to $\lambda_{min} =$ `lambda_min_ratio` $\times$ $\lambda_{max}$.
