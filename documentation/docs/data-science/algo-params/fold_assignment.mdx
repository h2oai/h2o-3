...
This option specifies the scheme to use for [cross-validation](../../cross-validation) fold assignment. This option is only applicable if a value for `nfolds` is specified and a `fold_column` is not specified. Options include:
...
- Modulo: Performs [modulo](https://en.wikipedia.org/wiki/Modulo_operation) operation when splitting the folds.
