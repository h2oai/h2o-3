...
- **PlusPlus**:
  1.  Choose one center $m_{1}$ at random.
  2.  Calculate the difference between $m_{1}$ and each of the remaining $N-1$ observations $x_{i}$. $d(x_{i}, m_{1}) = \|(x_{i}-m_{1})\|^2$
  3.  Let $P(i)$ be the probability of choosing $x_{i}$ as $m_{2}$. Weight $P(i)$ by $d(x_{i}, m_{1})$ so that those $x_{i}$ furthest from $m_{1}$ have a higher probability of being selected than those $x_{i}$ close to $m_{1}$.
