...
`nlambdas` works in conjunction with `lambda_min_ratio`. The sequence of the $\lambda$ values is automatically generated as an exponentially decreasing sequence. It ranges from $\lambda_{max}$ (the smallest $\lambda$ so that the solution is a model with all zero coefficients) to $\lambda_{min} =$ `lambda_min_ratio` $\times$ $\lambda_{max}$.
...
- When `alpha=0`, the default value for `nlambdas` is set to 30 because fewer lambdas are needed for ridge regression.
