...
Note that this is a strict parameter that takes into account the exact string of the column name. For example, if your dataset includes one column named **Type** and another column named **Types**, and you specify `x=["type"]`, then the algorithm will only include the **Type** column and will ignore the **Types** column.
