...
This option specifies the maximum number of clusters that K-Means will form. If `estimate_k` is disabled, the model will find $k$ centroids, otherwise it will find up to $k$ centroids. This parameter is required and has no default value.
