...
This option also works closely with the [lambda](lambda) parameter, which controls the amount of regularization applied. The following table describes the type of penalized model that results based on the values specifed for the `lambda` and `alpha` options.
