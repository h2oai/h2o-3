...
- Priors is standardized and changed when standardization is turned on for the model build. When standardization is turned on, the `beta_given` in `beta_constraints` is standardized as well. In the [code](https://github.com/h2oai/h2o-3/blob/master/h2o-algos/src/main/java/hex/glm/GLM.java#L1902), you see that the `beta_given` is multiplied by factor `d : _betaGiven *= d;`<span className="title-ref"> where </span><span className="title-ref">d = 1/sd</span>\`. In particular, be careful when using previous coefficients as priors and with standardization turned on because the penalty taken on different priors.
