...
Stacked Ensemble allows you to specify the metalearning algorithm to use when training the ensemble. When `metalearner_algorithm` is set to a non-default value (for example, "GBM"), Stacked Ensemble runs with the specified algorithm's default hyperparameter values. The `metalearner_params` option allows you to pass in a dictionary/list of hyperparameters to use for that algorithm instead of the defaults.
