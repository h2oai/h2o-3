...
- $P$ is treatment class distribution
- $Q$ is control class distribution

In a tree node, the result value for the split is the sum $metric(P, Q) + metric(1-P, 1-Q)$. For the split gain value, the result within the node is normalized using the Qini coefficient (Eclidean or ChiSquared) or entropy (KL) for each distribution before and after the split.
