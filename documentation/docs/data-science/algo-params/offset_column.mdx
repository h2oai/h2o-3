...
When used with distributions and family link functions, the offset corrections are applied in the linearized space before applying the inverse link function to get the actual response values. For example, you may have fitted some other (logistic) regression using other variables (and data), and now you want to see if the present variables can add anything. You can then use the predicted logit from the other model as an offset in the new model. To get the logit from a predicted probability in H2O, you can use this expression: $\text{logit} = \text{log}\big(\frac{prob}{(1-prob)}\big)$.

**Notes**:

- This option is not applicable for multinomial distributions
- The offset column cannot be the same as the [fold_column](fold_column).
