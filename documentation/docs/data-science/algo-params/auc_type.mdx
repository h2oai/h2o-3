...
This parameter is important when the [early_stopping](early_stopping) parameter is set to `AUC` and also for sorting model by metric in [Grid search](../../grid-search) using `sort_by` parameter.

More information about Multinomial AUC/AUCPR metric is on [Performance and prediction](../../performance-and-prediction) page.
...
:::note
`auc_type` is available only for multinomial distribution and is `NONE` by default.
:::
...
print(h2o.multinomial_auc_table(car_gbm, valid = TRUE))
# Print the whole AUCPR table:
print(h2o.multinomial_aucpr_table(car_gbm, valid = TRUE))
```
