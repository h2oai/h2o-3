...
3.  Save the H2O binary model. Refer to [Saving, Loading, Downloading, and Uploading Models](../../save-and-load-model) for more information. The binary model will contain the H2O version and the parameters used to train the model.
4.  Save the H2O logs. Refer to [Downloading logs](../../logs) for more information.
...
![ParseSetup heuristic line in the H2O logs](../../images/GBMReproducibility_ChunkSize_Logs.png)
...
![List All Frames menu in Flow](../../images/GBMReproducibility_ListAllFrames.png)

![Imported frame shown in Flow](../../images/GBMReproducibility_ImportedFrame.png)
...
1.  Use the logs of the initial model to determine the node order.
    ...
    ![Node logs zip files](../../images/GBMReproducibility_NodeLogs.png)

    1a. Unzip the logs from one of the nodes (the node chosen does not matter) and open the info logs.

    ![Info logs from a single node](../../images/GBMReproducibility_LogsFromSingleNode.png)

    1b. In the logs, search for `Cloud of size N formed [...]`. For example, in this 3 node cluster, search for the term: `Cloud of size 3 formed`.

    ![Cloud formation order in the logs](../../images/GBMReproducibility_CloudOrder_Logs.png)
...
2.  Find the IP and Port of the node that was processing requests in the H2O cluster.
    ...
    2a. In the logs search for: `ParseSetup`

    ![ParseSetup entry in the H2O logs](../../images/GBMReproducibility_ParseSetup.png)

    You may have to try opening multiple zip files to find this term in the logs.

    2b. Identify the IP and Port that processed the ParseSetup request. This is the IP and Port shown in the ParseSetup line.

    ![Log line showing the node receiving requests](../../images/GBMReproducibility_NodeGettingRequests.png)

3.  Determine the order of the node found in Step 2.

    Using the node order document created in Step 1, identify the position of the node that is processing the requests.
...
4.  Start up a new H2O cluster in the command line. This is the H2O cluster we will use to reproduce the model. For example, on each of the 3 nodes run:

```bash
java -jar h2o.jar -name my_cluster -flatfile flatfile.txt
```

This cluster must have the same number of nodes as the cluster used to train the initial model.
...
![Admin menu Cluster Status button in Flow](../../images/GBMReproducibility_ClusterStatusButton.png)

![Cluster Status page in Flow](../../images/GBMReproducibility_ClusterStatus.png)
...
![Cluster Status page with the target node selected](../../images/GBMReproducibility_ClusterStatusNodeSelected.png)
...
If you use a different leader node in the multi-node cluster, it can choose a different order for map-reduce operations and can cause a different roundoff error.
...
- `open_fde`: *Open File Descriptors*
