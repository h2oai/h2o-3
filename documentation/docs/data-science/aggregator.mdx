...
Aggregator currently does not support [MOJOs](../save-and-load-model#supported-mojos).
...
- **rel_tol_num_exemplars**: Specify the relative tolerance for the number of exemplars (for example, `0.5` is +/- 50 percent). This option defaults to `0.5`.
...
- [x](algo-params/x): Specify a vector containing the character names of the predictors in the model.
...
# Build an aggregated frame using eigen categorical encoding
...
[Wilkinson, Leland. “Visualizing Outliers.” (2016).](https://www.cs.uic.edu/~wilkinson/Publications/outliers.pdf)
