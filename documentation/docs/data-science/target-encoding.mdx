...
In this example, we will be trying to predict `survived` using the popular titanic dataset: [https://s3.amazonaws.com/h2o-public-test-data/smalldata/gbm_test/titanic.csv](https://s3.amazonaws.com/h2o-public-test-data/smalldata/gbm_test/titanic.csv). One of the predictors is `cabin`, a categorical column with a number of unique values. To perform target encoding on `cabin`, we will calculate the average of `home.dest` and `embarked` per cabin. So instead of using `cabin` as a predictor in our model, we could use the target encoding of `cabin`.
