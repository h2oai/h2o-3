...
- $j(=[1,2,...,J])$ denotes the cluster (level-2 variable) the measurement is taken from (e.g. the school index);
...
- [rand_family](algo-params/rand_family): Specify the distribution of the random effects. Currently only `rand_family="gaussisan"` is supported.
