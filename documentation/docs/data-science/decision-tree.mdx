...
- [max_depth](algo-params/max_depth): Specify the maximum depth of the final decision tree. The final depth can be less than the definied `max_depth` if another stopped criteria is met first. This value defaults to `20`.
...
The entropy of a set $S$ with respect to a binary target variable can be calculated using the following forumla:
