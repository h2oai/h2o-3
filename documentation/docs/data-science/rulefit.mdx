...
RuleFit supports importing and exporting [MOJOs](../save-and-load-model#supported-mojos).
...
- [lambda](algo-params/lambda): Specify the regularization strength for LASSO regressor.
...
- [x](algo-params/x): Specify a vector containing the names or indicies of the predictor variables to use when building the model. If `x` is missing, then all columns except `y` are used.
...
[Friedman, J. H., & Popescu, B. E. (2008). Predictive learning via rule ensembles. The Annals of Applied Statistics, 2(3), 916-954.](https://arxiv.org/abs/0811.1679)
