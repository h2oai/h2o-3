...
Parameters are optional unless specified as *required*. ANOVA GLM shares many [GLM parameters](glm#shared-glm-family-parameters).

### Algorithm-specific parameters

- **highest_interaction_term**: This option limits the number of interaction terms (that is, `2` means interaction between 2 columns only, `3` for three columns, and so on). This option defaults to `2`.
...
- **type**: Refer to the SS type 1, 2, 3, or 4. H2O-3 currently only supports type 3.

### Common parameters

- [balance_classes](algo-params/balance_classes): (Applicable only for classification) Specify whether to oversample the minority classes to balance the class distribution. This option can increase the data frame size. Majority classes can be undersampled to satisfy the `max_after_balance_size` parameter. This option defaults to `False` (disabled).

- [class_sampling_factors](algo-params/class_sampling_factors): (Applicable only if `balance_classes=True`) Specify the per-class (in lexicographical order) over/under-sampling ratios. By default, these ratios are automatically computed during training to obtain the class balance.

- [early_stopping](algo-params/early_stopping): Specify whether to stop early when there is no more relative improvement on the training or validation set. This option defaults to `False` (disabled).
...
- [max_after_balance_size](algo-params/max_after_balance_size): (Applicable only if `balance_classes=True`) Specify the maximum relative size of the training data after balancing class counts. The value can be \> 1 and defaults to `5.0`.

- [max_iterations](algo-params/max_iterations): Specify the number of training iterations. This option defaults to `0`.
...
- [offset_column](algo-params/offset_column): Specify a column to use as the offset. This will be added to the combination of columns before applying the link function.

- [score_each_iteration](algo-params/score_each_iteration): Specify whether to score during each iteration of the model training. This option is set to `False` (disabled) by default.

- [seed](algo-params/seed): Specify the random number generator (RNG) seed for algorithm components dependent on randomization. The seed is consistent for each H2O instance so that you can create models with the same starting conditions in alternative configurations. This option defaults to `-1` (time-based random number).

- [standardize](algo-params/standardize): Specify whether to standardize the numeric columns to have a mean of zero and unit variance. This option defaults to `True` (enabled).
...
- [stopping_rounds](algo-params/stopping_rounds): Stops training when the option selected for `stopping_metric` doesn't improve for the specified number of training rounds, based on a simple moving average. This option defaults to `0` (no early stopping). The metric is computed on the validation data (if provided); otherwise, training data is used.
...
![Total sum of squares decomposition (Equation 1)](../images/ss_decomp.png)
...
|-----------------|-------------------|-------------------------|--------------------------------------------|---------------------------------------------------|
| R               | $r-1$             | $SS(R\vert C,R:C)$           | Coefficients for R are zero.               | ${\frac{SS(R\vert C,R:C)(n-r*C)}{(r-1)*errorSS}}$      |
| C               | $c-1$             | $SS(C\vert R,R:C)$           | Coefficients for C are zero.               | ${\frac{SS(C\vert R,R:C)(n-r*c)}{(c-1)*errorSS}}$      |
| R:C Interaction | $(r-1)*(c-1)$     | $SS(R:C\vert R,C)$           | Coefficients for interaction R:C are zero. | ${\frac{SS(R:C\vert R,C)(n-r*c)}{(r-1)(c-1)*errorSS}}$ |
| Residual SS     | $n-r*c$           | $errorSS$ of full model |                                            |                                                   |
| Total:          | $n-1$             |                         |                                            |                                                   |
