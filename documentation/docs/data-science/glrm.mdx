---
title: "Generalized low rank models (GLRM)"
---
...
- [k](algo-params/k): *Required* Specify the rank of matrix approximation. This option defaults to `1`.

- [init](algo-params/init1): Specify the initialization mode. One of:
...
- [export_checkpoints_dir](algo-params/export_checkpoints_dir): Specify a directory to which generated models will be automatically exported.
...
- [max_iterations](algo-params/max_iterations): Specify the maximum number of training iterations. The range is 0 to 1e6, and the option defaults to `1000`.

- [max_runtime_secs](algo-params/max_runtime_secs): Specify the maximum allowed runtime in seconds for model training. This option defaults to `0` (disabled).
...
- $Y$ is the archetype
- $X$ contains the coefficients to the archetype
...
- $Y$ is the original archetype generated from the training dataset
- $Xnew$ contains the new coefficients to the archetype

When you call `score` in Java or `predict` in Python or R with a new dataset, $Xnew \times Y$ is returned. However, if you call `transform` in Java or `transform_frame` in Python or R, only $Xnew$ is returned. See the following example for how to use `transform_frame`.
...
[Udell, Madeline, Corinne Horn, Reza Zadeh, and Stephen Boyd. "Generalized low rank models." arXiv preprint arXiv:1410.0342, 2014.](http://arxiv.org/abs/1410.0342)

[Hamner, S.R., Delp, S.L. Muscle contributions to fore-aft and vertical body mass center accelerations over a range of running speeds. Journal of Biomechanics, vol 46, pp 780-787. (2013)](http://nmbl.stanford.edu/publications/pdf/Hamner2012.pdf)
