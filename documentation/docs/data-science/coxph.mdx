...
- [y](algo-params/y) (Python) / **event_column** (R): *Required* Specify the column to use as the dependent variable. The data can be numeric or categorical.
...
> 1)  $n = \tt{iter\ max}$ or
>
> 2)  the log-relative error $LRE\Big(pl\big(\beta^{(n)}\big), pl\big(\beta^{(n+1)}\big)\Big) >= \tt{lre\ min}$,
>
>     where
>
