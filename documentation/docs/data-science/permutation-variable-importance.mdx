...
1)  Variable `V` is randomly shuffled using [Fisher-Yates algorithm](https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle).
...
- **frame**: The dataset to use, both the train and test frames can be reasonable choices but the interpretation differs (see [Should I Compute Importance on Training or Test Data?](https://christophm.github.io/interpretable-ml-book/feature-importance.html#feature-importance-data) from the Interpretable Machine Learning by Christoph Molnar.).
...
When `n_repeats == 1`, the result is similar to the one from `h2o.varimp()`, that is, it contains the following columns `Relative Importance`, `Scaled Importance`, and `Percentage`.
