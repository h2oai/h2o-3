...
In Python, a sparse matrix is specified using `scipy.sparse`. For example:
...
H2O auto-detects two major date/time formats. Because many date time formats are ambiguous (for example, 01/02/03), general date time detection is not used.
...
The second date format is for dates formatted as dd-MMM-yy. The day must be one or two digits with a value ranging from 1 to 31. The month must be either a three-letter abbreviation or the full month name, and is not case sensitive. The year must be either two or four digits. In agreement with [POSIX](https://en.wikipedia.org/wiki/POSIX) standards, two-digit dates \>= 69 are assumed to be in the twentieth century (for example, 1969) and the rest are part of the twenty-first century. This date format can be followed by either a space or colon character and then a time. The '-' between the values is optional.

Times are specified as HH:mm:ss. HH is a two-digit hour and must be a value between 0–23 (for 24-hour time) or 1–12 (for a twelve-hour clock). mm is a two-digit minute value and must be a value between 0–59. ss is a two-digit second value and must be a value between 0–59. This format can be followed with either milliseconds, nanoseconds, and/or the cycle (that is, AM/PM). If milliseconds are included, the format is HH:mm:ss:SSS. If nanoseconds are included, the format is HH:mm:ss:SSSnnnnnn. H2O only stores fractions of a second up to the millisecond, so accuracy may be lost. Nanosecond parsing is only included for convenience. A valid time can also end with a space character and then either "AM" or "PM." For this format, the hours must range from 1 to 12. Within the time, the ':' character can be replaced with a '.' character.

------------------------------------------------------------------------

## How does H2O handle name collisions/conflicts in the dataset?

If there is a name conflict (for example, column 48 isn't named, but C48 already exists), the column name is concatenated to itself until a unique name is created. For the previously cited example, H2O tries renaming the column to C48C48, then C48C48C48, and so on until an unused name is generated.
...
When H2O parses a file, it uses rules to determine whether a column is numeric, enum, etc. If you need a column to be read as a specific type, especially the columns that can be interpreted as enum or numeric, you can use `col_types` in Python or `col.types` in R when you import a file.
