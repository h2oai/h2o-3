...
- Test connectivity using curl: First, log in to the first node and run `curl http://<Node2IP>:54321` (where `<Node2IP>` is the IP address of the second node). Then, log in to the second node and run `curl http://<Node1IP>:54321` (where `<Node1IP>` is the IP address of the first node). Look for output from H2O.
...
The syntax below comes from [https://github.com/ck37/savio-notes/blob/master/h2o-slurm-multinode.Rmd](https://github.com/ck37/savio-notes/blob/master/h2o-slurm-multinode.Rmd) and describes how to create a multi-node H2O cluster on a Simple Linux Utility for Resource Management (SLURM) system using R.
...
More information is available here: [https://docs.oracle.com/javase/8/docs/technotes/guides/vm/gctuning/g1_gc.html](https://docs.oracle.com/javase/8/docs/technotes/guides/vm/gctuning/g1_gc.html)
