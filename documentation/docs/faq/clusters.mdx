...
- Test connectivity using curl: First, log in to the first node and enter curl `http://<ip>:54321` (where is the IP address of the second node. Then, log in to the second node and enter curl `http://<ip>:54321` (where is the IP address of the first node). Look for output from H2O.
