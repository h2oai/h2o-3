...
This example uses the Boston Housing data and [H2O's GLM algorithm](data-science/glm#generalized-linear-model-glm) to predict the median home price using all available features. The dataset includes the following columns:
...
The example below uses the [K-Means](data-science/k-means#k-means-clustering) algorithm to build a simple clustering model of the Iris dataset.
...
This example uses the [Isolation Forest](data-science/if#isolation-forest) algorithm to detect anomalies in the Electrocardiograms (ECG) dataset.
