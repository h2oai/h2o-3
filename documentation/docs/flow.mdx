...
1.  First [Download H2O](http://www.h2o.ai/download/). This will download a zip file in your Downloads folder that contains everything you need to get started. Alternatively, you can run the following from your command line.

> ``` bash
> curl -o h2o.zip http://download.h2o.ai/versions/h2o-3.42.0.2.zip
> ```
...
2.  Next in your terminal, enter the following command lines one at a time. The first line changes into your Downloads folder, the second line unzips your zipfile, the third line changes into your h2o-3-3.42.0.2 folder, and the fourth line runs your jar file.
...
3.  Finally, to start Flow point your browser to [http://localhost:54321](http://localhost:54321).
...
**Note**: If your H2O package is not in the Downloads folder, replace the following path ~/Downloads/h2o-3.42.0.2 with the correct path to your h2o-3.42.0.2 package.

``` bash
cd ~/Downloads/h2o-3.42.0.2 
java -jar h2o.jar
```
...
![Flow keyboard shortcuts](images/Flow_shortcuts.png)
...
![Flow Assist Me button](images/Flow_AssistMeButton.png)
...
![Flow assist command output](images/Flow_assist.png)

The **Help** sidebar contains multiple resources to help you get started with Flow.

**Note**: To hide the sidebar, click the \>\> button above it

![Flow sidebar hide button](images/Flow_SidebarHide.png)
...
![Flow example flows list](images/Flow_ExampleFlows.png)
...
![Flow REST API documentation tab](images/Flow_REST_docs.png)

Before getting started with H2O Flow, make sure you understand the different cell modes. (Refer to [Understanding Cell Modes](#understanding-cell-modes).) Certain actions can only be performed when the cell is in a specific mode.
...
Clips enable you to save cells containing your workflow for later reuse. To save a cell as a clip, click the paperclip icon to the right of the cell (highlighted in the red box in the following screenshot). ![Paperclip icon](images/Flow_clips_paperclip.png)

To use a clip in a workflow, click the "Clips" tab in the sidebar on the right.

![Flow clips sidebar](images/Flow_clips.png)
...
:::note
The default clips listed under "System" cannot be deleted.
:::

Deleted clips are stored in the trash. To permanently delete all clips in the trash, click the **Empty Trash** button.

:::note
Saved data, including flows and clips, are persistent as long as the same IP address is used for the cluster. If a new IP is used, previously saved flows and clips are not available.
:::
...
![Flow outline tab](images/Flow_outline.png)

### Saving flows

You can save your flow for later reuse. After a Flow is saved, you can load it by clicking on the **Flows** tab in the right sidebar. Then in the pop-up confirmation window that appears, select **Load Notebook**. Refer to [Loading Flows](flow#loading-flows) for more information.
...
![Flow rename flow field](images/Flow_rename.png)

To confirm the name, click the checkmark to the right of the name field.

![Flow confirm rename](images/Flow_rename2.png)
...
![Flow saved flows list](images/Flow_flows.png)
...
To create a copy of the current flow, select the **Flow** menu, then click **Make a Copy**. The name of the current flow changes to `Copy of <FlowName>` (where `<FlowName>` is the name of the flow). You can save the duplicated flow using this name by clicking **Flow** \> **Save Flow**, or rename it before saving. (Refer to [Saving Flows](#saving-flows).)
...
![Flow load notebook confirmation](images/Flow_confirmreplace.png)
...
![Flow open flow dialog](images/Flow_Open.png)
...
Cells have two modes: Edit and Command.
...
![Flow cell in edit mode](images/Flow_EditMode.png)
...
>
> - clicking the **Run** button ![Flow - Run Button](images/Flow_RunButton.png) or
> - pressing **Ctrl+Enter**
>
![Flow markdown cell rendered](images/Flow_markdown.png)

- **CS**: Code (default)

![Flow code cell example](images/Flow_parse_code_ex.png)

- **RAW**: Raw format (for code comments)

![Flow raw format cell](images/Flow_raw.png)

- **H\[1-6\]**: Heading level (where 1 is a first-level heading)

![Flow heading level cells](images/Flow_headinglevels.png)

:::note
If there is an error in the cell, the flag is red.
:::

![Flow red error flag on cell](images/Flow_redflag.png)

If the cell is executing commands, the flag is teal. The flag returns to yellow when the task is complete.

![Flow teal running flag on cell](images/Flow_cellmode_runningflag.png)
...
![Flow run buttons toolbar](images/Flow_RunButtons.png)
...
- To run an individual cell in a flow, confirm the cell is in Edit Mode (refer to [Using Edit Mode](#using-edit-mode)), then:
...
![Flow progress bar with stop button](images/Flow_progressbar.png)

When the flow is complete, a message displays in the upper right.

![Flow - Completed Successfully](images/Flow_run_pass.png) ![Flow - Did Not Complete](images/Flow_run_fail.png)
...
The following commands must be entered in Command Mode. (Refer to [Using Command Mode](#using-command-mode).)
...
![Flow variable definition in code cell](images/Flow_VariableDefinition.png)

2.  Run the cell. H2O validates the variable.

![Flow variable validation result](images/Flow_VariableValidation.png)

3.  Use the variable in another code cell (for example, `importFiles [locA]`).

![Flow variable used in code cell](images/Flow_VariableExample.png)

To further simplify your workflow, you can save the cells containing the variables and definitions as clips. (Refer to [Using Clips](#using-clips).)

### Using Flow buttons

A series of buttons at the top of the page below the flow name lets you save the current flow, add a new cell, move cells up or down, run the current cell, and cut, copy, or paste the current cell. If you hover over the button, a description of the button's function displays.

![Flow action buttons below flow name](images/Flow_buttons.png)
...
If you don't have any data of your own to work with, you can find some example datasets at [http://data.h2o.ai](http://data.h2o.ai).

### Importing files

H2O Flow supports multiple ways to import data:

- Click the **Assist Me!** button in the row of buttons below the menus, then click the **importFiles** link. Enter the file path in the auto-completing **Search** entry field and press **Enter**. Select the file from the search results and confirm it by clicking the **Add All** link. ![Flow - Import Files Auto-Suggest](images/Flow_Import_AutoSuggest.png)
...
- To import the selected files, click the **Import** button.
...
![Flow import results summary](images/Flow_import_results.png)
...
![Flow upload dataset dialog](images/Flow_UploadDataset.png)

When the file has uploaded successfully, a message displays in the upper right and the **Setup Parse** cell displays.

![Flow file upload success message](images/Flow_FileUploadPass.png)
...
![Flow parse setup cell](images/Flow_parse_setup.png)
...
![Flow column name entry field highlighted](images/Flow_ColNameEntry.png)
...
![Flow previous and next page buttons](images/Flow_PageButtons.png)

After making your selections, click the **Parse** button. The code for the current job displays.

![Flow parse code cell example](images/Flow_parse_code_ex.png)
...
![Flow impute data options](images/Flow_impute.png)
...
![Flow jobs list](images/Flow_getJobs.png)
...
![Flow view job model details](images/Flow_ViewJob_Model.png)
...
**Note**: For a better understanding of how jobs work, make sure to review the [Viewing Frames](#viewing-frames) section as well.
...
![Flow model builder cell](images/Flow_ModelBuilder.png)

In the **Build a Model** cell, select an algorithm from the drop-down menu. (Refer to the Data Science Algorithms section for information about the available algorithms.) Available algorithms include:
...
>
![Flow model type dropdown menu](images/Flow_model_dropdown.png)
...
- **solver**: (GLM) Select the solver to use (AUTO, IRLSM, L_BFGS, COORDINATE_DESCENT_NAIVE, or COORDINATE_DESCENT). IRLSM is fast on on problems with a small number of predictors and for lambda-search with L1 penalty, while [L_BFGS](http://cran.r-project.org/web/packages/lbfgs/vignettes/Vignette.pdf) scales better for datasets with many columns. COORDINATE_DESCENT is IRLSM with the covariance updates version of cyclical coordinate descent in the innermost loop. COORDINATE_DESCENT_NAIVE is IRLSM with the naive updates version of cyclical coordinate descent in the innermost loop. COORDINATE_DESCENT_NAIVE and COORDINATE_DESCENT are currently experimental.
...
- **hidden**: (DL) Specify the hidden layer sizes (for example, 100,100). For Grid Search, use comma-separated values: (10,10),(20,20,20). The default value is \[200,200\]. All specified values must be positive.
...
- **fold_assignment**: (GLM, GBM, DL, DRF, K-Means, XGBoost) (Applicable only if a value for **nfolds** is specified and **fold_column** is not selected.) Select the cross-validation fold assignment scheme. The available options are Random or [Modulo](https://en.wikipedia.org/wiki/Modulo_operation).
...
- **train_samples_per_iteration**: (DL) Specify the number of global training samples per MapReduce iteration. To specify one epoch, enter 0. To specify all available data (for example, replicated training data), enter -1. To use the automatic values, enter -2.
...
- **hidden_dropout_ratios**: (DL) Specify the hidden layer dropout ratios to improve generalization. Specify one value per hidden layer, each value between 0 and 1 (exclusive). No default value is set. This option is applicable only if *TanhwithDropout*, *RectifierwithDropout*, or *MaxoutWithDropout* is selected from the **Activation** drop-down list.

- **distribution**: (GBM, DL, XGBoost) Specify the distribution (that is, the loss function). The options are AUTO, bernoulli, multinomial, gaussian, poisson, gamma, or tweedie.
...
- **monotone_constraints**: (XGBoost, GBM) A mapping representing [monotonic constraints](https://xiaoxiaowang87.github.io/monotonicity_constraint/). Use +1 to enforce an increasing constraint and -1 to specify a decreasing constraint. Note that constraints can only be defined for numerical columns. Also note that in GBM, this option can only be used when the distribution is either `gaussian` or `bernoulli`.
...
- **tree_method**: (XGBoost) Specify the construction tree method to use. This can be one of the following:
  ...
  - auto (default): Allow the algorithm to choose the best method. For small to medium dataset, `exact` will be used. For large datasets, `approx` will be used.
...
- **dmatrix_type**: (XGBoost) Specify the type of DMatrix. Valid options are `auto`, `dense`, and `sparse`. Note that for a DMatrix type of `sparse`, NAs and 0 are treated equally.
...
- **ties**: (CoxPH) The approximation method for handling ties in the partial likelihood. This can be either **efron** (default) or **breslow**). See the [Cox Proportional Hazards Model Details](data-science/coxph#cox-proportional-hazards-model-details) section below for more information about these options.

- **init**: (CoxPH) (Optional) Initial values for the coefficients in the model. This value defaults to 0.

- **lre_min**: (CoxPH) A positive number to use as the minimum log-relative error (LRE) of subsequent log partial likelihood calculations to determine algorithmic convergence. The role this parameter plays in the stopping criteria of the model fitting algorithm is explained in the [Cox Proportional Hazards Model Algorithm](data-science/coxph#cox-proportional-hazards-model-algorithm) section below. This value defaults to 9.
...
- **max_w2**: (DL) Specify the constraint for the squared sum of the incoming weights per unit (for example, for Rectifier).
...
- **sparsity_beta**: (DL) Specify the sparsity-based regularization optimization. For more information, refer to the following [link](http://www.mit.edu/~9.520/spring09/Classes/class11_sparsity.pdf).
...
- **backend**: (XGBoost) Specify the backend type. This can be one of the following: `auto`, `gpu`, or `cpu`.
...
![Flow list of models](images/Flow_getModels.png)
...
![Flow model details view](images/Flow_viewModel.png)
...
- **Download POJO**: Generates a Plain Old Java Object (POJO) that can use the model outside of H2O. Note that a POJO can be run in standalone mode or embedded in any Java application. To make the POJO work in your Java application, you will also need the `h2o-genmodel.jar` file (available via the **Download Generated Model** button, from the **Admin** menu, or in `h2o-3/h2o-genmodel/build/libs/h2o-genmodel.jar`). Note that POJOs are are not supported for XGBoost models.
...
![Flow export model cell](images/ExportModel.png)

**To import a built model:**

1.  Click the **Model** menu at the top of the screen.
2.  Select *Import Model…*
3.  Enter the location of the model in the *Path:* entry field. **Note**: The file path must be complete (for example, `Users/h2o-user/h2o-3/exported_models`). Do not rename models while importing.
...
![Flow import model cell](images/ImportModel.png)

------------------------------------------------------------------------

### Run AutoML

AutoML automatically trains and tunes models while requiring as few parameters as possible. A user is only required to point to a dataset, identify the response column and optionally specify a time constraint, a maximum number of models constraint, and early stopping parameters. AutoML will then begin training models and will stop as specified in the configuration (that is, when the maximum number of models has been reached, when the maximum run time has been reached, or when the stopping criteria are met).
...
![Flow Run AutoML menu option](images/Flow_Model_RunAutoML.png)
...
![Flow AutoML configuration form](images/Flow_RunAutoML.png)

Click the **View** button to view the Leaderboard and/or monitor the current AutoML run.

![Flow View button for AutoML run](images/Flow_ViewButton.png)
...
![Flow AutoML leaderboard](images/Flow_ViewLeaderboard.png)
...
![Flow grid search checkbox column](images/Flow_GridSearch.png)
...
![Flow confusion matrix table](images/Flow_ConfusionMatrix.png)

**ROC Curve**: (DRF) A [ROC Curve](https://en.wikipedia.org/wiki/Receiver_operating_characteristic) is a graph that represents the ratio of true positives to false positives. (For more information, refer to the Linear Digressions [podcast](http://lineardigressions.com/episodes/2017/1/29/rock-the-roc-curve) describing ROC Curves.) To view a specific threshold, select a value from the drop-down **Threshold** list. To view any of the following details, select it from the drop-down **Criterion** list:
...
![Flow ROC curve graph](images/Flow_ROC.png)
...
![Flow hit ratio table](images/HitRatioTable.png)
...
![Flow standardized coefficient magnitudes chart](images/SCM.png)
...
![Flow Score menu partial dependence plots option](images/score_pdp_menu.png)
...
>
![Flow compute partial dependence plots form](images/compute_pdp.png)

3.  After the job is finished, click **View** to see the plots.

![Flow partial dependence plot summary](images/pdp_summary.png)
...
![Flow make prediction form](images/Flow_makePredict.png)
...
![Flow list of stored predictions](images/Flow_getPredict.png)
...
This information is particularly useful for direct marketing applications, for example. The gains/lift chart shows the effectiveness of the current models compared to a baseline, allowing users to quickly identify the most useful model.
...
![Flow gains/lift chart](images/GainsLift.png)
...
![Flow gains/lift table](images/GainsLiftTable.png)
...
![Flow create synthetic frame dialog](images/Flow_CreateFrame.png)
...
![Flow frame summary cell](images/Flow_getFrame.png)
...
When you view a frame, you can navigate to the necessary level of detail (such as a specific column or row) using the **Inspect** button or by clicking the links. The following screenshot displays the results of clicking the **Inspect** button for a frame.

![Flow inspect frame results](images/Flow_inspectFrame.png)

This screenshot displays the results of clicking the **columns** link.

![Flow inspect columns view](images/Flow_inspectCol.png)
...
![Flow list of all frames](images/Flow_getFrames.png)
...
![Flow split frame options](images/Flow_splitFrame.png)
...
![Flow plot frame options](images/Flow_plottingFrames.png)
...
![Flow plotted frame data](images/Flow_plot.png)
...
![Flow cluster status summary](images/Flow_CloudStatus.png)
...
![Flow inspect log view](images/Flow_viewLog.png)
...
To download the logs for further analysis, click the **Admin** menu, then click **Download Log**. A new window opens and the logs download to your default download folder. You can close the new window after downloading the logs. Send the logs to [h2ostream](mailto:h2ostream@googlegroups.com) or file a GitHub issue for issue resolution. (Refer to [Reporting Issues](#reporting-issues).)
...
![Flow stack trace view](images/Flow_stacktrace.png)
...
![Flow network test results](images/Flow_NetworkTest.png)
...
![Flow profiler view](images/Flow_profiler.png)
...
![Flow timeline of events](images/Flow_timeline.png)
...
If you have questions or ideas to share, please post them to the [H2O community site on Stack Overflow](http://stackoverflow.com/questions/tagged/h2o).
...
4.  Enter your question and click the red **Post** button. If you are requesting assistance for an error you experienced, be sure to include your logs. (Refer to [Downloading Logs](#downloading-logs).)
