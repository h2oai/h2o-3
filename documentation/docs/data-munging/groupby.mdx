...
:::note
If no arguments are given to the aggregation (for example, `max()` in `grouped.sum(col="X1", na="all").mean(col="X5", na="all").max()`), then it is assumed that the aggregation should apply to all columns except the GroupBy columns.
:::
...
# Find the number of flights by airport using gb.control to handle NAs:
