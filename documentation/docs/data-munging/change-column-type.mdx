...
H2O-3 represents dates as (unix) timestamps. These are then raw input to the algorithm. However, raw timestamps are rarely useful on their own. You are expected to do your own feature engineering and break the data into day, month, and year using the functions H2O-3 provides.
