...
- Linux kernel version 3.8+ or Mac OS 10.6+
- VirtualBox
...
:::note
- Older Linux kernel versions can cause kernel panics that break Docker. Workarounds exist, but attempt them at your own risk. Check the version of your kernel by running `uname -r`.
...
:::note
By default, Docker allocates 2 GB of memory for Mac installations. Be sure to increase this value. H2O recommends 3–4 times the size of the dataset for the amount of memory required.
:::
...
2.  Download or create a Dockerfile, which is a build recipe that builds the container. Download and use our [Dockerfile template](https://github.com/h2oai/h2o-3/blob/master/Dockerfile):
...
Navigate to the `/opt` directory and launch H2O-3. Update the value of `-Xmx` to the amount of memory you want to allocate to the H2O-3 instance. By default, H2O-3 will launch on port `54321`.
...
<Tabs groupId="lang">
<TabItem value="on-linux" label="On Linux" default>
...
After the container starts, connect from your Python client:
