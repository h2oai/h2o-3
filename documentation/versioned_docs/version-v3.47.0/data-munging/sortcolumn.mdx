...
Use the `sort` function in Python or the `arrange` function in R to create a new frame that is sorted by one or more columns in ascending (default) or descending order.
