...
To access the Google Cloud Store Object Store, Google's cloud storage connector, `gcs-connector-latest-hadoop2.jar` is required. The official documentation and driver can be found [here](https://cloud.google.com/hadoop/google-cloud-storage-connector).
...
core-site.xml must be configured for at least the following properties (class, project-id, bucketname) as shown in the example below. A full list of configuration options is found [here](https://github.com/GoogleCloudPlatform/bigdata-interop/blob/master/gcs/conf/gcs-core-default.xml).
