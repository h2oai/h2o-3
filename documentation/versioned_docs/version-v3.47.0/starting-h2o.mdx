---
title: "Starting H2O-3"
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

H2O-3 can be started in several ways depending on which client you would like to use. The instructions below assume that you already downloaded and installed H2O-3. If you have not, then please refer to the [Downloading & Installing H2O](downloading) section.
...
- `max_mem_size`: Maximum memory to use for the new H2O-3 server. Integer input will be evaluated as gigabytes. Other units can be specified by passing in a string (for example, `"160M"` for 160 megabytes).
...
- `min_mem_size`: Minimum memory to use for the new H2O-3 server. Integer input will be evaluated as gigabytes. Other units can be specified by passing in a string (for example, `"160M"` for 160 megabytes).
...
This section describes how to run H2O-3 in an Anaconda Cloud environment. This section assumes that you have installed H2O-3 on Anaconda using the instructions in the [Install on Anaconda Cloud](downloading#install-on-anaconda-cloud) section.
...
1.  Import the H2O-3 and GBM modules.
    ...
    ![Import the H2O-3 and GBM modules](images/anaconda_import_module.png)

2.  Initialize H2O-3 using `h2o.init()`.

    ![Initialize H2O-3 with h2o.init()](images/anaconda_init.png)

3.  Import the Airlines dataset. This dataset will be used to classify whether a flight will be delayed.

    ![Import the Airlines dataset](images/anaconda_import_airlines.png)

4.  Convert columns to factors.

    ![Convert columns to factors](images/anaconda_convert_columns.png)

5.  Set the predictor names and the response column name.

    ![Set the predictor names and response column](images/anaconda_predictor_response.png)

6.  Split the dataset into training and validation sets.

    ![Split the dataset into training and validation sets](images/anaconda_split_data.png)

7.  Specify the number of bins that will be included in the histogram and then split.

    ![Specify the number of histogram bins](images/anaconda_nbins_cats.png)

8.  Train the models.

    ![Train the models](images/anaconda_train_model.png)

9.  Print the AUC scores for the training data and the validation data.

    ![Print the AUC scores for training and validation data](images/anaconda_print_auc.png)
...
- `nthreads`: This launches H2O-3 using all available CPUs and is only applicable if you launch H2O locally using R. If you start H2O-3 locally outside of R, the nthreads parameter is not applicable.
...
- `max_log_file_size`: Maximum size of INFO and DEBUG log files. The file is rolled over after the specified size has been reached. The range for this option is `1 MB` to `99999 MB`. The value defaults to `3 MB`.

- `max_mem_size`: (Optional) A character string specifying the maximum size, in bytes, of the memory allocation pool to H2O-3. This value must be a multiple of 1024 greater than 2 MB. Append the letter `m` or `M` to indicate megabytes, or `g` or `G` to indicate gigabytes.
...
- `min_mem_size`: (Optional) A character string specifying the minimum size, in bytes, of the memory allocation pool to H2O-3. This value must a multiple of 1024 greater than 2 MB. Append the letter `m` or `M` to indicate megabytes, or `g` or `G` to indicate gigabytes.
...
For more detailed instructions on how to build and launch H2O-3 (including how to clone the repository, how to pull from the repository, and how to install required dependencies), see the [developer documentation](https://github.com/h2oai/h2o-3#41-building-from-the-command-line-quick-start).
...
- `-Xmx<Heap Size>`: Configure the memory allocation option `-Xmx` to set the total heap size for an H2O-3 node. By default, this option is set to 1 GB (`-Xmx1g`). When launching nodes, we recommend allocating a total of four times the memory of your data.
...
> - IPv6: `-ip 2001:db8:1234:0:0:0:0:1` (Short version of IPv6 with `::` is not supported.)
  ...
  :::note
  If you are selecting a link-local address `fe80::/96`, it is necessary to specify the *zone index* (for example, `%en0` for `fe80::2acf:e9ff:fe15:e0f3%en0`) in order to select the right interface.
  :::
...
- `-file_deny_glob <GLOB pattern>`: Specify the [glob](https://docs.oracle.com/javase/tutorial/essential/io/fileOps.html#glob) pattern to deny access to certain directories. This parameter is added to remove vulnerabilities CVE-2023-6038, CVE-2023-6569 and CVE-2024-5986. The default is `{/bin/*,/etc/*,/var/*,/usr/*,/proc/*,**/.**}`.
...
To start an H2O-3 node with 4 GB of memory and a default cluster name: `java -Xmx4g -jar h2o.jar`

To start an H2O-3 node with 6 GB of memory and a specific cluster name: `java -Xmx6g -jar h2o.jar -name MyCluster`

To start an H2O-3 cluster with three 2 GB nodes using the default cluster names: `java -Xmx2g -jar h2o.jar &   java -Xmx2g -jar h2o.jar &   java -Xmx2g -jar h2o.jar &`
...
See the following [image on scopes for more information](http://www.tcpipguide.com/free/diagrams/ipv6scope.png).
...
- Set the correct cluster size for your given dataset size. The rule of thumb is to use at least 4 times the size of your data. For example, if the dataset is 10 GB, you should allocate at least 40 GB of memory.
