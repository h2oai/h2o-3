---
title: "Saving, Loading, Downloading, and Uploading Models"
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

This section describes how to save, load, download, and upload binary and [MOJO models](productionizing#about-pojos-and-mojos) using R, Python, and Flow.
...
In R and Python, you can save a model locally or to HDFS using `h2o.saveModel` in R or `h2o.save_model` in Python. This function accepts the model object and the file path. If no path is specified, then the model will be saved to the current working directory. After the model is saved, you can load it using `h2o.loadModel` in R or `h2o.load_model` in Python. You can also upload a model from a local path to your H2O cluster.
...
The steps for saving and loading models in Flow are described in the **Using Flow - H2O's Web UI** section. Specifically, refer to [Exporting and Importing Models](flow#exporting-and-importing-models) for information about exporting and importing binary models in Flow.

## MOJO Models

### Introduction

The MOJO import functionality provides a means to use external, pre-trained models in H2O - mainly for the purpose of scoring. Depending on each external model, metrics and other model information might be obtained as well. Currently, only selected H2O MOJOs are supported. (See the [Building a MOJO](mojo-quickstart.mdx#building-a-mojo) section for information about creating MOJOs.)
...
**Note**: Your model will not produce MOJOs if you build it using [interactions](data-science/algo-params/interactions).
...
- [model_id](data-science/algo-params/model_id): Specify a custom name for the model to use as a reference.
