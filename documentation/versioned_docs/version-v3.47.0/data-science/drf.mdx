---
title: "Distributed Random Forest (DRF)"
---
...
DRF supports importing and exporting [MOJOs](../save-and-load-model#supported-mojos).
...
- [binomial_double_trees](algo-params/binomial_double_trees): (Binary classification only) Build twice as many trees (one per class). Enabling this option can lead to higher accuracy, while disabling can result in faster model building. This option defaults to `False` (disabled).
...
- [calibrate_model](algo-params/calibrate_model): Use Platt scaling to calculate calibrated class probabilities. Calibration can provide more accurate estimates of class probabilities. Defaults to `False`.
- [check_constant_response](algo-params/check_constant_response): Check if the response column is a constant value. If enabled (default), then an exception is thrown if the response column is a constant value. If disabled, then the model will train regardless of the response column being a constant value or not. This option defaults to `True` (enabled).
...
- [histogram_type](algo-params/histogram_type): By default (`AUTO`), DRF bins from min to max in steps of $\frac{(\text{max-min})}{N}$. `Random` split points or quantile-based split points can be selected as well. `RoundRobin` can be specified to cycle through all histogram types (one per tree). Use this option to specify the type of histogram to use for finding optimal split points. One of:
  - `AUTO` (default)
  - `UniformAdaptive`
  - `UniformRobust`
  - `Random`
  - `QuantilesGlobal`
  - `RoundRobin`
- [max_depth](algo-params/max_depth): Specify the maximum tree depth. Higher values will make the model more complex and can lead to overfitting. Setting this value to `0` specifies no limit. This value defaults to `20`.
- [min_rows](algo-params/min_rows) (Python API) / **node_size** (R API): Specify the minimum number of observations for a leaf. This value defaults to `1`.
- [min_split_improvement](algo-params/min_split_improvement): The value of this option specifies the minimum relative improvement in squared error reduction in order for a split to happen. When properly tuned, this option can help reduce overfitting. Optimal values would be in the 1e-10 to 1e-3 range. This value defaults to `1e-05`.
...
- [balance_classes](algo-params/balance_classes): (Applicable for classification only) Oversample the minority classes to balance the class distribution. This can increase the data frame size. This option defaults to `False` (disabled)
...
- [keep_cross_validation_predictions](algo-params/keep_cross_validation_predictions): Enable this option to keep the cross-validation prediction. This option defaults to `False` (disabled).
...
- A graph of the [ROC curve](https://en.wikipedia.org/wiki/Receiver_operating_characteristic) (TPR vs. FPR)
...
> Large numbers of categoricals are handled efficiently—there is never any one-hot encoding.
...
[Building Random Forest at Scale](https://www.slideshare.net/0xdata/rf-brighttalk) from Sri Ambati

## References

[P. Geurts, D. Ernst., and L. Wehenkel, "Extremely randomized trees," Machine Learning, 63(1), 3-42, 2006.](http://link.springer.com/article/10.1007%2Fs10994-006-6226-1)
