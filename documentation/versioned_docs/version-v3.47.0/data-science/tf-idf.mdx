...
TF-IDF (Term Frequency–Inverse Document Frequency) is a statistical measure that aims to reflect how important a word is to a document in a collection of documents (also known as a [corpus](https://en.wikipedia.org/wiki/Corpus_linguistics)).
...
where natural logarithm is being used (that is, the logarithm has a base equal to $e$). Based on the equation above, IDF of a word present in all documents from the corpus is equal to 0, and the fewer documents contain the word, the higher its IDF value.
...
A Jupyter notebook with a Python demo is available [here](https://github.com/h2oai/h2o-3/blob/master/h2o-py/demos/tf-idf.ipynb).
...
# Construct data
documents = c(
    'H2O is an in-memory platform for distributed, scalable machine learning. H2O uses familiar interfaces like R, Python, Scala, Java, JSON and the Flow notebook/web interface, and scales seamlessly across multi-node clusters.',
