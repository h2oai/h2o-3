...
- [`max_runtime_secs`](algo-params/max_runtime_secs) (Defaults to `0`/disabled.)
...
- [`stopping_rounds`](algo-params/stopping_rounds) - Defaults to:

  > - AutoML: `3`
  > - Deep Learning: `5`
  > - DRF, GBM, XGBoost: `0`/disabled

- [`stopping_tolerance`](algo-params/stopping_tolerance) - Defaults to:
...
- [`stopping_metric`](algo-params/stopping_metric) - Defaults to:
...
Additionally, you can take [`score_tree_interval`](algo-params/score_tree_interval) and/or [`score_each_iteration`](algo-params/score_each_iteration) into account when using these early stopping methods. The stopping rounds applies to the number of scoring iterations H2O-3 has performed, so regular scoring iterations of a small size can help control early stopping the most (though there is a speed tradeoff to scoring more often). The default is to use H2O-3's assessment of a reasonable ratio of training time to scoring time, which often results in inconsistent scoring gaps.

## Early stopping in GLM and GAM

In GLM and GAM, the following additional parameters are used for early stopping:

- [`early_stopping`](algo-params/early_stopping) (Defaults to enabled.)
- [`max_active_predictors`](algo-params/max_active_predictors) (Default varies based on the `solver`.)
- [`stopping_rounds`](algo-params/stopping_rounds) (Defaults to `0` in GLM and GAM.)
- [`stopping_tolerance`](algo-params/stopping_tolerance) (Defaults to `0.001` in GLM and GAM.)
- [`stopping_metric`](algo-params/stopping_metric) (Defaults to `"logloss"` for classification and `"deviance"` for regression.)
