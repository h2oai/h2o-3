...
This implementation only supports numeric features and a binary target variable. Additionally, [it utilizes binning](#binning-for-tree-building) for efficient tree building when dealing with large datasets. The splitting rule employed by the algorithm is entropy.
...
- [max_depth](algo-params/max_depth): Specify the maximum depth of the final decision tree. The final depth can be less than the defined `max_depth` if another stopping criterion is met first. This value defaults to `20`.
- [min_rows](algo-params/min_rows): Specify the minimum number of observations for a leaf. This value defaults to `10`.
...
5.  **Stopping criteria**: The recursive division process stops when specific conditions are met. The employed criteria include:
    ...
    1.  *Maximum tree depth*: The process stops when the tree reaches a predefined maximum depth (set with `max_depth`).
    2.  *Minimum number of samples*: The process stops when a node contains a minimum number of samples or instances (set with `min_rows`).
    3.  *Limit of entropy improvement*: The process stops when the improvement of entropy between the parent node and the current split reaches a specified minimum threshold (refer to the [Entropy as a splitting rule](#entropy-as-a-splitting-rule) section for more information).
...
2.  **Binning algorithm**: Applying a binning algorithm to divide the feature values into a specified number of bins (this technique is equal width binning).
...
- $p_1$ is the proportion of positive (or class 1) samples in $S$
- $p_0$ is the proportion of negative (or class 0) samples in $S$
...
20. Hastie, R. Tibshirani, J. Friedman, “The elements of Statistical Learning Data Mining, Inference and Prediction”, Chapter 9.2, Second Edition, Springer Series in Statistics, 2017.
