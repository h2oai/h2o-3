...
For more information, refer to [Huang (1997): fast clustering for large categorical datasets](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.134.83&rep=rep1&type=pdf) and [Huang (1998): k-Means extensions for categorical values](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.15.4028&rep=rep1&type=pdf).

### MOJO support

K-Means currently only supports exporting [MOJOs](../save-and-load-model#supported-mojos).
...
- [cluster_size_constraints](algo-params/cluster_size_constraints): An array specifying the minimum number of points that should be in each cluster. The length of the constraints array has to be the same as the number of clusters.

- [estimate_k](algo-params/estimate_k): Specify whether to estimate the number of clusters ($\leq$ `k`) iteratively (independent of `seed`) and deterministically (beginning with `k=1,2,3...`). If enabled, for each `k` the estimate will go up to `max_iteration`. This option defaults to `False` (disabled).

- [init](algo-params/init1): Specify the initialization mode. The options are :
...
- [k](algo-params/k): Specify the number of clusters (groups of data) in a dataset that are similar to one another. This option defaults to `1`.
...
- [user_points](algo-params/user_points): Specify a dataframe, where each row represents an initial cluster center.
...
- [max_iterations](algo-params/max_iterations): Specify the maximum number of training iterations. The range is 0 to 1e6, and the default value is `10`.

- [max_runtime_secs](algo-params/max_runtime_secs): Maximum allowed runtime in seconds for model training. This option defaults to `0` (disabled).
...
- [nfolds](algo-params/nfolds): Specify the number of folds for cross-validation. This option defaults to `0`.
...
- [standardize](algo-params/standardize): Enable this option to standardize the numeric columns to have a mean of zero and unit variance. Standardization is highly recommended; if you do not use standardization, the results can include components that are dominated by variables that appear to have larger variances relative to other attributes as a matter of scale, rather than true contribution. This option defaults to `True` (enabled).
...
- [validation_frame](algo-params/validation_frame): Specify the dataset to calculate validation clustering metrics.

- [x](algo-params/x): Specify a vector containing the names or indices of the predictor variables to use when building the model. If `x` is missing, then all columns are used.
...
More information about how to convert the standard K-means algorithm to the Minimal Cost Flow problem is described in this paper: [https://pdfs.semanticscholar.org/ecad/eb93378d7911c2f7b9bd83a8af55d7fa9e06.pdf](https://pdfs.semanticscholar.org/ecad/eb93378d7911c2f7b9bd83a8af55d7fa9e06.pdf).
...
- 10,000 rows, 5 features: ~9 min 21 s
- 20,000 rows, 5 features: ~39 min 27 s
- 30,000 rows, 5 features: ~1 hr 26 min 43 s
- 40,000 rows, 5 features: ~2 hr 13 min 31 s
- 50,000 rows, 5 features: ~4 hr 4 min 18 s
...
To solve Constrained K-means in a shorter time, you can use the [H2O Aggregator algorithm](aggregator) to aggregate data to smaller sizes first and then pass this data to the Constrained K-means algorithm to calculate the final centroids to be used with scoring. The results won't be as accurate as the results of a model with the whole dataset; however, it should help solve the problem of huge datasets.
...
- [https://github.com/h2oai/h2o-3/blob/master/h2o-py/demos/constrained_kmeans_demo_cluto.ipynb](https://github.com/h2oai/h2o-3/blob/master/h2o-py/demos/constrained_kmeans_demo_cluto.ipynb)
- [https://github.com/h2oai/h2o-3/blob/master/h2o-py/demos/constrained_kmeans_demo_chicago.ipynb](https://github.com/h2oai/h2o-3/blob/master/h2o-py/demos/constrained_kmeans_demo_chicago.ipynb)
...
[Hastie, Trevor, Robert Tibshirani, and J Jerome H Friedman. The Elements of Statistical Learning. Second Edition. N.p., Springer New York, 2001.](http://statweb.stanford.edu/~tibs/ElemStatLearn/printings/ESLII_print10.pdf)
...
[Hartigan, John A. Clustering Algorithms. New York: John Wiley & Sons, Inc., N.p., 1975.](http://people.inf.elte.hu/fekete/algoritmusok_msc/klaszterezes/John%20A.%20Hartigan-Clustering%20Algorithms-John%20Wiley%20&%20Sons%20(1975).pdf)
