...
- The response column cannot be the same as the [fold_column](fold_column).
- For supervised learning, the response column cannot be the same as the [weights_column](weights_column), and the response column must exist in both the training frame and in the validation frame.
