...
Detailed information about each of these options is available in the [Solvers](../glm.mdx#solvers) section. The bullets below describe how the algorithm chooses the solver when `solver=AUTO`:
...
- IRLSM and COORDINATE_DESCENT share the same path (that is, they both compute the same gram matrix), they just solve it differently.
...
- Use GRADIENT_DESCENT_LH or GRADIENT_DESCENT_SQERR when `family=ordinal`. With GRADIENT_DESCENT_LH, the model parameters are adjusted by minimizing the loss function; with GRADIENT_DESCENT_SQERR, the model parameters are adjusted by minimizing the squared error.
