...
This option specifies the threshold value for probability. If this threshold is not met, then the `min_prob` value is used. This option can be used, for example, if one response category has few observations compared to the total. In this case, the conditional probability may be low. The `min_prob` and `eps_prob` values serve as a cutoff by setting a floor on the calculated probability.
