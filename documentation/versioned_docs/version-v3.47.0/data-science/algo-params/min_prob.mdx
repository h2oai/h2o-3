...
This option specifies the minimum probability to use for observations without enough data. This option can be used, for example, if one response category has few observations compared to the total. In this case, the conditional probability may be low. The `min_prob` and `eps_prob` values serve as a cutoff by setting a floor on the calculated probability.
