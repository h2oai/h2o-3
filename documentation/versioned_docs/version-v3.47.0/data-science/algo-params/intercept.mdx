...
The `intercept` command allows you to specify whether a single constant term should be included in the model. By including a constant term (that is, an "intercept" or "bias"), we ensure that the model is unbiased—that is, the mean of the residuals will be exactly zero.

The intercept term adjusts all predictions up or down by a constant amount; that is, it is the predicted value when all inputs are exactly 0. It can be excluded (forced to equal 0) by setting `intercept=FALSE`.
