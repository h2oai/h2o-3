...
For simplicity, we start with some single-node experiments quantifying the raw training throughput. Of course, there are many parameters that affect predictive accuracies and modeling performance, but for this first part, we only consider the speed of training.
...
First, we set the path to a scratch directory that will contain the downloaded datasets and the CSV files containing the results.
```{r}
workdir="/tmp"
```
...
Notes:
  * We first look at single-node training speed in terms of training samples per second.
  * We note that H2O Deep Learning uses online learning with stochastic gradient descent. There are *no mini-batches*, every training sample immediately affects the network weights.
  * In part 2, we look at the test set error as a function of total training time, a more meaningful met/Pric.
...
### Default parameters on various network topologies
As a first test, we vary the network topology, and we also fix a number of training samples, specified by the number of epochs. All other parameters are left at default values.
```{r}
EPOCHS=0.1
