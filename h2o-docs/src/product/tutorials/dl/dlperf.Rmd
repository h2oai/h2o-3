...
Observations:
  * We cannot reproduce the slow numbers (~10 samples/second) cited by the other benchmark study. Using the same parameters, we measure a training speed of 75 samples per second on a single i7 5820k consumer PC.
  * When removing the scoring overhead by not specifying `validation_frame` and reducing the `score_training_samples` to 100 (while keeping the `Tanh` activation and `adaptive_rate=T`), we obtain training speeds of 85 samples per second (notably with H2O's mini-batch size of 1).
