...
* We are unable to reproduce their findings (our numbers are 8x to 45x faster, on a single node)
...
The full logs of the run that created this blog post are available [for download here](../../../2015_02_deep-learning-performance/deeplearning-perf.log).
...
We use nightly build 1079 of h2o-dev, but you should be able to use any future nightly or stable version as well. The code below is copied from the instructions from the [h2o download page](http://h2o.ai/download).
...
Notes:
  ...
  * Hardware specs: [Intel i7 5820k (6 cores @4.4GHz)](http://rog.asus.com/realbench/show_comment.php?id=6764). Yes, it's an overclocked sub-$400 processor. If it dies, I'll have to get the 8-core version...
...
Observations:
  * **We cannot reproduce the slow numbers (~10 samples per second) cited by the other benchmark study.** Using the same parameters, we measure a training speed of **75 samples per second** on a single i7 5820k consumer PC.
  * When removing the scoring overhead by not specifying `validation_frame` and reducing the `score_training_samples` to 100 (while keeping the `Tanh` activation and `adaptive_rate=T`), we obtain training speeds of 84 samples per second (notably with H2O's mini-batch size of 1).
  * When further switching to `adaptive_rate=F` (and no momentum), we obtain training speeds of 138 samples per second (albeit at the cost of accuracy).
  * When further switching to the `Rectifier` activation function, speeds increase to **310 samples per second**. This is more than *4 times faster than the original parameters and results in half the test set error!* Note that we used the `max_w2` parameter, which helps to improve the numerical stability for deep `Rectifier` networks.
  * We observe training speeds of **457 samples per second while obtaining a lower test set error** than the original parameters, when using the `RectifierWithDropout` activation function (a good choice for MNIST, see world-record performance in Part 3 below). Hence, **H2O Deep Learning is faster on a single PC than the other method on 16 Xeon nodes** (400 samples per second with `Tanh` and AdaGrad), so further benchmark comparisons really have to take test set accuracy into account to be meaningful.
