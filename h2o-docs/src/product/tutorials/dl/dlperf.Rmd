...
Observations:
  ...
  * We observe training speeds of **395 samples per second while obtaining a lower test set error** than the original parameters, when using the `RectifierWithDropout` activation function (a good choice for MNIST, see world-record performance in Part 3 below). Hence, **H2O Deep Learning is as fast on a single PC as the other method on 16 Xeon nodes** (400 samples per second with `Tanh` and AdaGrad), so further benchmark comparisons really have to take test set accuracy into account to be meaningful.
...
args <- list(
  ...
  list(epochs=EPOCHS, hidden=c(200,200,200)),
  
  list(epochs=EPOCHS, hidden=c(200,200,200), 
       activation="RectifierWithDropout", input_dropout_ratio=0.2, l1=1e-5),
  
  list(epochs=EPOCHS, hidden=c(200,200,200,200), 
       activation="RectifierWithDropout", input_dropout_ratio=0.2, l1=1e-5),
