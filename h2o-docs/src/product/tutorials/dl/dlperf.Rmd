...
library('stringr')
```
Start a single-node instance of H2O using all available processor cores and reserve 16GB of memory (check your logs for `WARN: Pausing to swap to disk; more memory may help` using the Flow GUI at [localhost:54321](http://localhost:54321), `Admin` -> `View Log` -> `SELECT LOG FILE TYPE: warn`).
```{r}
...
Observations:
  * **We cannot reproduce the slow numbers (~10 samples per second) cited by the other benchmark study.** Using the same parameters, we measure a training speed of **78 samples per second** on a single i7 5820k consumer PC.
  ...
  * When further switching to the `Rectifier` activation function, speeds increase to over **240 samples per second**. This is more than *4 times faster than the original parameters and results in half the test set error!* Note that we used the `max_w2` parameter, which helps to improve the numerical stability for deep `Rectifier` networks.
