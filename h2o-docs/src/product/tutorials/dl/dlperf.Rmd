...
It is clear that the fastest option is to run with a manual learning rate without momentum (default: 0), as this reduces the memory and computational overhead to a minimum. Enabling the adaptive learning rate is roughly equivalent to turning on momentum training (as each model coefficient has its own momentum history), it is only slightly more computationally expensive. Also, different hyperparameters for adaptive learning rate can affect the training speed (at least for Rectifier activation functions, where backpropagation depends on the activation values).
...
Notes:
  *  Unfortunately, there was no attempt made at comparing test set errors in the benchmark study. Throughput alone is not a good measure of model performance, see part 2 below for test set error comparisons and part 3 below for our model with world-record test set accuracy.
  ...
  * H2O Deep Learning does online learning (or, mini-batch size of 1), while we assume that the other project used a [mini-batch size of 1000](http://www.comp.nus.edu.sg/~dbsystem/singa/examples/2015/01/20/deep-simple-mlp/), but this our guess.
...
Here's a plot summarizing all the models we've built so far. Default options and fewer parameters often end up in favorable spots on the lower left corner.
