...
* We are unable to reproduce their findings (our numbers are 8x to 40x faster, on a single node)
* The benchmark was only on raw training throughput and didn't consider model accuracy (at all)
* The parameters for H2O Deep Learning were poorly chosen (more below)
* We present a single-node model that performs as fast as their 16-node model (and leads to higher accuracy than with their settings)
