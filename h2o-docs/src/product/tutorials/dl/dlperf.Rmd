...
As many of you might know, distributed benchmarking has its own issues and complexity. I've burned many millions of core hours on the world's fastest supercomputers over the last decade, so I have a good sense of what it takes to get code to scale. Interconnect performance, load balancing and algorithmic choices greatly affect the scalability of large distributed systems. This benchmark script can seamlessly run on a large multi-node H2O cluster (where some parameters would benefit from some changes), and the scalability of H2O Deep Learning can be fully tuned from close-to-linear speedup with very little communication to totally communication-dominated regimes. The current implementation automatically attempts to balance computation and communication with dynamic auto-tuning during runtime. Due to the complexity involved, we refer to a later blog on distributed benchmarking of H2O Deep Learning, so please stay tuned for that.
...
We use nightly build 1072 of h2o-dev, but you should be able to use the latest nightly or latest stable version as well. The code below is copied from the instructions from the [h2o download page](http://h2o.ai/download).
...
We first run with the same parameters as the benchmark study, and then modify the parameters to improve the trainings speed successively.
```{r}
...
Observations:
  * We cannot reproduce the slow numbers (~10 samples/second) cited by the other benchmark study. Using the same parameters, we measure a training speed of 78 samples per second on a single i7 5820k consumer PC.
  * When removing the scoring overhead by not specifying `validation_frame` and reducing the `score_training_samples` to 100 (while keeping the `Tanh` activation and `adaptive_rate=T`), we obtain training speeds of 89 samples per second (notably with H2O's mini-batch size of 1).
  * When further switching to `adaptive_rate=F` (and no momentum), we obtain training speeds of more than 130 samples per second (albeit at the cost of accuracy).
  * When further switching to the `Rectifier` activation function, speeds increase to over 240 samples per second (and test set accuracy improves significantly too).
  * We observe training speeds of **395 samples per second with H2O Deep Learning on a single node**, when using the `RectifierWithDropout` activation function (a good choice for MNIST, see world-record performance in Part 3 below). Note that *H2O Deep Learning is as fast on a single node than what the other project achieved when training on 16 Xeon nodes* (400 samples per second with `Tanh` and AdaGrad), so further benchmark comparisons really have to take accuracy into account to be meaningful.
...
We are glad to see that default parameters lead to models that are among the best (in the lower left corner). 
