...
The `h2o-scala` module is marked as deprecated since version `3.30.0.5` in order to be completely removed in next major version `3.32.0.1`. This by no means has any impact on Scala support, as the previously-mentioned module has been intended for internal use only and does not represent H2O Scala API.
