...
H2O AnovaGLM is used to calculate Type III SS (sum of squares) which is used to investigate the contributions of individual predictors and their interactions to a model. Predictors or interactions with negligible contributions to the model will have high p-values while those with more contributions will have low p-values. Since AnovaGLM is mainly used to investigate the contribution of each predictor or interaction, scoring, MOJO, and cross-validation are not supported.
...
- **note**: This algorithm will support multiple categorical/numerical columns and other families as well; you just need to replace the SS with the residual deviance for other families.

SS (Sum of Squares)
'''''''''''''''''''

In Analysis of Variance (ANOVA), the partition of the response variable sum of squares in a linear model is described in "explained" and "unexplained" components. Consider a dataset generated by
...
To understand why a transformation is needed, look at the one-way ANOVA model with one categorical predictor with :math:`m` levels. Let:

- :math:`Y_{ij}` denote the :math:`i{\text{th}}` observation with predictor level :math:`j`;
- :math:`n_j` be the number of observations in the :math:`j{\text{th}}` group;
- :math:`n = {\sum^m_{j=1}}n_j` be the total number of observations;
- :math:`\mu_j = E(Y_{ij})` represent the population mean in group :math:`j`.

The one-way ANOVA model can be described by:

  .. math::
    Y_{ij} = \mu + \alpha_j + \epsilon_{ij} {\text{ }\text{ Equation 2}}

where

- :math:`\mu` represents the general level of the response variable in the population;
- :math:`\alpha_j` should represent the effect on the response variable when the predictor takes on value :math:`j`;
- :math:`\epsilon_{ij}` is an error variable that follows the usual linear-model assumptions.

If you take expectation on **Equation 2** over :math:`i`, you get
  
  .. math::
    \mu_j = \mu + \alpha_j.

Let :math:`m=3` and you have
...
Note that you have three equations but four unknowns meaning you have an under-determined and can have infinitely many solutions. 

Two-way ANOVA
'''''''''''''

With two categorical predictors, similar procedures apply. The model is now:

  .. math::
    Y_{ijk} = u + \alpha_j + \beta_k + \gamma_{jk} + \epsilon_{ijk}

where

- :math:`Y_{ijk}` is the :math:`i{\text{th}}` observation with first predictor at level :math:`j`, second predictor at level :math:`k`;
- :math:`u` is the general mean of :math:`Y`;
- :math:`\alpha_j` is the effect of predictor 1;
- :math:`\beta_k` is the effect of predictor 2;
- :math:`\gamma_{jk}` is the effect of the interaction between predictors 1 and 2;
- :math:`\epsilon_{ijk}` is the error term.

Using the same procedure as in a One-way ANOVA, first transform both the categorical predictors using the procedure described for a single predictor. Let the first predictor be R with 3 levels and the second predictor be C with 4 levels. The transformed first predictor will have two values :math:`R_0,R_1` and the second predictor will be formed by multiplying the two transformed categorical predictors as :math:`R_0C_0, R_0C_1,R_0C_2,R_1C_0,R_1C_1` and :math:`R_1C_2`. The GLM model for these two predictors and their interaction is

  .. math::
    \hat{u_i} = \beta_0 + \beta_1R_0 + \beta_2C_0 + \beta_3C_1 + \beta_4R_0C_0 + \beta_5R_0C_1 + \beta_6R_1C_0 + \beta_7R_1C_1.

The degree of freedom for this interaction term is degree of freedom of predictor R times the degree of freedom of predictor C.
...
The same procedure applies if there are more predictors. In general, to calculate the Type III SS, you build the model with all the predictors and all the predictor interactions and compare the full model to taking out either one predictor or one interaction. The calculation of the SS difference is then used to estimate how important the predictor that is left out is. To do this, F-tests are used. Finally, to answer the question that certain coefficients should be zero, you calculate the p-value from the F-tests just like the p-value calculation with a Gaussian distribution. If the p-value calculated is small, you reject the hypothesis that the set of parameters associated with a predictor should be set to zero. 
