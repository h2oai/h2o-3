...
**GBM/DRF/Isolation Forest**
  ...
  - ``enum_limited`` or ``EnumLimited``: Automatically reduce categorical levels to the most prevalent ones during Aggregator training and only keep the **T** (1024) most frequent levels, and then internally do one hot encoding in the case of XGBoost.
