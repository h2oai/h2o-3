...
To enable this feature, set the ``-ssl_config <path>`` parameter while starting an H2O node pointing to a configuration file (key=value format) containing the following values:

 - h2o_ssl_jks_internal (optional): A path (absolute or relative) to a key-store file used for internal SSL communication
 - h2o_ssl_jks_password (optional): Ahe password to the internal key-store
 - h2o_ssl_jts (optional): A path (absolute or relative) to a trust-store file used for internal SSL communication
 - h2o_ssl_jts_password (optional): The password to the internal trust-store
 - h2o_ssl_protocol (optional): Protocol name used during encrypted communication (supported by JVM). Defaults to TSLv1.2.
 - h2o_ssl_enabled_algorithms (optional) 
 - Comma separated list of enabled cipher algorithms (ones supported by JVM)

If the first four (jks_internal, jks_password, jts, jts_password) parameters are missing, Java defaults will be used for all SSL-related parameters. This is highly dependant on the Java version in use and in some cases might not work. Therefore, it is advised to set them.

The ``ssl_config <path>`` parameter has to be set for every node in the cluster. Every node needs to have access to both the Java keystore and Java truststore containing appropriate keys and certificates.

This feature should not be used together with ``useUDP`` flag, as at this time we do not support UDP encryption through DTLS or any other protocol that might result in unencrypted data transfers.
...
Basic keystore/truststore generation can be done using the keytool program, which ships with Java, documentation can be found `here <https://docs.oracle.com/javase/7/docs/technotes/tools/solaris/keytool.html>`__.

Each node should have a key pair generated, and all public keys should be imported into a single truststore, which should be distributed to all the nodes.

The simplest (and not recommended) way would be to call:

``keytool -genkeypair -keystore h2o-internal.jks -alias h2o-internal``

Distribute the h2o-internal.jks file to all the nodes, and set it as both the keystore and truststore in ssl.config
...
2. Extract the certificate on each node:
	...
	``keytool -export -keystore h2o-internal.jks -alias signFiles -file node#.cer``
...
3. Distribute all of the above certificates to each node and on each node create a truststore containing all of them (or put all certificates on one node, import to truststore and distribute that truststore to each node):
	...
	``keytool -importcert -file node#.cer -keystore truststore.jks -alias node#``

Hadoop
~~~~~~

When deploying H2O cluster on Hadoop the user has 2 options:

- The one described above, where the user has to generate and distribute all the key pairs and use the ``-ssl_config <path>`` parameter
- Use the ``-ssl_config`` parameter without specifying a path. In this case, the H2O driver will generate a new public/private key pair and distribute it to all the nodes automatically. Each cloud/deployment will have a unique key pair in this case.
...
1. Generate public/private keys and distributed them.

2. ssl.properties file content:
...
::
		...
		hadoop jar h2odriver.jar -nodes 1 -mapperXmx 6g -output hdfsOutputDirName -ssl_config
