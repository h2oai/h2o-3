...
Maxout & $f(\cdot) = \max(w_i x_i + b) $ & $f(\cdot) \in \mathbb{R}$\\  % [1ex] adds vertical space
...
\newpage
\waterExampleInR
\lstinputlisting[style=R]{deeplearning/deeplearning_extractmodelparams.R}

\waterExampleInPython
\lstinputlisting[style=python]{deeplearning/deeplearning_extractmodelparams.py}
...
\item \texttt{h2o.importFile(path, conn = h2o.getConnection())}: Imports a file into H2O.
