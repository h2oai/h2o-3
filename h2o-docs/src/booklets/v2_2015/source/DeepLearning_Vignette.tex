...
\item \texttt{localH2O = h20.init()}:  Connects to an instance of H2O

\item \texttt{h2o.shutdown()}: Shuts down h2o.

\item \texttt{h2o.importFile(server, path)}: Imports a file into h2o.

\item \texttt{h2o.deepLearning(x, training_frame,autoencoder, hidden, epochs)}: Creates a Deep Learning model.
