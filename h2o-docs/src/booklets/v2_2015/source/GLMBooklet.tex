...
Let $X_i\in\rm \Bbb I \!\Bbb R^{(p)}$, $y$ can belong to any of the $K$ classes. In logistic ordinal regression, we model the cumulative probability that $y$ belongs to class $j$, given $X_i$ as the logistic function:
...
\textbf{Note}: Be careful when using ordinal regression. H2O's GLM uses a likelihood function to adjust the system parameters. However, during prediction, we look at the log CDF odds. As a result, there is a small disconnect between the two. Please also consider using recommendation methods as well. 
