...
%so when write the distribution for the tweedie we can replcae $\mu$ by Ginverse of eta and we will have Xbeta in the expression. 
...
datasets (thousands of predictors and up), the L-BFGS solver scales better. %comment on using IRLSM with coordinate descent or with ADMM as in the IRLSM called algo?
...
When using  $\ell_1$ penalty with lambda search, you can specify the \texttt{max\_active\_predictors} parameter to stop
...
Generalized Linear Modeling with H2O\\
  by Tomas Nykodym, Ariel Rao, Amy Wang, \\
Tom Kraljevic, Jessica Lanford \& Nadine Hussami\ \\
\bigskip
  Published by H2O.ai, Inc. \\
2307 Leghorn St. \\
Mountain View, CA 94043\\
...
\subsubsection{Lambda Search}
Lambda search enables efficient and automatic search for optimal value of the \texttt{lambda} parameter. When lambda search is enabled, GLM will first fit a model with maximum regularization strength and then keep decreasing it until overfitting. The resulting model will be based on the best \texttt{lambda} value. When looking for sparse solution (\texttt{alpha} $>$ 0), lambda search can also be used to efficiently handle very wide datasets as it can filter out inactive predictors (noise) and only build models for a small subset of predictors. Common use of lambda search is to run on a dataset with many predictors but limit the number of active predictors to a relatively small value.  
...
datasets (thousands of predictors and up), the L-BFGS solver scales better. If there are less than $\sim 500$ predictors in the data, use the default option which runs IRLSM. 

For larger numbers of predictors, it is recommended to run IRLSM with lambda search and compare it to L-BFGS with just the $\ell_2$ penalty. The more advanced user may follow these general guidelines:
...
\begin{itemize}
\item For a dense solution and a dense dataset, use IRLSM if there are less than $\sim 500$ predictors in the data and L-BFGS otherwise. Set \texttt{alpha} to 0, this way only the $\ell_2$ regularization is included in the elastic net penalty term so no sparsity is induced in the model.
...
\item For a dense solution with a sparse dataset, use IRLSM if there are less than $\sim 2000$ predictors in the data and L-BFGS otherwise. Set \texttt{alpha} to 0.
...
\item For a sparse solution with a dense dataset, use IRLSM with lambda-search if you expect less than $\sim 500$ active predictors in the solution and L-BFGS otherwise. Set \texttt{alpha} to be greater than zero. This adds an $\ell_1$ penalty to the elastic net regularization which induces sparsity in the estimated coefficients. 
\item For a sparse solution with a sparse dataset, use IRLSM with lambda-search if you expect less than $\sim 5000$ active predictors in the solution and L-BFGS otherwise. Set \texttt{alpha} to be greater than zero. 
...
\end{itemize}
 ...
 The above recommendations are rough and the user may want to experiment with the options if the performance of the method seems slow. 
...
To summarize, the solver options for fitting a GLM in H2O are:
\begin{itemize}
...
\subsubsection{L-BFGS solver}
In each iteration, L-BFGS computes a gradient at the current vector of coefficients and then computes an updated vector of coefficients in an approximated Newton-method step. The cost of the coefficient update is k*N, where N is number of predictors and k is a constant, the cost of gradient computation is $M*N\over pn$ where M is number of observations in the dataset and $pn$ is the number of CPU cores in the cluster. Since k is a (small) constant, the runtime of L-BFGS is dominated by the gradient computation, which is fully parallelized and L-BFGS thus scales almost linearly. 

\subsection{Data corner cases}

\begin{itemize}
\item What if: the training data has an NA? All rows containing (any) missing value are omitted form the training set. 

\item What if: the test data has an NA? The prediction for that observation is NA.

\item What if: a predictor column is categorical, you are making predictions on test data, and the predictor is
a level that wasn't seen during training? The value is zero for all predictors associated with the particular categorical variable.
