...
\texttt{offset\_column} is an optional column name or index referring to a column in the training frame. This column specifies a prior known component to be included in the linear predictor during training. Offsets are per-row ``Äúbias values''Äù that are used during model training. 
...
\texttt{LogLoss} is the log loss.  $LogLoss = {‚àí1 \over N} \sum_i^N \sum_j^C y_{i}log(p_{i,j})$
