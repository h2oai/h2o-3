...
gaussian.fit = h2o.glm(x = c("AGE", "RACE", "PSA", "GLEASON"), y = "VOL", training_frame = prostate.hex, family = "gaussian")
\end{lstlisting}
...
binomial.fit = h2o.glm(x = c("AGE", "RACE", "PSA", "GLEASON"), y = "CAPSULE", training_frame = prostate.hex, family = "binomial")
\end{lstlisting}
...
poisson.fit = h2o.glm(x = c("District", "Group", "Age"), y = "Claims", training_frame = insurance.hex, family = "poisson")
\end{lstlisting}
...
\section{GLM on H2O}
H2O's GLM implementation presents a high-performance distributed algorithm, which scales linearly with the number of rows and works extremely well for datasets with a limited number of active predictors.
...
Every model must specify its predictors and response; it is the equivalent formula object in R. Predictors and response are specified by \texttt{training\_frame}, \texttt{x}, and \texttt{y} parameters, with an optional \texttt{offset} parameter.

\texttt{training\_frame} refers to a frame containing a training dataset. All predictors and the response (and offset, if there is one) must be part of this frame.

\texttt{x} contains the list of column names or column indices referring to vectors from the training frame; it can not contain periods.

\texttt{y} is a column name or index referring to a vector from the training frame.

\texttt{offset} is a column name or index referring to a vector from the training frame.
