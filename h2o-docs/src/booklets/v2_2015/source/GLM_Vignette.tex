...
\item For a dense solution with a sparse dataset, use IRLSM if there are less than $\sim 2000$ predictors and L-BFGS otherwise. Set \texttt{alpha} equal to 0.
...
\item For a sparse solution with a dense dataset, use IRLSM with lambda-search if there are less than $\sim 500$ predictors and L-BFGS otherwise. Set \texttt{alpha} to be strictly greater than zero. This adds an $\ell_1$ penalty to the elastic net regularization which induces sparsity in the estimated coefficients. 
...
\begin{itemize}
 \item \textbf{L-BFGS}
 \item \textbf{IRLSM}: IRLSM with the ADMM solver in the innermost loop
