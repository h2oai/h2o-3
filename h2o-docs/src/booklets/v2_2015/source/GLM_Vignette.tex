...
Tweedie distributions are a family of distributions which include gamma, normal, Poisson and their combination. It is especially useful for modeling positive continuous variables which have exact zeros. The variance of the tweedie distribution is proportional to the $p$-th power of the mean. 
...
$$  \max_{\beta,\beta_0} \ \sum_{i=1}^N \log(a(y_i,\phi)) + \left( \frac{1}{\phi}\left( \frac{x\mu^{1-p}}{1-p} - \kappa(\mu,p) \right)  \right) -\lambda  \left(  \alpha \| \beta \|_1 + \frac{1}{2}(1-\alpha)\| \beta \|_2^2  \right), $$
where $\kappa(\mu,p) = \mu^{2-p}/(2-p)$ for $p \neq 2$ and $\kappa(\mu,p) = \log(\mu)$ for $p=2$ and where the function $a(y_i,\phi)$ is evaluated using series expansion since it does not have an analytical solution. The link function in the GLM representation of the tweedie distribution is $g(\mu) = \mu^{1-p} = \eta = X\beta$. 
%
...
H2O's GLM offers two different approaches to handling wide datasets:   $\ell_1$ regularization and the L\_BFGS solver.
...
For performance reasons, we recommend not using any  $\ell_1$ penalty when using the L\_BFGS solver, since
