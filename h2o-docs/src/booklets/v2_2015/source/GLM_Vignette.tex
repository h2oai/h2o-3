...
H2O's GLM offers two different approaches to handling wide datasets: the IRLSM and the L-BFGS solvers.

Quick recommendations:

\begin{itemize}
\item For a sparse solution (i.e. one which only keeps some predictors in the final model), use the IRLSM solver with 
      \texttt{alpha} greater than 0. This adds an $\ell_1$ penalty to the elastic net regularization which induces sparsity in the estimated coefficients. Enable \texttt{lambda\_search} to get models for the full regularization path.
\item For a dense solution (i.e. one where all predictors have non-zero coefficients in the final model), use the L-BFGS solver with
...
The two solvers provided in H2O are the Iteratively Reweighted Least Squares Method (IRLSM) and the Limited-memory Broyden-Fletcher-Goldfarb-Shanno algorithm (L-BFGS).  IRLSM uses
...
IRLSM can be run with two algorithms to solve the innermost loop: the ADMM algorithm, or cyclical coordinate descent. The latter is used in glmnet and is able to handle large datasets well and deals efficiently with sparse features. It should improve the performance if the data contains categorical variables with a large number of levels, as it is implemented to deal with such variables in a parallelized way. Coordinate descent can be implemented in the naive way or with covariance updates as explained in the glmnet paper. The covariance updates version is faster when $p>N$. 

In summary, the options are:
\begin{itemize}
 \item L-BFGS
 \item IRLSM: IRLSM with the ADMM solver for the innermost loop
 \item \texttt{COORDINATE\char`_DESCENT}: IRLSM with the covariance updates version of cyclical coordinate descent for the innermost loop
 \item \texttt{COORDINATE\char`_DESCENT\char`_NAIVE}: IRLSM with the naive updates version of cyclical coordinate descent for the innermost loop
\end{itemize}
