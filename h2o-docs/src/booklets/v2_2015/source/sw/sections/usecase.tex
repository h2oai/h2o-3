...
Up until now, we have seen two classification task applications.  Now, we are going to build an application that looks to predict a real number as opposed to a 'label'€™.  In particular, we are going to play air traffic controller and see if we can predict an arrival delay - in minutes - leveraging H2O and the Spark SQL library.

\textbf{What is the task?}

As Chief Air Traffic Controller, your job is come up with a prediction engine that can be used to tell passengers whether an incoming flight will be delayed by X number of minutes.  To accomplish this task, we have an airlines dataset containing ${\sim}$44k flights since 1987 with features such as: Origin and Destination codes,  distance traveled, carrier, etc.  The key variable we are trying to predict is 'ArrDelay'€ (arrival delay) in minutes.
...
The flight data file is imported directly into H2O already as an H2O frame. The weather table, however, is first processed in Spark where we do some parsing of the data and data scrubbing.
...
\begin{itemize}
\item Convert the RDD into a DataFrame
\item Register the resulting DataFrame as '€˜Temp Table' 
