...
\textbf{Steps}:
\begin{enumerate}
\item In Databricks, click "Create Cluster" in the Clusters dashboard.
...
\item Select 0 on-demand workers. On demand workers is currently not supported with Sparkling Water.
...
\item Once the cluster has started, run the below command in a terminal session:
\begin{lstlisting}[style=Bash]
...
\item Create libraries for the python modules: request, tabulate, future and colorama
