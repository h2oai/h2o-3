...
H2O supports elastic net regularization, which is a combination of the $\ell_1$ and $\ell_2$ penalties parametrized by $\alpha$ and $\lambda$ arguments (similar to \citetalias{glmnet}).
\begin{itemize}
...
The corresponding deviance is equal to (CHECK CHANGED THAT FROM PREV EXP- triv derivation)
...
$$D = -2\sum_{i=1}^{N}{ \log\left({\frac{ y_i }{\hat{y}_i}}\right) - {y_i - \frac{\hat{y}_i}{\hat{y}_i} }}$$
