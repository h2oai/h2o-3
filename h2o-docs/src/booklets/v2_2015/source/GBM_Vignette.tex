...
\textsc{Viraj Parmar \hspace{10pt} Hank Roark \hspace{10pt} Arno Candel}
...
Michal Malohlava, Viraj Parmar, Hank Roark\\
\&\  Arno Candel\\
\bigskip
...
Stochastic GBM is a way to improve generalization of the method by sampling columns (per split) and rows (per-tree) during the model building process. The parameters to control the sampling ratios are \texttt{sample\_rate} (rows) and \texttt{col\_sample\_rate} (columns). Both parameters must range from 0 to 1.
