...
Note that values (much) larger than 1024 (the maximum number of levels supported in R) are supported, but might increase model training time.

Note that the value of \texttt{nbins\_cats} for categorical factors has a much stronger impact on generalization error than \texttt{nbins} for real- or integer-valued columns (where higher values just lead to more accurate numerical split points).
This is because a small value of \texttt{nbins\_cats} can add randomness in the split decisions for columns with many factors (since they are grouped together somewhat arbitrarily),
while large values can lead to perfect splits that can lead to overfitting.
