...
To specify a model that considers all factors individually, specify the number of factor levels using the \texttt{nbins\_cats} parameter. Values over 1024 (the maximum number of levels supported in R),
are supported, which increases the model generation time.
...
set of factors with a known optimal split. 

This optimal split is represented as a bitset in the embedded Java model. Therefore,
