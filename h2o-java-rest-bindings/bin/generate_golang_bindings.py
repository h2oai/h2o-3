...
parser = argparse.ArgumentParser(
        description='Attach to an H2O instance and call its REST API to generate the GoLang REST API bindings docs and write them to the filesystem.',
)
...
parser.add_argument('--dest', help='destination directory', default=(here + '/../src-gen/main/golang'))
...
def cons_golang_type(pojo_name, name, h2o_type, schema_name):
    ...
    if h2o_type.startswith('Map'):
        spl = h2o_type.find(',')
        key_type = h2o_type[4:spl]
        if key_type == "Object":
            key_type = "Polymorphic"
        val_type = h2o_type[spl + 1:-1]
        if val_type == "Object":
            val_type = "Polymorphic"
        golang_type = 'map[' + key_type + ']' + val_type
        return golang_type

    if simple_type == 'string':
        return brackets + simple_type
    if simple_type == 'byte':
        return brackets + simple_type
...
#       return 'String'
    ...
    if simple_type in type_dict:
        return brackets + type_dict[simple_type]
    if simple_type == 'enum':
        return brackets + schema_name
...
def generate_pojo(schema, pojo_name, model_builders_map):
    ...
    constructor = []
    constructor.append("func new{pojo_name}() *{pojo_name}".format(pojo_name=pojo_name) + '{')
    constructor.append("return &{pojo_name}".format(pojo_name=pojo_name) + "{")
    ...
    for field in schema['fields']:
        ...
        golang_type = cons_golang_type(pojo_name, name, type, schema_name)
        ...
        if golang_type == 'int32':
            value = str(value)
        elif golang_type == 'float32':
            if value == "Infinity":
                value = "Float.POSITIVE_INFINITY"
            else:
              value = str(value)
        elif golang_type == 'float64':
            if value == "Infinity":
              value = "Double.POSITIVE_INFINITY"
        elif golang_type == 'bool':
            value = str(value).lower()
        elif golang_type == 'string' and (value == '' or value == None):
            value = '""';
        elif golang_type == 'string':
            value = '"' + value + '"';
        elif value == None:
            value = "nil";
            ...
            if basetype == 'Iced':
                basetype = 'Polymorphic'

            value = str(value)
            values = value[1:len(value) - 1]

            value = '{' + "{values}".format(values=values) + '}'
        ...
        if is_model_builder and 'parameters' == field['name']:
            if 'ModelBuilderSchema' == pojo_name:
                ...
                pojo.append("    Parameters []ModelParameterSchemaV3")
            else:
...
def generate_enum(name, values):
    ...
    pojo.append("")
    pojo.append("type " + name + " int")
    pojo.append("")
    pojo.append("const (")
    first = True
    for value in values:
        if first:
            pojo.append("{name}{value} {name} = iota".format(value=value, name=name))
            first = False
        else:
            pojo.append("{name}{value}".format(value=value, name=name))
    pojo.append(")")
    return pojo
...
def generate_retrofit_proxies(endpoints_meta, all_schemas_map):
    ...
    for entity in endpoints_by_entity:
        ...
        pojo.append("import golang.util.Map;")
        pojo.append("")
        ...
        for meta in endpoints_by_entity[entity]:
            ...
            for parm in path_parm_names:
                ...
                # cons up the proper GoLang type:
                parm_type = cons_golang_type(entity, field['name'], field['type'], field['schema_name'])
...
print('creating the GoLang bindings in {}. . .'.format(args.dest))
...
for schema in all_schemas:
    ...
    save_full = args.dest + os.sep + 'water/bindings/pojos/' + pojo_name + '.go'
...
for entity, proxy in retrofitProxies.items():
    save_full = args.dest + os.sep + 'water/bindings/proxies/retrofit/' + entity + '.go'
    save_dir = os.path.dirname(save_full)
