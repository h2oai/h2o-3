...
public class BinaryDomainMapConstructorTest {
  ...
  public void create() {
    ...
    Map<Integer, CategoricalEncoder> dm = DefaultCategoricalEncoding.Binary.createCategoricalEncoders(mockModel, columnNameToIndex);
    assertEquals(new HashSet<>(Arrays.asList(1, 3)), dm.keySet());
    for (CategoricalEncoder ce : dm.values()) {
      assertTrue(ce instanceof BinaryEncoder);
    }
  }
}
