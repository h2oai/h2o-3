...
public class DeepwaterBackend implements BackendTrain {
  ...
  /**
   * DeepwaterLayer is the basic node used for backend implementations
   * Layer: array of units (neurons) sharing parameters (activation, etc.)
   */

  private static class DeepwaterLayer {
    public DeepwaterLayer(String name, String type, int[] shape, int width, String[] activation, double dropout_ratio, String type_spec) {
      this.name = name;
      this.type = type;
      this.width = width;
    ...
    int [] next_layer;        // Adjacency list, for linear topology it's just next layer
    // Variable parameters
