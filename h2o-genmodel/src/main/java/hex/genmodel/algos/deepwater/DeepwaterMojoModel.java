...
public class DeepwaterMojoModel extends MojoModel {
  ...
  public final double[] score0(double[] doubles, double offset, double[] preds) {
    ...
    if (_nums > 0) {
      ...
      GenModel.setInput(
          doubles, floats, _nums, _cats, _catOffsets, _normMul, _normSub, _useAllFactorLevels);
    } else {
    ...
    assert (_nclasses == predFloats.length)
        : "nclasses " + _nclasses + " predFloats.length " + predFloats.length;
    if (_nclasses > 1) {
  ...
  public static BackendTrain createDeepWaterBackend(String backend) throws InvalidBackendException {
    try {
      if (backend.equals("mxnet")) {
        ...
        throw new InvalidBackendException("backend not supported:"+backend);
      }
      return (BackendTrain) (Class.forName(backend).newInstance());
    } catch (Exception ignored) {
      ignored.printStackTrace();
      throw new InvalidBackendException("could not instantiate class:"+backend, ignored);
    }
  }
}
