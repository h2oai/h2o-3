...
class ScalaDoubles extends Doubles with ScalaFactory[java.lang.Double, Double, DataColumn[java.lang.Double]] {
  def newColumn(size: Long, f: Long => Double): DataColumn[java.lang.Double] =
    super.newColumn(size, ff1LD(f))

  def newColumnOpt(size: Long, f: Long => Option[Double]): DataColumn[java.lang.Double] =
    super.newColumn(size, ff1LDO(f))
...
object MoreColumns extends DataColumns {
  ...
  object Dates extends Dates with ScalaFactory[Date, Date, DataColumn[Date]] {
    def newColumn(size: Long, f: Long => Date): DataColumn[java.util.Date] =
      super.newColumn(size, ff1L(f))

    def newColumnOpt(size: Long, f: Long => Option[Date]): DataColumn[java.util.Date] =
      super.newColumn(size, ff1LO(f))

    override def conv(x: Date): Date = x
  }

  object Strings extends Strings with ScalaFactory[String, String, DataColumn[String]] {
    def newColumn(size: Long, f: Long => String): DataColumn[String] =
      super.newColumn(size, ff1LS(f))

    def newColumnOpt(size: Long, f: Long => Option[String]): DataColumn[String] =
      super.newColumn(size, ff1LO(f))
  ...
  class ScalaEnums(domain: Iterable[String]) extends Enums(domain.toArray) with ScalaFactory[java.lang.Integer, Integer, EnumColumn] {
    def newColumn(size: Long, f: Long => Integer): EnumColumn =
      super.newColumn(size, ff1LI(f))

    def newColumnOpt(size: Long, f: Long => Option[Integer]): EnumColumn =
      super.newColumn(size, ff1LO(f))
  ...
  class ScalaUnfoldable[X, Y]() extends F.Unfoldable[X, Y] {
    private var f: X => Iterable[Y] = _ // java serialization problem
