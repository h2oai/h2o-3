...
import water.udf.MoreColumns.Dates._
import water.udf.MoreColumns.Strings
...
class PUBDEV4159Test extends Test0 with BeforeAndAfter with BeforeAndAfterAll {
  ...
  test("produce dates") {
    ...
    val materialized = materialize(dates)
  ...
  test("delta years, delta months") {
    ...
    val d1: Column[Date] = newColumn(1200, (i: Long) => {
    ...
    val years: Column[lang.Double] = ScalaDoubles.materialize(column1)
    ...
    val months: Column[lang.Double] = ScalaDoubles.materialize(column2)
