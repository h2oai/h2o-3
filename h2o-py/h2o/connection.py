...
class H2OConnection(object):
    ...
    def _connect(self,size):
        ...
        while True:
            ...
            if retries > max_retries:
                raise EnvironmentError("Max retries exceeded. Could not establish link to the H2O cloud @ " + self._ip + ":" + str(self._port))
    ...
    @staticmethod
    def get(url_suffix=None, params=None, **kwargs):
        return __H2OCONN__._do_raw_rest(url_suffix, params, "GET", None, **kwargs)

    @staticmethod
    def post(url_suffix, params=None, file_upload_info=None, **kwargs):
        return __H2OCONN__._do_raw_rest(url_suffix, params, "POST", file_upload_info, **kwargs)

    @staticmethod
    def delete(url_suffix, params=None, **kwargs):
        return __H2OCONN__._do_raw_rest(url_suffix, params, "DELETE", None, **kwargs)

    @staticmethod
    def get_json(url_suffix, params=None, **kwargs):
        return __H2OCONN__._rest_json(url_suffix, params, "GET", None, **kwargs)

    @staticmethod
    def post_json(url_suffix, params=None, file_upload_info=None, **kwargs):
        return __H2OCONN__._rest_json(url_suffix, params, "POST", file_upload_info, **kwargs)

    def _rest_json(self, url_suffix, params, method, file_upload_info, **kwargs):
        raw_txt = self._do_raw_rest(url_suffix, params, method, file_upload_info, **kwargs)
        return H2OConnection._from_json(raw_txt, **kwargs)

    # Massage arguments into place, call _attempt_rest
    def _do_raw_rest(self, url_suffix=None, params=None, method=None, file_upload_info=None, **kwargs):
        if not params:
            params = {}
        ...
        http_result = self._attempt_rest(url, method, post_body, file_upload_info, params=params, **kwargs)
        ...
        if not http_result.ok:
            raise EnvironmentError("h2o-py got an unexpected HTTP status code:\n {} {} (method = {}; url = {})"
                                   .format(http_result.status_code,http_result.reason,method,res["url"]))
    ...
    def _attempt_rest(self, url, method, post_body, file_upload_info, params, **kwargs):
        try:
            if method == "GET":
                ...
                return requests.post(url, files=files, **kwargs)
            elif method == "POST":
                return requests.post(url, params=post_body, **kwargs)
            elif method == "DELETE":
    ...
    def _from_json(txt=None, **kwargs):
        # res = H2OConnection._process_matrices(txt.json(**kwargs))
        return H2OConnection._process_tables(txt.json(**kwargs))
