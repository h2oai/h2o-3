...
class H2OConnection(object):
    ...
    def __init__(self, ip, port, start_h2o, enable_assertions, license, nthreads, max_mem_size, min_mem_size, ice_root,
                 strict_version_check, proxy, https, insecure, username, password, cluster_name, max_mem_size_GB,
                 min_mem_size_GB, proxies, size):
        """
        ...
        for name, value in os.environ.items():
            if name.lower()[-6:] == '_proxy' and value:
                warnings.warn(
                    "Proxy environment variable `" + name + "` with value `" + value + "` found. This may interfere with your H2O Connection.")
        ...
        try:
            ...
            if not start_h2o: raise ValueError(
                "Cannot connect to H2O server. Please check that H2O is running at {}".format(
            ...
            if path_to_jar:
                ...
                cld = self._start_local_h2o_jar(max_mem_size, min_mem_size, enable_assertions, license, ice_root,
                                                jar_path, nthreads)
            else:
    ...
    def jar_paths():
        ...
        if sys_prefix1.startswith('/Library'):
            sys_prefix2 = '/System' + sys_prefix1
        elif sys_prefix1.startswith('/System'):
            sys_prefix2 = sys_prefix1.split('/System')[1]
    ...
    def _connect(self, size=1, max_retries=5, print_dots=False):
        ...
        while True:
            ...
            if retries > max_retries:
                raise EnvironmentError(
                    "Max retries exceeded. Could not establish link to the H2O cloud @ " + str(self._ip) + ":" + str(
                        self._port))
    ...
    def _shutdown(conn, prompt):
        ...
        if prompt:
            question = "Are you sure you want to shutdown the H2O instance running at {0}:{1} (Y/N)? ".format(conn.ip(),
                                                                                                              conn.port())
    ...
    @staticmethod
    def rest_version():
        return __H2OCONN__._rest_version

    @staticmethod
    def session_id():
        return __H2OCONN__._session_id

    @staticmethod
    def port():
        return __H2OCONN__._port

    @staticmethod
    def ip():
        return __H2OCONN__._ip

    @staticmethod
    def https():
        return __H2OCONN__._https

    @staticmethod
    def username():
        return __H2OCONN__._username

    @staticmethod
    def password():
        return __H2OCONN__._password

    @staticmethod
    def insecure():
        return __H2OCONN__._insecure
    ...
    def cluster_is_up(conn):
        ...
        rv = conn.current_connection()._attempt_rest(
            url=("https" if conn.https() else "http") + "://{0}:{1}/".format(conn.ip(), conn.port()), method="GET",
            post_body="", file_upload_info="")
        if rv.status_code == 401: warnings.warn(
            "401 Unauthorized Access. Did you forget to provide a username and password?")
            ...
            if isinstance(v, list):
                ...
                for l in v:
                    if isinstance(l, list):
                        x += '['
                        x += ','.join([str(e) if PY3 else str(e).encode(H2OConnection.__ENCODING__,
                                                                        errors=H2OConnection.__ENCODING_ERROR__) for e
                                       in l])
                        x += ']'
                    else:
                        x += str(l) if PY3 else str(l).encode(H2OConnection.__ENCODING__,
                                                              errors=H2OConnection.__ENCODING_ERROR__)
                    x += ','
                x = x[:-1]
                x += ']'
                ...
                x = str(v) if PY3 else str(v).encode(H2OConnection.__ENCODING__,
                                                     errors=H2OConnection.__ENCODING_ERROR__)
        ...
        if not http_result.ok:
            ...
            try:
                ...
                if 'messages' in result.keys():
                    detailed_error_msgs = '\n'.join(
                            [m['message'] for m in result['messages'] if m['message_type'] in ['ERRR']])
    ...
    def _attempt_rest(self, url, method, post_body, file_upload_info):
        ...
        try:
            if method == "GET":
                ...
                return requests.post(url, data=post_body, headers=headers, proxies=self._proxy, auth=auth,
                                     verify=verify)
            elif method == "DELETE":
    ...
    @staticmethod
    def rest_ctr():
        global _rest_ctr; return _rest_ctr
