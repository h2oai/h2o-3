...
class H2OClusteringModel(ModelBase):
    def size(self, train=False, valid=False, xval=False):
        ...
        m = {}
        for k, v in zip(list(tm.keys()), list(tm.values())): m[k] = None if v is None else [v[2] for v in
                                                                                            v._metric_json[
                                                                                                "centroid_stats"].cell_values]
    ...
    def tot_withinss(self, train=False, valid=False, xval=False):
        ...
        tm = ModelBase._get_metrics(self, train, valid, xval)
        m = {}
        for k, v in zip(list(tm.keys()), list(tm.values())): m[k] = None if v is None else v._metric_json[
            "tot_withinss"]
        return list(m.values())[0] if len(m) == 1 else m
    ...
    def withinss(self, train=False, valid=False, xval=False):
        ...
        tm = ModelBase._get_metrics(self, train, valid, xval)
        m = {}
        for k, v in zip(list(tm.keys()), list(tm.values())): m[k] = None if v is None else [z[-1] for z in
                                                                                            v._metric_json[
                                                                                                "centroid_stats"].cell_values]
        return list(m.values())[0] if len(m) == 1 else m
