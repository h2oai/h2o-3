...
class ModelBase(h2o_meta(Keyed)):
    ...
    def pr_plot(self, train=False, valid=False, xval=False, plot=True, server=False, 
        ...
        :param train: True to plot for training dataset
        :param valid: True to plot for validation dataset
        :param xval: True to plot for cross-validation dataset
        :param plot: True to plot precision recall, False to get a tuple or precision, recall
        :param server: 
        :param save_to_file: filename to save the plot to
        :param figsize: size of figure
        :return: precision/recall as a tuple or nothing if the plot is requested
        """
        title_text = "Precision Recall Plot for "
        data_frames = []
        metric_keys = []
        if train or (not(train) and not(valid) and not(xval)):
            data_frames.append(self.parms["training_frame"]["actual_value"]["name"])
            title_text = title_text + "training"
            metric_keys.append("train")
        if valid:
            data_frames.append(self.parms["validation_frame"]["actual_value"]["name"])
            title_text = title_text + ", validation"
            metric_keys.append("valid")
        if xval:
            data_frames.append(self.parms["training_frame"]["actual_value"]["name"])
            title_text = title_text + ", cross-validation"
            metric_keys.append("xval")
            
        precisions, recalls = self.__extract_precision_recall(ModelBase._get_metrics(self, train, valid, xval), 
                                                              metric_keys, data_frames)
        if not plot:
            return (precisions, recalls)
        
        self.__generate_pr_plot(precisions, recalls, plot, server, save_to_file, figsize, 
                                title_text, metric_keys)

    def __extract_precision_recall(self, tm, metric_keys, data_frames):
        recalls = []
        precisions = []
        count = 0
        for key in metric_keys:
            ...
            recall = v.tprs
            fprs = v.fprs
            # calculate number of response in positive class and negative class
            data = h2o.get_frame(data_frames[count])
    ...
    def __generate_pr_plot(self, precision, recall, plot, server, save_to_file, figsize, title_text, metric_keys):
        plt = _get_matplotlib_pyplot(server)
        if not plt:
            raise H2OValueError("Needed toolbox matplotlib is not available.  Please install it.")
        np = _get_numpy("pr_plots")
        if np is None:
            raise H2OValueError("Needed toolbox numpy is not available.  Please install it.")
        fig, ax = plt.subplots(figsize=figsize)
        ax.plot(np.array(recall).T, np.array(precision).T)
        ax.legend(metric_keys)
