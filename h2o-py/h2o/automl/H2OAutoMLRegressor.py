...
class H2OAutoMLRegressor(BaseEstimator, RegressorMixin): #?
	def __init__(self, 
    ...
    stopping_tolerance = None,   # dont know how to set default value?
    stopping_rounds = 3, 
    sort_metric = 'AUTO', 
    seed = None, 
    project_name = None, 
	...
	def predict(self, X):  
		...
		"""
		X = self.build_matrix(X)  
		# preds = self.model.predict(X).as_data_frame().as_matrix()[:,2] # unsure??
