...
def binop_minus(ip,port):
    ...
    res = 2 - iris[1]
    assert abs(sum([res[i] for i in range(rows)]) - -82.6) < 1e-1, "expected same values"
    ...
    res = iris[0] - iris[1]
    assert abs(sum([res[i] for i in range(rows)]) - 127.3) < 1e-1, "expected same values"

    res = iris[2] - iris[1]
    assert abs(sum([res[i] for i in range(rows)]) - -75.7) < 1e-1, "expected same values"

    res = iris[0] - iris[1] * iris[2] - iris[3]
    assert abs(sum([res[i] for i in range(rows)]) - -122.15) < 1e-2, "expected same values"
    ...
    # LHS: H2OVec, RHS: scaler
    res = iris[0] - 2
    assert abs(sum([res[i] for i in range(rows)]) - 209.9) < 1e-2, "expected different column sum"
