...
def binop_plus(ip,port):
    ...
    res = iris[0] + iris[1]
    res.show()
    assert abs(sum([res[i].eager() for i in range(rows)]) - 1334.6) < 1e-7, "expected different sum"

    res = iris[0] + iris[1] + iris[2] + iris[3]
    res.show()
    assert abs(sum([res[i].eager() for i in range(rows)]) - 2078.2) < 1e-7, "expected different sum"
