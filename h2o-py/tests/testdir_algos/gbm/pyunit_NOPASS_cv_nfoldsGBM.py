...
def cv_nfoldsGBM(ip,port):
  ...
  prostate_gbm = h2o.gbm.cv(y=prostate[1], x=prostate[2:9], nfolds = 5)
  prostate_gbm.show()
  
  # Can't specify both nfolds >= 2 and validation data at once
  try:
    h2o.gbm.cv(y=prostate[1], x=prostate[2:9], nfolds=5, validation_y=prostate[1], validation_x=prostate[2:9])
