...
def bernoulliGBM(ip,port):
  ...
  prostate_train = h2o.import_frame(path=h2o_home_dir + "smalldata/logreg/prostate_train.csv")
  ...
  trainData = np.loadtxt(h2o_home_dir + "smalldata/logreg/prostate_train.csv", delimiter=',', skiprows=1)
  ...
  prostate_test = h2o.import_frame(path=h2o_home_dir + "smalldata/logreg/prostate_test.csv")

  #Log.info("Converting CAPSULE and RACE columns to factors...\n")
  prostate_test["CAPSULE"] = prostate_test["CAPSULE"].asfactor()

  # Import prostate_test.csv as numpy array for scikit comparison
  testData = np.loadtxt(h2o_home_dir + "smalldata/logreg/prostate_test.csv", delimiter=',', skiprows=1)
