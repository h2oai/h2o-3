...
def smallcat_gbm():
  ...
  trainData = np.loadtxt(pyunit_utils.locate("smalldata/gbm_test/alphabet_cattest.csv"), delimiter=',', skiprows=1, converters={0:lambda s: ord((s.decode() if isinstance(s, bytes) else s).split("\"")[1])})
