...
def fair_cut_forest_save_and_load():
    ...
    fcf_model = H2OFairCutForestEstimator(ntrees=7, seed=12, sample_size=5, k_planes=1)
    ...
    model_path = h2o.save_model(fcf_model, path=path, force=True)
    ...
    assert isinstance(reloaded, 
                      H2OFairCutForestEstimator), \
        "Expected and H2OFairCutForestEstimator, but got {0}"\
        .format(reloaded)
...
if __name__ == "__main__":
    pyunit_utils.standalone_test(fair_cut_forest_save_and_load)
else:
    fair_cut_forest_save_and_load()
