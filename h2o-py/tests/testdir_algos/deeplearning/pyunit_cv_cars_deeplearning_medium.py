...
def cv_cars_dl():
  ...
  dl = H2ODeepLearningEstimator(nfolds=random.randint(3,10),fold_assignment="Modulo",epochs=10)
  ...
  dl1 = H2ODeepLearningEstimator(nfolds=nfolds,fold_assignment="Random",epochs=10)
  dl1.train(x=predictors,y=response_col,training_frame=cars)
  dl2 = H2ODeepLearningEstimator(nfolds=nfolds,fold_assignment="Random",epochs=10)
  try:
  ...
  dl = H2ODeepLearningEstimator(keep_cross_validation_predictions=True,epochs=10)
  ...
  dl = H2ODeepLearningEstimator(nfolds=cars.nrow, fold_assignment="Modulo",epochs=10)
  dl.train(x=predictors,y=response_col,training_frame=cars)

  # 2. nfolds = 0
  dl = H2ODeepLearningEstimator(nfolds=0,epochs=10)
  dl.train(x=predictors,y=response_col,training_frame=cars)

  # 3. cross-validation and regular validation attempted
  dl = H2ODeepLearningEstimator(nfolds=random.randint(3,10),epochs=10)
  ...
  # 1. nfolds == 1 or < 0
  try:
    dl = H2ODeepLearningEstimator(nfolds=random.sample([-1,1], 1)[0],epochs=10)
    dl.train(x=predictors, y=response_col, training_frame=cars)
  ...
  try:
    dl = H2ODeepLearningEstimator(nfolds=cars.nrow+1,fold_assignment="Modulo",epochs=10)
    dl.train(x=predictors, y=response_col, training_frame=cars)
  ...
  try:
    ...
    dl.train(x=predictors, y=response_col, fold_column="fold_assignments", training_frame=cars, epochs=10)
