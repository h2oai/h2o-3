...
def link_functions_gaussian(ip,port):
	...
	sm_model = sm.GLM(endog=sm_data_response, exog=sm_data_features, family=sm.families.Gaussian(sm.genmod.families.links.identity)).fit()

	print("Compare model deviances for link function identity")
	h2o_deviance_log = h2o_model._model_json['output']['residual_deviance'] / h2o_model._model_json['output']['null_deviance']
	sm_deviance_log = sm_model.deviance / sm_model.null_deviance
	print(h2o_deviance_log)
	print(sm_deviance_log)
