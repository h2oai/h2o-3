...
def link_functions_poisson(ip,port):
	...
	sm_model_log = sm.GLM(endog=sm_data_response, exog=sm_data_features, family=sm.families.Poisson(sm.genmod.families.links.log)).fit()
	...
	sm_model_id = sm.GLM(endog=sm_data_response, exog=sm_data_features, family=sm.families.Poisson(sm.genmod.families.links.identity)).fit()
