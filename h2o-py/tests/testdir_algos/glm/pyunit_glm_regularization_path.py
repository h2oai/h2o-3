...
def reg_path_glm():
    ...
    m = glm(family='binomial',lambda_search=True,solver='COORDINATE_DESCENT', seed=1234)
    ...
    for l in range(0,len(r['lambdas'])): # starting to fail at 12
        m = glm(family='binomial',lambda_search=False,Lambda=r['lambdas'][l],solver='COORDINATE_DESCENT', seed=1234)
        m.train(training_frame=d,x=[2,3,4,5,6,7,8],y=1)
        cs = r['coefficients'][l]
        ...
        print("For lambda index {0}".format(l))
        print(diff)
        print(diff2)
        assert diff < 1e-2
        assert diff2 < 1e-2
