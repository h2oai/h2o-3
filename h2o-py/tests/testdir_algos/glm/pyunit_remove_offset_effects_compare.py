...
def glm_remove_offset_effects():
    ...
    glm_model_remove_offset_effects.train(x=list(range(2,8)),y="economy_20mpg", training_frame=cars, offset_column=offset_col)
    ...
    predictions_train_remove_offset_manualy = glm_model.predict(cars).as_data_frame()
    print(predictions_train_remove_offset_manualy)

    # metrics with offset effects removed
    perf_remove_offset_manualy = glm_model.model_performance(cars)
    print(perf_remove_offset_manualy)
    
    assert perf.mse() != perf_remove_offset_manualy.mse()
    assert perf_remove_offset_manualy.mse() == perf_remove_offset_effects.mse()

    # check predictions are different
    for i in range(predictions_train.shape[0]):
        pyunit_utils.assert_not_equal(predictions_train.iloc[i, 1], predictions_train_remove_offset_effects.iloc[i, 1], f"Predictions at position {i} should differ but they don't!")
        pyunit_utils.assert_equals(predictions_train_remove_offset_manualy.iloc[i, 1], predictions_train_remove_offset_effects.iloc[i, 1], f"Predictions at position {i} should equal but they don't!")
