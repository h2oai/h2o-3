...
def xgboost_remove_offset_baseline():
    ...
    for family, y, link, col in configs:
        ...
        assert err < 1e-4, "{0}: link(predWith)-link(predZero) must equal offset, err={1}".format(family, err)


if __name__ == "__main__":
    pyunit_utils.standalone_test(xgboost_remove_offset_baseline)
else:
    xgboost_remove_offset_baseline()
