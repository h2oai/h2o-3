...
constr_prefix = fullname(H2OTargetEncoderEstimator.__init__)[:-8]
...
def test_deprecated_k_param_is_alias_for_inflection_point():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert "``k`` param of ``{}__init__`` is deprecated".format(constr_prefix) in str(w[0].message)
...
def test_deprecated_f_param_is_alias_for_smoothing():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert len(w) == 1
        assert issubclass(w[0].category, H2ODeprecationWarning)
        assert "``f`` param of ``{}__init__`` is deprecated".format(constr_prefix) in str(w[0].message)
...
def test_deprecated_noise_level_param_is_alias_for_noise():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert len(w) == 1
        assert issubclass(w[0].category, H2ODeprecationWarning)
        assert "``noise_level`` param of ``{}__init__`` is deprecated".format(constr_prefix) in str(w[0].message)
