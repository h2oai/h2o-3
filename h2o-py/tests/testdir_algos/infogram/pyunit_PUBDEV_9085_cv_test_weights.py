...
def test_cv_warning_messages():
    """
    In this test, I will enable cv, add weights to training dataset and the validation
    dataset.  There should be no warning on weight columns generated
    """
    ...
    weight = pyunit_utils.random_dataset_real_only(fr.nrow, 1, misFrac=0)
    weight = weight.abs()
    ...
    pyunit_utils.checkLogWeightWarning("weight_column", wantWarnMessage=False)
