...
# In this test, we check to make sure that a grid search on a GAM functions correctly
# when the hyper_parameter dictionary contains an invalid value.
# The test searches over 2 parameters, alpha and lambda.
# Because lambda contains an invalid value of -1, only 2 models should be created.
...
class test_gam_gridsearch_error:

    h2o_data = []
    myX = []
    myY = []
    hyper_parameters = {'alpha': [0.1, 0.9], 'lambda': [-1, 0.01]}
    manual_gam_models = []
    h2o_model = []
    num_grid_models = 0
    num_expected_models = len(hyper_parameters['alpha']) * ((len(hyper_parameters['lambda'])) - 1)
    ...
    def setup_data(self):
        ...
        for alpha_param in self.hyper_parameters['alpha']:
            self.manual_gam_models.append(H2OGeneralizedAdditiveEstimator(family = "gaussian", gam_columns=["C11", "C12", "C13"],
                                                                          keep_gam_cols = True, scale = [1,1,1], num_knots = [5, 6, 7],
                                                                          alpha = alpha_param, lambda_ = [0.01], bs=[0,2,0]))
...
def test_gridsearch_error():
    test_gam_grid = test_gam_gridsearch_error()
    test_gam_grid.train_models()
    test_gam_grid.match_models()

if __name__ == "__main__":
    pyunit_utils.standalone_test(test_gridsearch_error)
else:
    test_gridsearch_error()
