...
class test_gam_gridsearch_specific:
                ...
                for scale_param in self.hyper_parameters['scale']:
                    for num_knots_param in self.hyper_parameters['num_knots']:
                        self.manual_gam_models.append(
                            H2OGeneralizedAdditiveEstimator(family = "gaussian", gam_columns=["C11", "C12", "C13"],
                                                            keep_gam_cols = True, scale = scale_param, bs=[2,2,0],
                                                            num_knots = num_knots_param, alpha = alpha_param, 
    ...
    def train_models(self):
        self.h2o_model = H2OGridSearch(H2OGeneralizedAdditiveEstimator(
            ...
            keep_gam_cols = True, bs=[2,2,0]), self.hyper_parameters)
