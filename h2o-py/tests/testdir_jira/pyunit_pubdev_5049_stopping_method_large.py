...
def testStoppingMethod(model_index, training_data, x_indices, y_index, validation_data, seed):
    ...
    models_sm_xval = ["H2ODeepLearningEstimator(distribution='gaussian', seed=seed, hidden=[3], stopping_method='xval', stopping_rounds=3, stopping_tolerance=0.01, nfolds=3)"]
    models_sm_valid = ["H2ODeepLearningEstimator(distribution='gaussian', seed=seed, hidden=[3], stopping_method='valid', stopping_rounds=3, stopping_tolerance=0.01, nfolds=3)"]
    models_sm_train = ["H2ODeepLearningEstimator(distribution='gaussian', seed=seed, hidden=[3], stopping_method='train', stopping_rounds=3, stopping_tolerance=0.01, nfolds=3)"]
    ...
    model.train(x=x_indices, y=y_index, training_frame=training_data, validation_frame=validation_data)
    pyunit_utils.assert_H2OTwoDimTable_equal(model_auto._model_json["output"]["scoring_history"], model._model_json["output"]["scoring_history"], col_header_list, tolerance=1e-6, check_sign=True, check_all=True,
  #                              num_per_dim=10)
