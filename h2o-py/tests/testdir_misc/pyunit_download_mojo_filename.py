...
def download_mojo_filename():
    ...
    mojo_path = model.download_mojo(tmpdir)
    ...
    mojo_path = model.download_mojo(tmpdir, filename="gbm_prostate.zip")
    assert "gbm_prostate.zip" in mojo_path
    mojo_model = h2o.import_mojo(mojo_path)
    assert isinstance(mojo_model, H2OGenericEstimator)

    # Custom filename with default path
    mojo_path = model.download_mojo(filename="gbm_prostate2.zip")
