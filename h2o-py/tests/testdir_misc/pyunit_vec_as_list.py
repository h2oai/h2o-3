...
def vec_as_list():
    ...
    assert abs(float(res2[0][4]) - -2.6) < 1e-10 and abs(float(res2[0][18]) - -3.1) < 1e-10 and \
           abs(float(res2[0][25]) - -2.8) < 1e-10, "incorrect values"

    res3 = h2o.as_list(res[1], use_pandas=False)
    assert abs(float(res3[0][4]) - -1.1) < 1e-10 and abs(float(res3[0][6]) - -1.9) < 1e-10 and \
           abs(float(res3[0][10]) - -1.1) < 1e-10, "incorrect values"
