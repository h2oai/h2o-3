...
public class HGLM extends ModelBuilder<HGLMModel, HGLMModel.HGLMParameters, HGLMModel.HGLMModelOutput> {
  ...
  static class ScoringHistory {
    ...
    public ArrayList<Double> getTauEVar() { return _tauEVar; }
    ...
    public TwoDimTable to2dTable() {
      String[] cnames = new String[]{"timestamp", "iterations", "loglikelihood", "noise_variance"};
  ...
  private class HGLMDriver extends Driver {
    ...
    private TwoDimTable generateSummary(HGLMModel.HGLMModelOutput modelOutput) {
      String[] names = new String[]{"iteration", "loglikelihood", "noise_variance"};
    ...
    void fitEM(HGLMModel model, Job job, ScoringHistory scTrain, ScoringHistory scValid) {
      ...
      if (_parms._max_iterations > 0) {
        ...
        double tauEVar2;  // calculate tauEVar using equation 10 of the doc.
        ...
        while (true) {
          ...
          cjInv = generateCJInverse(engineTask._ArjTArj, tauEVar, tMatInv); // for each level 2 value
    ...
    public boolean progress(double[] beta, double[][] ubeta, double[][] tmat, double tauEVar, ScoringHistory scTrain,
                            ScoringHistory scValid, HGLMModel model, HGLMTask.ResidualLLHTask rLlh2) {
      _state._iter++;
