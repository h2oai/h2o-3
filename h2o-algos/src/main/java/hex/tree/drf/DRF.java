    ...
    private void growTrees(DTree[] ktrees, int[] leafs, Random rand) {
      ...
      for (int k = 0; k < _nclass; k++) {
        if (_model._output._distribution[k] != 0) { // Ignore missing classes
          ...
          new UndecidedNode(ktrees[k], -1, DHistogram.initialHist(_train, _ncols, adj_nbins, hcs[k][0], rseed, _parms, getGlobalQuantilesKeys(), null,true), null, null); // The "root" node
        }
      }
