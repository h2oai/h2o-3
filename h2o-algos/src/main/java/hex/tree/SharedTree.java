...
public abstract class SharedTree<M extends SharedTreeModel<M,P,O>, P extends SharedTreeModel.SharedTreeParameters, O extends SharedTreeModel.SharedTreeOutput> extends ModelBuilder<M,P,O> {
  ...
  protected DHistogram[][][] buildLayer(final Frame fr, final int nbins, int nbins_cats, final DTree ktrees[], final int leafs[], final DHistogram hcs[][][], boolean build_tree_one_node) {
    ...
    for( int k=0; k<_nclass; k++ ) {
      ...
      Frame fr2 = new Frame(Arrays.copyOf(fr.names(),_ncols+1), Arrays.copyOf(vecs,_ncols+1)); //predictors and actual response
      fr2.add(fr.name(idx_tree(k)),vecs[idx_tree(k)]); //tree predictions
      fr2.add(fr.name(idx_work(k)),vecs[idx_work(k)]); //target value to fit (copy of actual response for DRF, residual for GBM)
      fr2.add(fr.name(idx_nids(k)),vecs[idx_nids(k)]); //node indices
      if (idx_weight() >= 0)
        fr2.add(fr.names()[idx_weight()],vecs[idx_weight()]); //observation weights
  ...
  private static class ScoreBuildOneTree extends H2OCountedCompleter {
    ...
    boolean _did_split;
    ScoreBuildOneTree(SharedTree st, int k, int nbins, int nbins_cats, DTree tree, int leafs[], DHistogram hcs[][][], Frame fr2, boolean build_tree_one_node, float[] improvPerVar, Distribution.Family family) {
      _st   = st;
      _k    = k;
      _nbins= nbins;
