...
//              Log.info("Quantiles for column " + i + ": " + Arrays.toString(quantiles[i]));
    ...
    protected final void scoreAndBuildTrees(boolean oob) {
      for( int tid=0; tid< _ntrees; tid++) {
        ...
        if (scored && ScoreKeeper.stopEarly(_model._output.scoreKeepers(_parms._stopping_method, _cv_enabled, _validation_set_present),
                _parms._stopping_rounds, _nclass > 1, _parms._stopping_metric, _parms._stopping_tolerance,
                "model's last", true)) {
