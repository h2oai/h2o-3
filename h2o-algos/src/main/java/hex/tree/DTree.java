...
public class DTree extends Iced {
  ...
  public static class Split extends Iced {
    ...
    final float _se;            // Squared error without a split
    final float _se0, _se1;     // Squared error of each subsplit
    final long    _n0,  _n1;    // Rows in each final split
    final double  _p0,  _p1;    // Predicted value for each split

    public Split( int col, int bin, IcedBitSet bs, byte equal, float se, float se0, float se1, long n0, long n1, double p0, double p1 ) {
    ...
    public final float se() { return _se0+_se1; }
