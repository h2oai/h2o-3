...
/**
 * Fair cut forest implementation for anomaly detection. Algorithm comes from https://arxiv.org/pdf/2110.13402.pdf paper.
 *
 * @author Maroš Kramár
 */
public class FairCutForest extends ModelBuilder<FairCutForestModel,
        FairCutForestModel.FairCutForestParameters,
        FairCutForestModel.FairCutForestOutput> {

    transient private static final Logger LOG = Logger.getLogger(FairCutForest.class);
    public static final int MAX_NTREES = 100_000;
    public static final int MAX_SAMPLE_SIZE = 100_000;
    public static final int MAX_K_PLANES = 100;

    private FairCutForestModel _model;
    transient Random _rand;
    transient IsolationTreeStats isolationTreeStats;

    // Called from an http request
    public FairCutForest(FairCutForestModel.FairCutForestParameters parms) {
        super(parms);
        init(false);
    }

    public FairCutForest(FairCutForestModel.FairCutForestParameters parms, Key<FairCutForestModel> key) {
        super(parms, key);
        init(false);
    }

    public FairCutForest(FairCutForestModel.FairCutForestParameters parms, Job job) {
        super(parms, job);
        init(false);
    }

    public FairCutForest(boolean startup_once) {
        super(new FairCutForestModel.FairCutForestParameters(), startup_once);
    }
    ...
    protected void checkMemoryFootPrint_impl() {
        ...
        double sizeOfInnerNode = 3 * _train.numCols() * Double.BYTES + Double.BYTES;
        ...
        if (estimatedComputingMemory > H2O.SELF._heartbeat.get_free_mem() || estimatedComputingMemory < 0 /* long overflow **/) {
            String msg = "Fair Cut Forest computation won't fit in the driver node's memory ("
    ...
    public void init(boolean expensive) {
        ...
        if (_parms.train() != null) {
            ...
            if(_parms._k_planes < 1 || _parms._k_planes > MAX_K_PLANES)
                error("k_planes", "Parameter k_planes must be in interval [1, "
                        + MAX_K_PLANES + "] but it is " + _parms._k_planes);
        }
    ...
    protected Driver trainModelImpl() {
        return new FairCutForestDriver();
    }
    ...
    private class FairCutForestDriver extends Driver {
        ...
        public void computeImpl() {
            ...
            try {
                init(true);
                if(error_count() > 0) {
                    throw H2OModelBuilderIllegalArgumentException.makeFromBuilder(FairCutForest.this);
                }
                _rand = RandomUtils.getRNG(_parms._seed);
                isolationTreeStats = new IsolationTreeStats();
                _model = new FairCutForestModel(dest(), _parms,
                        new FairCutForestModel.FairCutForestOutput(FairCutForest.this));
        ...
        private void buildIsolationTreeEnsemble() {
            ...
            IsolationTree isolationTree = new IsolationTree(heightLimit, _parms._extension_level, _parms._k_planes);
    ...
    public TwoDimTable createModelSummaryTable() {
        ...
        colHeaders.add("Number of Hyperplanes"); colTypes.add("int"); colFormat.add("%d");
        ...
        table.set(row, col++, _parms._k_planes);
