...
import static hex.genmodel.algos.isoforfaircut.FairCutForestMojoModel.*;
...
public class CompressedIsolationTree extends Keyed<CompressedIsolationTree> {
    ...
    /**
     * Implementation of Algorithm 3 (treeScore) from paper.
     *
    ...
    public double computePathLength(double[] row) {
        ...
        while (!isLeaf(node)) {
            ...
            for (int i = 0; i < row.length; i++) {
                z += ((row[i] - compressedNode._means[i]) / compressedNode._stds[i]) * compressedNode.getNormalVector()[i];
            }
            
            if (z < compressedNode.getIntercept()) {
     ...
     *
     * sizeOfInternalArrays -> size of normal vector, means, stds and intercept
    ...
    public byte[] toBytes() {
        ...
        ab.put4(compressedNode(_nodes[0]).getNormalVector().length); // size of the internal arrays
