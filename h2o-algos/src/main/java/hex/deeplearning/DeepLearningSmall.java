  ...
  public DataInfo makeDataInfo(Frame train, Frame valid, DeepLearningParameters parms, int nClasses) {
    ...
    DataInfo dinfo = new DataInfo(
        ...
        response_transform, //transform response for regression with identity link
            parms._missing_values_handling == MissingValuesHandling.Skip, //whether to skip missing
    ...
    if (ymt.wsum() == 0 && parms._missing_values_handling == MissingValuesHandling.Skip)
...
//    assert(makeDataInfo(_train, _valid, _parms).fullN() == p);
  ...
  @Override public void cv_computeAndSetOptimalParameters(ModelBuilder[] cvModelBuilders) {
    ...
    for( ModelBuilder cvmb : cvModelBuilders )
      sum += ((DLModel)DKV.getGet(cvmb.dest())).last_scored().epoch_counter;
  ...
  public class DeepLearningDriver extends Driver {
    ...
    public final void buildModel() {
      ...
      if (_parms._checkpoint == null) {
        ...
        final DLModel previous = DKV.getGet(_parms._checkpoint);
     ...
     */
    public final DLModel trainModel(DLModel model) {
...
//        if (!mp._quiet_mode) Log.info("Initial model:\n" + model.model_info());
        ...
        for(;;) {
          ...
          model.set_model_info(mp._epochs == 0 ? model.model_info() : H2O.CLOUD.size() > 1 && mp._replicate_training_data ? (mp._single_node_mode ?
                  new DeepLearningTask2(_job._key, train, model.model_info(), syncFraction, model.iterations).doAll(Key.make(H2O.SELF)).model_info() : //replicated data + single node mode
                  new DeepLearningTask2(_job._key, train, model.model_info(), syncFraction, model.iterations).doAllNodes(             ).model_info()): //replicated data + multi-node mode
                  new DeepLearningTask (_job._key,        model.model_info(), syncFraction, model.iterations).doAll     (    train    ).model_info()); //distributed data (always in multi-node mode)
        ...
        if (!stop_requested() && _parms._overwrite_with_best_model && model.actual_best_model_key != null && _parms._nfolds == 0) {
          DLModel best_model = DKV.getGet(model.actual_best_model_key);
    ...
    private float rowFraction(Frame train, DeepLearningParameters p, DLModel m) {
