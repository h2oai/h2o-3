...
public class DLModel extends 
  ...
  private void makeWeightsBiases(Key destKey) {
    if (!model_info.get_params()._export_weights_and_biases) {
      ...
      System.out.println("^^^^^^^^^^^ WILL EXPORT WEIGHTS AND BIASES");
   ...
   */
  public DLModel(final Key destKey, final DeepLearningParameters parms, final DeepLearningModelOutput output, Frame train, Frame valid, int nClasses, DeepLearning deepLearning) {
...
//    Log.info("2nd speed: (samples: " + model_info().get_processed_total() + ", total_run_time: " + total_training_time_ms + ", total_scoring_time: " + total_scoring_time_ms + ", total_setup_time: " + total_setup_time_ms + ")");
  ...
  @Override protected Frame predictScoreImpl(Frame orig, Frame adaptedFr, String destination_key, Job j, boolean computeMetrics) {
    if (get_params()._autoencoder) {
      Frame of = reconstructOutput(adaptedFr, destination_key);
      DKV.put(of);
  ...
  private Frame reconstructOutput(Frame adaptedFr, String destination_key) {
    // Reconstruction
    ...
    return new Frame(Key.<Frame>make(destination_key), names, f.vecs());
  }
