    ...
    public final void buildModel() {
      ...
      if (_parms._checkpoint == null) {
        ...
        try {
          ...
          DKV.put(dinfo);
          cp = new DeepLearningModel(_job._result, _parms, previous, false, dinfo);
          cp.write_lock(_job);
...
//      if (checkpoint == null && !quiet_mode) logStart(); //if checkpoint is given, some Job's params might be uninitialized (but the restarted model's parameters are correct)
        ...
        if (mp._force_load_balance) {
          _job.update(0,"Load balancing training data...");
        ...
        if (val_fr != null) {
          ...
          if (mp._force_load_balance) {
            _job.update(0,"Balancing class distribution of validation data...");
