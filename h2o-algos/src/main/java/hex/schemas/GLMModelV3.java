...
import hex.glm.GLMModel.GLMModelMultinomial;
import hex.glm.GLMModel.GLMOutputMultinomial;
  ...
  public static final class GLMModelOutputV3 extends ModelOutputSchema<GLMModel.GLMOutput, GLMModelOutputV3> {
    ...
    public GLMModelOutputV3 fillFromImpl(GLMModel.GLMOutput impl) {
      ...
      if(impl.isStandardized()){
        ...
        TwoDimTable tdt;
        if(impl instanceof GLMOutputMultinomial){
          int n = impl.nclasses();
          String [] cols = new String[n*2];
          for(int i = 0; i < n; ++i) {
            cols[i] = "Coefficients." + i;
            cols[n+i] = "Standardized Coefficients." + i;
          }
          ...
          tdt = new TwoDimTable("Coefficients","glm coefficients", ns, new String[]{"Coefficients", "Standardized Coefficients"}, colTypes, colFormats, "names");
