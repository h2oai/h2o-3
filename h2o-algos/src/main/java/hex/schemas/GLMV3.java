...
public class GLMV3 extends ModelBuilderSchema<GLM,GLMV3,GLMV3.GLMParametersV3> {
  ...
  public static final class GLMParametersV3 extends ModelParametersSchemaV3<GLMParameters, GLMParametersV3> {
    ...
    @API(help="A list of predictor column names whose contribution is suppressed in predictions, " +
            "training metrics, and validation metrics -- their coefficients are zeroed before " +
            "scoring, so their effect on scored output is excluded, not just on reported metrics. " +
            "The columns remain as predictors in the trained model. Supports cross-validation " +
            "(nfolds). With-control-variables metrics are stored in separate slots and exposed via " +
            "make_unrestricted_glm_model(). Cannot be combined with interactions or lambda search. " +
            "Experimental.",
