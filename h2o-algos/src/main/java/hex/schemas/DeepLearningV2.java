...
//  @API(help = "Activation function", filter = Default.class, json = true, importance = ParamImportance.CRITICAL)
    public DeepLearningParameters.Activation activation = DeepLearningParameters.Activation.Tanh;
...
//  @API(help = "Enable shuffling of training data (recommended if training data is replicated and train_samples_per_iteration is close to #nodes x #rows)", filter = Default.class, json = true, importance = ParamImportance.EXPERT)
  ...
  @Override public DeepLearning createImpl() {
    ...
    if (parameters.source.numCols() <= 1)
    ...
    if(parameters.autoencoder && parameters.sparsity_beta > 0) {
      if (parameters.activation == DeepLearningModel.DeepLearningParameters.Activation.Tanh || parameters.activation == DeepLearningModel.DeepLearningParameters.Activation.TanhWithDropout) {
        if (parameters.average_activation >= 1 || parameters.average_activation <= -1)
        ...
        if (parameters.average_activation <= 0)
    ...
    if (parameters.autoencoder && parameters.classification) { parameters.classification = false; Log.info("Using regression mode for auto-encoder.");}
