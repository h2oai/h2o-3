...
public class MojoDelegatingModelHandler extends Handler {
    ...
    private final ByteVec getUploadedMojo(final String key) throws IllegalArgumentException{
        ...
        if (value == null) throw new IllegalArgumentException(String.format("Given MOJO file key '%s' is not to be found in H2O.", value));
        if(!value.isFrame()) throw new IllegalArgumentException(String.format("Given MOJO file key should a one-columnar Frame, however the following type has been found: '%d'", value.type()));

        Frame mojoFrame = value.get();
        if(mojoFrame.numCols() > 1) throw new IllegalArgumentException(String.format("Given MOJO frame with key '%s' should contain only 1 column with MOJO bytes. More columns found. Incorrect key provided ?", key));
        ByteVec mojoData = (ByteVec) mojoFrame.anyVec();
        
        if(mojoData.length() < 1) throw new IllegalArgumentException(String.format("Given MOJO frame with key '%s' is empty (0 bytes). Please provide a non-empty MOJO file.", key) );
