...
import hex.deeplearning.DlColumn;
...
public class DataInfo extends Keyed<DataInfo> {
  ...
  public int responseChunkId(int n) {
    return n + _cats + _nums + (_weights ? 1 : 0) + (_offset ? 1 : 0) + (_fold ? 1 : 0);
  }

  public int foldChunkId() {
    return _cats + _nums + (_weights ? 1 : 0) + (_offset ? 1 : 0);
  }

  public int offsetChunkId() {
    return _cats + _nums + (_weights ? 1 : 0);
  }
  ...
  public int outputChunkId(int n) {
    return n + _cats + _nums + (_weights ? 1 : 0) + (_offset ? 1 : 0) + (_fold ? 1 : 0) + _numResponses;
  }
  ...
  public void setResponse(String name, Vec v) {
    setResponse(name, v, 0);
  }

  public void setResponse(String name, Vec v, int n) {
    _adaptedFrame.insertVec(responseChunkId(n), name, v);
  }
...
//    _adaptedFrame = train;
  ...
  protected int[] fullCatOffsets() {
    return _fullCatOffsets == null ? _catOffsets : _fullCatOffsets;
  }
  ...
  public boolean isInteractionVec(int colid) {
    if (!haveInteractions()) return false;
  ...
  public final int largestCat() {
    return _cats > 0 ? _catOffsets[1] : 0;
  }

  public final int numStart() {
    return _catOffsets[_cats];
  }
  ...
  public final int numNums() {
    ...
    if (_numOffsets == null && _intLvls.length > 0) {  // filtered columns?
      for (int[] _intLvl : _intLvls)
        nnums += _intLvl == null ? 0 : _intLvl.length - 1;  // minus 1 for the fact that we get a +1 from the dummy interaction vec sitting in the frame!
      return nnums + _nums;
    }
  ...
  public final class Row extends Iced {
    ...
    public boolean isBad() {
      return predictors_bad || responseBad();
    }
    ...
    public int nBins = 0;    // number of enum    columns (not expanded)
  ...
  public Row buildRow(int rowNumber) {
    Row row = newDenseRow();
    row.rid = rowNumber;

    for (int i = 0; i < _nums; i++) {
      final DlColumn<Double>[] columns = currentData.weights;
      assert (columns != null);
      final DlColumn<Double> column = columns[i];
      final List<Double> data = column.data;
      assert (data != null);
      row.numVals[i] = data.get(rowNumber);
    }
  ...
  public boolean haveInteractions() {
    return _interactions != null && _interactionVecs != null;
  }
  ...
  public final Row extractDenseRow(long absPos, Chunk[] chunks, int rid, Row row) {
    ...
    DataInfoExtras.fillBinIds(this, chunks, row);
    ...
    for (int i = 0; i < _nums; i++) {
      if (isInteractionVec(_cats + i)) {  // categorical-categorical interaction is handled as plain categorical (above)... so if we have interactions either v1 is categorical, v2 is categorical, or neither are categorical
        numValsIdx = getValueFromInteractions(chunks, row, numValsIdx, i);
      } else {
        final DlColumn<Double>[] columns = currentData.weights;
        assert (columns != null);
        final DlColumn<Double> column = columns[i];
        final List<Double> data = column.data;
        assert (data != null);
        double d = currentData != null ? data.get((int) row.rid) : chunks[_cats + i].atd(rid); // can be NA if skipMissing() == false
        if (Double.isNaN(d))
  ...
  private int getValueFromInteractions(Chunk[] chunks, Row row, int numValsIdx, int i) {
    int rid = row.cid;
    ...
    for (int offset = 0; offset < iwv.expandedLength(); ++offset) {
      if (i < _intLvls.length && _intLvls[i] != null && Arrays.binarySearch(_intLvls[i], offset) < 0)
        continue; // skip the filtered out interactions
      double d = 0;
  ...
  public Row newDenseRow() {
    return new Row(false, numNums(), _cats, _numResponses, 0, 0);
  }  // TODO: _nums => numNums since currently extracting out interactions into dense
  ...
  public final Row[] extractSparseRows(Chunk[] chunks) {
    ...
    for (int cid = 0; cid < _nums; ++cid) {
      ...
      if (c instanceof InteractionWrappedVec.InteractionWrappedChunk) {  // for each row, only 1 value in an interaction is 'hot' all other values are off (i.e., are 0)
        interactionOffset = getInteractionOffset(chunks, rows, interactionOffset, cid, c);
      } else {
        interactionOffset = getInteractionOffset(rows, interactionOffset, cid, c);
      }
    }
    // response(s)
    for (int i = 0; i < _numResponses; ++i) {
  ...
  private int getInteractionOffset(Row[] rows, int interactionOffset, int cid, Chunk c) {
    int oldRow = -1;
  ...
  private int getInteractionOffset(Chunk[] chunks, Row[] rows, int interactionOffset, int cid, Chunk c) {
    for (int r = 0; r < c._len; ++r) {  // the vec is "vertically" dense and "horizontally" sparse (i.e., every row has one, and only one, value)
      ...
      if (cidVirtualOffset >= 0) {
        if (cid < _intLvls.length && _intLvls[cid] != null && Arrays.binarySearch(_intLvls[cid], cidVirtualOffset) < 0)
          continue; // skip the filtered out interactions
        if (c.atd(r) == 0) continue;
        double d = c.atd(r);
        final int i = interactionOffset + cidVirtualOffset;
        if (Double.isNaN(d))
          d = _numMeans[i];  // FIXME: if this produces a "true" NA then should sub with mean? with?
        if (_normMul != null)
          d *= _normMul[i];
