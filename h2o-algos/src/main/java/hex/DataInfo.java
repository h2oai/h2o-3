...
public class DataInfo extends Keyed<DataInfo> {
  ...
  public void addResponse(String [] names, VecAry vecs) {
  ...
  public void addOutput(String name, VecAry v) {_adaptedFrame.add(name,v);}
  public VecAry getOutputVec(int i) {return _adaptedFrame.vecs(outputChunkId(i));}
  public void setResponse(String name, VecAry v){ setResponse(name,v,0);}
  public void setResponse(String name, VecAry v, int n){ _adaptedFrame.insertVec(responseChunkId(n),name,v);}
...
//          throw new IllegalArgumentException("missing column from the dataset, could not make interaction: " + interactions[i]);
    ...
    final int n = tvecs.len()-_responses - (offset?1:0) - (weight?1:0) - (fold?1:0);
    int [] nums = MemoryManager.malloc4(n);
    int [] cats = MemoryManager.malloc4(n);
    int nnums = 0, ncats = 0;
    for(int i = 0; i < n; ++i)
      if (tvecs.isCategorical(i))
        cats[ncats++] = i;
      else
        nums[nnums++] = i;
    cats = Arrays.copyOf(cats,ncats);
    nums = Arrays.copyOf(nums,nnums);
    _nums = nnums;
    _cats = ncats;
    ...
    for(int i = 0; i < ncats; ++i)
      for(int j = i+1; j < ncats; ++j)
        if( tvecs.domain(cats[i]).length < tvecs.domain(cats[j]).length ) {
          int x = cats[i];
          cats[i] = cats[j];
          cats[j] = x;
        }
    ...
    if( _interactions==null ) {
      ...
      for(int i=0;i<tvecs.len();++i)
        if( tvecs.getVecForCol(i) instanceof InteractionWrappedVec ) interactionIds.add(i);
    ...
    VecAry tvecs2 = tvecs.getVecs(ArrayUtils.join(cats,nums));
    for(int i = 0; i < ncats; ++i) {
      names[i] = train._names.getName(cats[i]);
      _catMissing[i] = missingBucket; //needed for test time
      if( tvecs2.getVecForCol(i) instanceof InteractionWrappedVec ) {
        ...
        _catOffsets[i + 1] = (len += tvecs2.domain(i).length + (missingBucket ? 1 : 0));
      }
      else
        _catOffsets[i+1] = (len += tvecs2.domain(i).length - (useAllFactorLevels?0:1) + (missingBucket? 1 : 0)); //missing values turn into a new factor level
      _catModes[i] = imputeMissing?imputeCat(tvecs2.getVecs(i)):_catMissing[i]?tvecs2.domain(i).length - (_useAllFactorLevels || isInteractionVec(i)?0:1):-100;
    ...
    for(int i = 0; i < nnums; ++i) {
      names[i+ncats] = train._names.getName(nums[i]);
      isIWV = tvecs2.getVecForCol(i+ncats) instanceof InteractionWrappedVec;
      if( isIWV ) {
        if( null!=_interactions ) _interactions[interactionIdx].vecIdx=i+ncats;
        _interactionVecs[interactionIdx++]=i+ncats;
      }
      _numOffsets[i+1] = (len+= (isIWV ? ((InteractionWrappedVec) tvecs2.getVecForCol(i+ncats)).expandedLength() : 1));
      _permutation[i+ncats] = nums[i];
    }
    _numMeans = new double[numNums()];
    int meanIdx=0;
    for(int i=0;i<nnums;++i) {
      Vec v = train.vecs().getVecForCol(nums[i]);
      if( v instanceof InteractionWrappedVec ) {
        ...
        _numMeans[meanIdx++]=train.vecs().mean(nums[i]);
    }
    int [] responses = new int[nResponses];
    for(int i = names.length-nResponses - (weight?1:0) - (offset?1:0) - (fold?1:0); i < names.length; ++i) {
      names[i] = train._names.getName(i);
      responses[i] = i;
    }
    tvecs2.addVecs(train.vecs(responses));
    _adaptedFrame = new Frame(names,tvecs2);
    train.restructure(new Frame.Names(names),tvecs2);
    if (valid != null)
      valid.restructure(new Frame.Names(names),valid.vecs(names));
//    _adaptedFrame = train;
  ...
  public DataInfo validDinfo(Frame valid) {
    ...
    res._adaptedFrame = new Frame(null,_adaptedFrame._names,valid.vecs(_adaptedFrame._names.getNames()));
    res._valid = true;
    return res;
  }
  ...
  public DataInfo scoringInfo(Frame adaptFrame){
    ...
    if(resId == -1 || adaptFrame.vecs().naCnt(resId) == adaptFrame.numRows())
      res._responses = 0;
  ...
  public double[] denormalizeBeta(double [] beta) {
    int N = fullN()+1;
    assert (beta.length % N) == 0:"beta numRows = " + beta.length + " expected multiple of" + N;
  ...
  private DataInfo(DataInfo dinfo,Frame fr, double [] normMul, double [] normSub, int[][] catLevels, int[][] intLvls, int [] catModes, int[] activeCols) {
    ...
    for(int i=0;i<fr.numCols();++i)
      if( fr.vecs().getVecForCol(i) instanceof InteractionWrappedVec ) interactionVecs.add(i);
  ...
  public static int imputeCat(VecAry v) {
    if(v.isCategorical(0)) return v.mode(0);
    return (int)Math.round(v.mean(0));
  }
  ...
  public DataInfo filterExpandedColumns(int [] cols){
    ...
    Frame f = new Frame(_adaptedFrame);
  ...
  private void setTransform(TransformType t, double [] normMul, double [] normSub, int vecStart, int n) {
    ...
    VecAry vecs = _adaptedFrame.vecs();
    for (int i = 0; i < n; ++i) {

      boolean isIWV = _adaptedFrame.vecs().getVecForCol(vecStart + i) instanceof InteractionWrappedVec;
      switch (t) {
        case STANDARDIZE:
          if( isIWV ) {
            InteractionWrappedVec iwv = (InteractionWrappedVec)_adaptedFrame.vecs().getVecForCol(vecStart + i);
            ...
            normMul[idx] = (vecs.sigma(vecStart + i) != 0) ? 1.0 / vecs.sigma(vecStart + i) : 1.0;
            normSub[idx] = vecs.mean(vecStart + i);
          }
          break;
        case NORMALIZE:
          if( isIWV ) throw H2O.unimpl();
          normMul[idx] = (vecs.max(vecStart + i) - vecs.min(vecStart + i) > 0)?1.0/(vecs.max(vecStart + i) - vecs.min(vecStart + i)):1.0;
          normSub[idx] = vecs.mean(vecStart + i);
          break;
        case DEMEAN:
          if( isIWV ) throw H2O.unimpl();
  ...
  public boolean isInteractionVec(int colid) {
    ...
    if( _adaptedFrame!=null )
      return _adaptedFrame.vecs().getVecForCol(colid) instanceof InteractionWrappedVec;
    else
  ...
  public final String[] coefNames() {
    ...
    final VecAry vecs = _adaptedFrame.vecs();

    // first do all of the expanded categorical names
    for(int i = 0; i < _cats; ++i) {
      for (int j = (_useAllFactorLevels || vecs.getVecForCol(i) instanceof InteractionWrappedVec) ? 0 : 1; j < vecs.domain(i).length; ++j) {
        int jj = getCategoricalId(i, j);
        if(jj < 0)
          continue;
        res[k++] = _adaptedFrame._names.getName(i) + "." + vecs.domain(i)[j];
      }
      if (_catMissing[i] && getCategoricalId(i,_catModes[i]) >=0)
        res[k++] = _adaptedFrame._names.getName(i) + ".missing(NA)";
      if( vecs.getVecForCol(i) instanceof InteractionWrappedVec ) {
        InteractionWrappedVec iwv = (InteractionWrappedVec)vecs.getVecForCol(i);
    ...
    if( _interactions==null ) {
      ...
      for (int i = 0; i <= _nums; i++) {
        ...
        if (vecs.getVecForCol(i+_cats) instanceof InteractionWrappedVec && ((v = (InteractionWrappedVec) vecs.getVecForCol(i+_cats)).domain(0) != null)) { // in this case, get the categoricalOffset
          for (int j = _useAllFactorLevels?0:1; j < v.domain(0).length; ++j) {
            if (getCategoricalIdFromInteraction(_cats+i, j) < 0)
              continue;
            res[k++] = _adaptedFrame._names.getName(i+_cats) + "." + vecs.domain(i+_cats)[j];
          }
        } else
          res[k++] = _adaptedFrame._names.getName(i+_cats);
      }
    }
    _coefNames = res;
    return res;
  ...
  public int[] mapNames(String[] names) {
    assert names.length == _adaptedFrame._names.len() : "Names must be the same length!";
    int[] idx = new int[names.length];
    Arrays.fill(idx, -1);

    for(int i = 0; i < _adaptedFrame._names.len(); i++) {
  ...
  public final int getCategoricalIdFromInteraction(int cid, int val) {
    InteractionWrappedVec v;
    if( (v=(InteractionWrappedVec)_adaptedFrame.vecs().getVecForCol(cid)).isCategorical(0) ) return getCategoricalId(cid,val);
    assert v.domain(0)!=null : "No domain levels found for interactions! cid: " + cid + " val: " + val;
    cid -= _cats;
  ...
  public final Row extractDenseRow(Chunks chunks, int rid, Row row) {
    row.predictors_bad = false;
    row.response_bad = false;
    row.rid = rid + chunks.start();
    row.cid = rid;
    if(_weights)
      row.weight = chunks.atd(rid,weightChunkId());
    ...
    int nbins = 0;
    for (int i = 0; i < _cats; ++i) {
      int cid = getCategoricalId(i,chunks.isNA(rid,i)?_catModes[i]:(int)chunks.at8(rid,i));
      if(cid >= 0)
    ...
    for( int i=0;i<n;i++) {
      if( isInteractionVec(_cats + i) ) {  // categorical-categorical interaction is handled as plain categorical (above)... so if we have interactions either v1 is categorical, v2 is categorical, or neither are categorical
        InteractionWrappedVec iwv = (InteractionWrappedVec)_adaptedFrame.vecs().getVecForCol(_cats+i);
        int interactionOffset = getInteractionOffset(chunks,_cats+i,rid,_useAllFactorLevels);
        for(int offset=0;offset<iwv.expandedLength();++offset) {
    ...
    if(_offset)
      row.offset = chunks.atd(rid,offsetChunkId());
    return row;
  }
  public int getInteractionOffset(Chunks chunks, int cid, int rid, boolean useAllFactors) {
    InteractionWrappedVec.InteractionWrappedChunk c = (InteractionWrappedVec.InteractionWrappedChunk)chunks.getChunk(cid);
    return c.getRaw(rid,useAllFactors);
  }
  public VecAry getWeightsVec(){return _adaptedFrame.vecs(weightChunkId());}
  public VecAry getOffsetVec(){return _adaptedFrame.vecs(offsetChunkId());}
  ...
  public final class Rows {
    ...
    private final Chunks _chks;

    private Rows(Chunks chks, boolean sparse) {
      _nrows = chks.numRows();
      _sparse = sparse;
  ...
  public Rows rows(Chunks chks) {
    int cnt = 0;
    for(Chunk c:chks.getChunks())
      if(c.isSparseZero())
        ++cnt;
    return rows(chks,cnt > (chks.numCols() >> 1));
  }
  public Rows rows(Chunks chks, boolean sparse) {return new Rows(chks,sparse);}
   ...
   */
  public final Row[] extractSparseRows(Chunks chunks) {
    Row[] rows = new Row[chunks.numRows()];
    ...
    for (int i = 0; i < rows.length; ++i) {
      ...
      if(_offset)  {
        rows[i].offset = chunks.atd(i,offsetChunkId());
      ...
      if(_weights) {
        rows[i].weight = chunks.atd(i,weightChunkId());
    ...
    for (int i = 0; i < _cats; ++i) {
      for (int r = 0; r < rows.length; ++r) {
        Row row = rows[r];
        int cid = getCategoricalId(i,chunks.isNA(r,i)?_catModes[i]:chunks.at4(r,i));
        if(cid >=0)
    ...
    for (int cid = 0; cid < _nums; ++cid) {
      ...
      if (chunks.getChunk(c) instanceof InteractionWrappedVec.InteractionWrappedChunk) {  // for each row, only 1 value in an interaction is 'hot' all other values are off (i.e., are 0)
        for(int r=0;r<rows.length;++r) {  // the vec is "vertically" dense and "horizontally" sparse (i.e., every row has one, and only one, value)
          Row row = rows[r];
          if( chunks.isNA(r,c) && _skipMissing)
            row.predictors_bad = true;
          if(row.predictors_bad) continue;
          int cidVirtualOffset = getInteractionOffset(chunks,_cats+cid,r,_useAllFactorLevels);  // the "virtual" offset into the hot-expanded interaction
        ...
        int n = chunks.getChunk(c).asSparseDoubles(vals,ids, _skipMissing?Double.NaN:_numMeans[interactionOffset]);
        for (int i = 0; i < n; ++i) {
          int r = ids[i];
          assert r > oldRow;
