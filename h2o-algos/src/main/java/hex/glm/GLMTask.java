...
//        if(i != _c)
  ...
  public static class GLMMultinomialSpeedUpUpdate extends FrameTask2<GLMMultinomialSpeedUpUpdate> {
    ...
    private transient double [] _sparseOffsets;
    private transient double [] _etas;
    private int _nclass;            // number of multinomial classes
    ...
    @Override public void chunkInit(){
      // initialize
      _sparseOffsets = MemoryManager.malloc8d(_nclass);
  ...
  public static class GLMIterationTask extends FrameTask2<GLMIterationTask> {
    ...
    public void chunkInit() {
      ...
      _sparse = _sparse && !_multiClassSpeedup; // turn off sparse for native multinomial
      if(_sparse ) {
        if (_multiClassSpeedup) // keep this part until I figure out how to support _sparse
