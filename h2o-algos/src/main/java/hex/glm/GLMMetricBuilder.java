...
public class GLMMetricBuilder extends MetricBuilderSupervised<GLMMetricBuilder> {
  ...
  public double[] perRow(double ds[], float[] yact, double weight, double offset, Model m) {
    ...
    if (_glmf != null) {
      if (Family.negativebinomial.equals(_glmf._family))
        _log_likelihood += m.likelihood(weight, yact[0], ds[0]);
      if (!ArrayUtils.hasNaNsOrInfs(ds) && !ArrayUtils.hasNaNsOrInfs(yact)) {
        if (Family.multinomial.equals(_glmf._family) || Family.ordinal.equals(_glmf._family))
          add2(yact[0], ds, weight, offset);
        else if (Family.binomial.equals(_glmf._family) || Family.quasibinomial.equals(_glmf._family) ||
                Family.fractionalbinomial.equals(_glmf._family))
