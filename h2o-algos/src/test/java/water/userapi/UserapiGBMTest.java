...
public class UserapiGBMTest extends TestUtil {
  ...
  public void testGBMRegressionGaussian() {
    Scope.enter();
    Frame fr = Scope.track(parse_test_file("./smalldata/gbm_test/Mfgdata_gaussian_GBM_testing.csv"));
...
//             new PrepData() { int prep(Frame fr) { return fr.numCols()-1; } });
  ...
  public void testGBMTrainTest() {
    ...
    try {
      ...
      ModelMetricsBinomial mm = ModelMetricsBinomial.getFromDKV(gbm, parms.valid());
  ...
  public void testModelAdaptMultinomial() {
    ...
    try {
      ...
      ModelMetricsMultinomial mm = ModelMetricsMultinomial.getFromDKV(gbm, parms.valid());
...
//      DKV.put(tfr);
  ...
  public void testCategorical() {
    ...
    try {
      ...
      for (int i = 0; i < N; ++i) {
        ...
        ModelMetricsBinomial mm = ModelMetricsBinomial.getFromDKV(gbm, parms.train());
        double auc = mm._auc._auc;
  ...
  public void testCUST_A() {
    ...
    try {
      ...
      ModelMetricsBinomial tmm = ModelMetricsBinomial.getFromDKV(gbm, tfr);
      ModelMetricsBinomial vmm = ModelMetricsBinomial.getFromDKV(gbm, vfr);
      ...
      ModelMetricsBinomial tmm2 = ModelMetricsBinomial.getFromDKV(gbm, tfr);
      ModelMetricsBinomial vmm2 = ModelMetricsBinomial.getFromDKV(gbm, vfr);
      assert tmm != tmm2;
      assert vmm != vmm2;
  ...
  public void testNoRowWeights() {
    ...
    try {
      ...
      ModelMetricsBinomial mm2 = ModelMetricsBinomial.getFromDKV(gbm, parms.train());
...
//        parms._offset_column = "logInsured"; //POJO scoring not supported for offsets
  ...
  public void testStochasticGBMHoldout() {
    ...
    double[] col_sample_rates_per_tree = new double[]{0.4f, 0.6f, 1.0f};

    Map<Double, Triple<Double>> hm = new TreeMap<>();
    GradientBoosting gb = 
        new GradientBoosting(split)
            .responseColumn("Angaus") //regression
            .seed(42)
            .minRows(2)
            .maxDepth(12)
            .nTrees(6);

    for (double sampleRate : sample_rates) {
      for (double colSampleRate : col_sample_rates) {
        for (double colSampleRatePerTree : col_sample_rates_per_tree) {
          gb.colSampleRate(colSampleRate)
            .colSampleRatePerTree(colSampleRatePerTree)
            .sampleRate(sampleRate);

          ModelMetricsRegression mm = gb.metrics();
          hm.put(mm.mse(), new Triple<>(sampleRate, colSampleRate, colSampleRatePerTree));
    ...
    // iterator over results (min to max MSE) - best to worst
    for (Map.Entry<Double, Triple<Double>> n : hm.entrySet()) {
      Triple<Double> v = n.getValue();
      Log.info("MSE: " + n.getKey()
          + ", row sample: " + v.v1
          + ", col sample: " + v.v2
          + ", col sample per tree: " + v.v3);
    }
  }
