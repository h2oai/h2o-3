...
public class WorkFlowTest extends TestUtil {
  ...
  private void testWorkFlow(String[] files) {
    try {
      ...
      // Drop bike id, as it should not add any improvement to predictions
      // CNC - currently raising r2 from .66 to .70, so leaving in
      data.remove("bikeid").remove();

      // Drop long/lat as the station name is already a unique identifier
      // CNC - currently raising r2 from .62 to .70, so leaving in
