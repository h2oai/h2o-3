...
public abstract class DeepWaterAbstractIntegrationTest extends TestUtil {
  ...
  public void memoryLeakTest() {
    ...
    while (counter-- > 0) {
      try {
        ...
        p._train =
            (tr = parse_test_file("bigdata/laptop/deepwater/imagenet/cat_dog_mouse.csv"))._key;
  ...
  public void convergenceInceptionColor() {
    checkConvergence(3, DeepWaterParameters.Network.inception_bn, 30);
  }

  @Test
  public void convergenceInceptionGrayScale() {
    checkConvergence(1, DeepWaterParameters.Network.inception_bn, 30);
  }
  ...
  public void convergenceGoogleNetGrayScale() {
    checkConvergence(1, DeepWaterParameters.Network.googlenet, 100);
  }

  @Test
  public void convergenceLenetColor() {
    checkConvergence(3, DeepWaterParameters.Network.lenet, 125);
  }

  @Test
  public void convergenceLenetGrayScale() {
    checkConvergence(1, DeepWaterParameters.Network.lenet, 50);
  }

  @Ignore
  @Test
  public void convergenceVGGColor() {
    checkConvergence(3, DeepWaterParameters.Network.vgg, 50);
  }

  @Ignore
  @Test
  public void convergenceVGGGrayScale() {
    checkConvergence(1, DeepWaterParameters.Network.vgg, 50);
  }

  @Ignore
  @Test
  public void convergenceResnetColor() {
    checkConvergence(3, DeepWaterParameters.Network.resnet, 50);
  }

  @Ignore
  @Test
  public void convergenceResnetGrayScale() {
    checkConvergence(1, DeepWaterParameters.Network.resnet, 50);
  }

  @Ignore
  @Test
  public void convergenceAlexnetColor() {
    checkConvergence(3, DeepWaterParameters.Network.alexnet, 50);
  }

  @Ignore
  @Test
  public void convergenceAlexnetGrayScale() {
    checkConvergence(1, DeepWaterParameters.Network.alexnet, 50);
  }
  ...
  public void restoreState(DeepWaterParameters.Network network) {
    ...
    try {
      ...
      Log.info(
          "Original LL: " + ((ModelMetricsMultinomial) m1._output._training_metrics).logloss());
      ...
      Assert.assertEquals(
          ((ModelMetricsMultinomial) m1._output._training_metrics).logloss(),
          mm1.logloss(),
          1e-5 * mm1.logloss()); //make sure scoring is self-consistent
  ...
  public void imageURLs() {
    ...
    try {
      ...
      p._backend = getBackend();
      p._train =
          (tr = parse_test_file("smalldata/deepwater/imagenet/binomial_image_urls.csv"))._key;
      p._response_column = "C2";
  ...
  public void MNISTLenet() {
    ...
    try {
      ...
      if (file != null) {
        ...
        m = j.trainModel().get();
        Assert.assertTrue(
            ((ModelMetricsMultinomial) (m._output._validation_metrics)).mean_per_class_error()
                < 0.05);
      }
    } finally {
      if (tr != null) tr.remove();
  ...
  private void MOJOTest(
      Model.Parameters.CategoricalEncodingScheme categoricalEncodingScheme,
      boolean enumCols,
      boolean standardize) {
    Frame tr = null;
    Frame tr2 = null;
    ...
    try {
      ...
      Assert.assertTrue(
          Math.abs(auc - ((ModelMetricsBinomial) m._output._training_metrics).auc()) < 1e-3);
      if (standardize) Assert.assertTrue(auc > 0.7);
    ...
    try {
      ...
      try {
        ...
        for (int i = 0; i < table.getRowDim(); ++i) {
          ...
          Assert.assertTrue(
              "Timestamp must be later than outside timer start", timeStampLong >= start);
          Assert.assertTrue(
              "Timestamp must be earlier than outside timer end", timeStampLong <= end);
          ...
          try {
            ...
            Assert.assertTrue(
                "Duration must increase: " + priorDurationDouble + " -> " + durationDouble,
                durationDouble >= priorDurationDouble);
            Assert.assertTrue(
                "Duration cannot be more than outside timer delta",
                durationDouble <= (end - start) / 1e3);
          ...
          Assert.assertTrue(
              "Epoch counter must be contiguous", (Double) table.get(i, 3) == i); //1 epoch per step
          Assert.assertTrue(
              "Iteration counter must match epochs",
              (Integer) table.get(i, 4) == i); //1 iteration per step
        }
        try {
          ...
          Assert.assertTrue(
              "Duration must be smooth",
              Double.parseDouble(durationAfter) - Double.parseDouble(durationBefore)
                  < sleepTime + 1);
          ...
          Assert.assertTrue(
              "Time stamp must experience a delay",
              timeStampAfterLong - timeStampBeforeLong >= (sleepTime - 1 /*rounding*/) * 1000);
          ...
          Assert.assertTrue(
              "Speed shouldn't change more than 50%",
              Math.abs(speedAfterDouble - speedBeforeDouble) / speedBeforeDouble
                  < 0.5); //expect less than 50% change in speed
        } catch (NumberFormatException ex) {
  ...
  */
  ...
  public void testCheckpointOverwriteWithBestModel() {
    ...
    try {
      ...
      FrameSplitter fs =
          new FrameSplitter(
              tfr, new double[] {0.8}, new Key[] {Key.make("train"), Key.make("valid")}, null);
      fs.compute2();
