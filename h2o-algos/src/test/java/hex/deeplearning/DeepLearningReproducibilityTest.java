    ...
    for (boolean repro : new boolean[]{true, false}) {
      ...
      for (int repeat = 0; repeat < N; ++repeat) {
        try {
          ...
          Assert.assertTrue(FrameUtils.isBitIdentical(data, golden)); //test parser consistency
...
//          p._nfolds = 2;
          ...
          for (int i=0; i<5; ++i) {
            ...
            Assert.assertTrue("Prediction #" + i + " for repeat #" + repeat + " differs!", FrameUtils.isBitIdentical(preds[repeat],tmp));
            tmp.delete();
          }
