...
public class DeepLearningTest extends TestUtil {
  ...
  static VecAry unifyFrame(DeepLearningParameters drf, Frame fr, PrepData prep, boolean classification) {
  ...
  public void basicDL(String fnametrain, String hexnametrain, String fnametest, PrepData prep, int epochs, double[][] expCM, String[] expRespDom, double expMSE, int[] hidden, double l1, boolean classification, DeepLearningParameters.Activation activation) throws Throwable {
    ...
    try {
      ...
      VecAry removeme = unifyFrame(dl, frTrain, prep, classification);
      if (removeme != null) Scope.track(removeme.vecs());
  ...
  @Test public void elasticAveragingTrivial() {
    ...
    try {
      for (int i = 0; i < N; ++i) {
        ...
        VecAry resp = frTrain.lastVec().toCategoricalVec();
        frTrain.removeVecs(frTrain.vecs()._numCols - 1).removeVecs();
  ...
  @Test public void elasticAveraging() {
    ...
    if (covtype) {
      ...
      VecAry resp = frTrain.vecs(0).toCategoricalVec();
      frTrain.removeVecs(0).removeVecs();
...
//      first1kCSV.removeVecs(first1kCSV.find("C1")).removeVecs(); //removeVecs id
      VecAry response = first1kCSV.removeVecs(first1kCSV.find("C2")); //removeVecs response, but keep it around outside the frame
      VecAry responseFactor = response.toCategoricalVec(); //turn response into a categorical
      response.remove();
...
//      second1kCSV.removeVecs(second1kCSV.find("C1")).removeVecs(); //removeVecs id
//      response = second1kCSV.removeVecs(second1kCSV.find("C2")); //removeVecs response, but keep it around outside the frame
...
//      third1kCSV.removeVecs(third1kCSV.find("C1")).removeVecs(); //removeVecs id
//      response = third1kCSV.removeVecs(third1kCSV.find("C2")); //removeVecs response, but keep it around outside the frame
//      responseFactor = response.toCategoricalVec(); //turn response into a categorical
//      response.removeVecs();
...
//        DKV.removeVecs(model._key); model._key = Key.make(); DKV.put(model);
    ...
    for (boolean ae : new boolean[]{
      ...
      try {
        ...
        for (String s : new String[]{
          ...
          VecAry resp = tfr.vec(s).toCategoricalVec();
...
//      Assert.assertTrue(dl1._output._training_metrics.mse() < dl2._output._training_metrics.mse());
  ...
  public void testCategoricalEncodingAUTO() {
    ...
    try {
      ...
      VecAry v = tfr.removeVecs("survived");
  ...
  public void testCategoricalEncodingBinary() {
    ...
    try {
      ...
      if (tfr.vec(response).isBinary()) {
        VecAry v = tfr.removeVecs(response);
  ...
  public void testCategoricalEncodingEigen() {
    ...
    try {
      ...
      if (vfr.vec(response).isBinary()) {
        VecAry v = vfr.removeVecs(response);
  ...
  public void testMultinomialMNIST() {
    ...
    try {
      ...
      if (file != null) {
        ...
        preds.removeVecs(0); //removeVecs label, keep only probs
        VecAry labels = small.vec("C785"); //actual
  ...
  public void testMultinomial() {
    ...
    try {
      ...
      VecAry v = train.removeVecs("pclass");
      ...
      preds.removeVecs(0); //removeVecs label, keep only probs
      VecAry labels = train.vec("pclass"); //actual
  ...
  public void testBinomial() {
    ...
    try {
      train = parse_test_file("./smalldata/junit/titanic_alt.csv");
      VecAry v = train.removeVecs("survived");
      ...
      large = fs.getResult()[1];
      preds = model.score(small);
      VecAry labels = small.vec("survived"); //actual
  ...
  public void testRegression() {
    ...
    try {
      ...
      preds = model.score(train);
      VecAry targets = train.vec("age"); //actual
