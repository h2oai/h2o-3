...
import hex.AUC2;
import hex.ScoredClassifierRegressor;
...
//             new PrepData() { int prep(Frame fr) { return fr.numCols()-1; } });
  ...
  @Test public void testModelMSEEqualityOnProstateBernoulli() {
  ...
  @Test public void testModelMSEEqualityOnProstateGaussian() {
    final PrepData prostatePrep = new PrepData() { @Override int prep(Frame fr) { fr.remove("ID").remove(); return ~fr.find("CAPSULE"); } };
    ScoredClassifierRegressor[] mseWithoutVal = basicGBM("./smalldata/logreg/prostate.csv", prostatePrep, false, Family.gaussian)._scored_train;
    ScoredClassifierRegressor[] mseWithVal    = basicGBM("./smalldata/logreg/prostate.csv", prostatePrep, true , Family.gaussian)._scored_valid;
    Assert.assertArrayEquals("GBM has to report same list of MSEs for run without/with validation dataset (which is equal to training data)", mseWithoutVal, mseWithVal);
  }
  ...
  @Test public void testModelMSEEqualityOnTitanicBernoulli() {
    ...
    ScoredClassifierRegressor[] mseWithoutVal = basicGBM("./smalldata/junit/titanic_alt.csv", titanicPrep, false, Family.bernoulli)._scored_train;
    ScoredClassifierRegressor[] mseWithVal    = basicGBM("./smalldata/junit/titanic_alt.csv", titanicPrep, true , Family.bernoulli)._scored_valid;
    Assert.assertArrayEquals("GBM has to report same list of MSEs for run without/with validation dataset (which is equal to training data)", mseWithoutVal, mseWithVal);
  }

  @Test public void testModelMSEEqualityOnTitanicMultinomial() {
    final PrepData titanicPrep = new PrepData() { @Override int prep(Frame fr) { return fr.find("survived"); } };
    ScoredClassifierRegressor[] mseWithoutVal = basicGBM("./smalldata/junit/titanic_alt.csv", titanicPrep, false, Family.multinomial)._scored_train;
    ScoredClassifierRegressor[] scoredWithVal = basicGBM("./smalldata/junit/titanic_alt.csv", titanicPrep, true , Family.multinomial)._scored_valid;
    Assert.assertArrayEquals("GBM has to report same list of MSEs for run without/with validation dataset (which is equal to training data)", mseWithoutVal, scoredWithVal);
  }

  @Test public void testBigCat() {
    final PrepData prep = new PrepData() { @Override int prep(Frame fr) { return fr.find("y"); } };
    basicGBM("./smalldata/gbm_test/50_cattest_test.csv" , prep, false, Family.bernoulli);
    basicGBM("./smalldata/gbm_test/50_cattest_train.csv", prep, false, Family.bernoulli);
    basicGBM("./smalldata/gbm_test/swpreds_1000x3.csv"  , prep, false, Family.bernoulli);
  }
  ...
  @Test @Ignore public void testKDDTrees() {
    ...
    try {
      ...
      ScoredClassifierRegressor[] firstScored = gbm1._output._scored_valid;
      ...
      for( int i=0; i<10; i++ ) {
        ...
        job2.remove();
        ScoredClassifierRegressor[] secondScored = gbm2._output._scored_valid;
