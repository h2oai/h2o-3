 ...
 *
 * Oracle (no frozen arrays): the offset is always added at the LINK scale (linear predictor, before link-inverse),
 ...
 *
 * GLM notes vs the GBM baseline: GLM selects behavior by _family (not _distribution). Regularization is
 * disabled (_lambda=0) so the fit is a plain GLM and the oracle holds cleanly. gamma defaults to the inverse
 * link, so it is forced to log; tweedie keeps its tweedie link with _tweedie_link_power=0 (= log scale).
 */
@RunWith(Parameterized.class)
public class GLMRemoveOffsetBaselineTest extends TestUtil {
  ...
  public void defaultScoringAppliesOffsetAndIsStable() {
    ...
    try {
      ...
      parms._lambda = new double[]{0};                 // no regularization -> plain GLM, clean oracle
      if (family == Family.gamma) parms._link = GLMParameters.Link.log; // gamma defaults to inverse
      if (family == Family.tweedie) {
        parms._tweedie_variance_power = 1.5;
        parms._tweedie_link_power = 0;                  // 0 -> log link
      }
