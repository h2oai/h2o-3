...
//    System.out.println("made copy of vec " + v._key + " -> " + v2._key + ", in DKV? src =" + ((DKV.get(v._key) != null)) + ", dst = " + (DKV.get(v2._key) != null));
  ...
  public void testSparse() {
    ...
    DKV.put(_airlinesMM._key,_airlinesMM);
    Frame airlinesStd = _airlinesMM.deepCopy("airlines_std");
    DKV.put(airlinesStd);
    for(int i = 0; i < airlinesStd.numCols()-1; ++i){
      Vec v = airlinesStd.vec(i);
      double sub = v.mean();
      double mul = 1.0/v.sigma();
      Vec.Writer vw = v.open();
      for(int j = 0; j < airlinesStd.numRows(); ++j)
        vw.set(j,(v.at(j) - sub)*mul);
      vw.close();
    }
    GLMParameters parms0 = new GLMParameters(Family.gaussian);
    parms0._weights_column = "weights";
    parms0._train = airlinesStd._key;
    parms0._lambda = new double[]{1e-2};
    parms0._alpha = new double[]{0};
    parms0._solver = Solver.IRLSM;
    parms0._ignored_columns = new String[]{"C1"};
//    parms._remove_collinear_columns = true;
    parms0._response_column = "IsDepDelayed";
    parms0._standardize = false;
    parms0._objective_epsilon = 0;
    parms0._gradient_epsilon = 1e-10;
    parms0._max_iterations = 1000;
    parms0._missing_values_handling = DeepLearningModel.DeepLearningParameters.MissingValuesHandling.Skip;
    long t0 = System.currentTimeMillis();
    GLMModel m0 = new GLM(parms0).trainModel().get();
    long tdelta0 = System.currentTimeMillis()-t0;
    System.out.println("dense model done in " + tdelta0 + "ms");
    parms0 = new GLMParameters(Family.gaussian);
    parms0._weights_column = "weights";
    parms0._train = _airlinesMM._key;
    parms0._lambda = new double[]{1e-2};
    parms0._alpha = new double[]{0};
    parms0._solver = Solver.IRLSM;
    parms0._ignored_columns = new String[]{"C1"};
//    parms._remove_collinear_columns = true;
    parms0._response_column = "IsDepDelayed";
    parms0._standardize = true;
    parms0._objective_epsilon = 0;
    parms0._gradient_epsilon = 1e-10;
    parms0._max_iterations = 1000;
    parms0._missing_values_handling = DeepLearningModel.DeepLearningParameters.MissingValuesHandling.Skip;
    long t1 = System.currentTimeMillis();
    GLMModel m1 = new GLM(parms0).trainModel().get();
    long tdelta1 = System.currentTimeMillis()-t1;
    System.out.println("dense model done in " + tdelta1 + "ms");
    System.out.println("coefs0 " + Arrays.toString(m0.beta()));
    System.out.println("coefs1 " + Arrays.toString(m1._output.getNormBeta()));
    Assert.assertArrayEquals(m0.beta(),m1._output.getNormBeta(),1e-8);
//      for(int i = 0; i < model1._output._coefficient_names.length; ++i)
//        assertEquals("mismatch at coef " + model1._output.coefficientNames()[i],exp_coefs[i],model1._output.getNormBeta()[i],Math.abs(exp_coefs[i])*1e-8);
//    } finally {
...
//      double[] pvals_expected = new double[]{2.546098e-01, 1.979113e-01, 8.814975e-01, 9.531266e-01, 5.747131e-01, 2.683977e-02, 1.439295e-03, 2.228612e-01, 1.082711e-01, 1.893210e-03, 1.644916e-01, 2.805776e-07 };
      ...
      for (int i = 0; i < zvals_expected.length; ++i) {
        int id = coefMap.get(names_actual[i]);
        assertEquals("zval for " + names_actual[i] + " does not match",zvals_expected[id], zvals_actual[i], Math.abs(zvals_expected[id]) * 1e-5);
        assertEquals(pvals_expected[id], pvals_actual[i], pvals_expected[id] * 1e-3);
      }
      predict = model.score(params._train.get());
      Vec.Reader r = predict.vec("StdErr").new Reader();
