...
//      params._lambda_search = true;
  ...
  public void testMultinomialADMMSpeedUp(){
    ...
    try {
      ...
      GLMModel noadmml2pen = checkADMM(fr, lambda,0, true, Solver.IRLSM_SPEEDUP_NO_ADMM);  // check with only enum columns with both
      GLMModel noadmmbothreg = checkADMM(fr, lambda,0.5, true, Solver.IRLSM_SPEEDUP_NO_ADMM);  // check with only enum columns with both
      Scope.track_generic(noadmml2pen);
      Scope.track_generic(noadmmbothreg);
      
      // compare admm and no admm with same kind of reg
