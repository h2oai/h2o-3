...
public class GLMTweedieDispersionOnlyTest extends TestUtil {
    ...
    public void testTweedieDispersionPLess2() {
        ...
        try {
            ...
            assertMLBetterThanPearson(train2, 2, 1.8, 1.5, "resp", 15);
            assertMLBetterThanPearson(train2, 2, 1.8, 2.5, "resp", 8);
        } finally {
            Scope.exit();
        }
    }
    ...
    public void testTweedieDispersionPExceed2p3phi0p5() {
        ...
        try {
            ...
            assertMLBetterThanPearson(train, 0.5, 3, 0.1, "x", 8);
            assertMLBetterThanPearson(train, 0.5, 3, 1.5, "x", 8);
        } finally {
            Scope.exit();
        }
    }
    ...
    public void testTweedieDispersionPExceed2P3phi1() {
        ...
        try {
            ...
            assertMLBetterThanPearson(train, 1, 3, 1.5, "x", 8);
            assertMLBetterThanPearson(train, 1, 3, 0.5, "x", 8);
        } finally {
            Scope.exit();
        }
    }
    ...
    public void testTweedieDispersionPExceed2p3phi1p5() {
        ...
        try {
            ...
            assertMLBetterThanPearson(train, 1.5, 3, 0.1, "x", 40);
            assertMLBetterThanPearson(train, 1.5, 3, 2.0, "x", 8);
        } finally {
            Scope.exit();
        }
    }
    ...
    public void testTweedieDispersionPExceed2P5Phi0p5() {
        ...
        try {
            ...
            assertMLBetterThanPearson(train, 0.5, 5, 0.1, "x", 8);
            assertMLBetterThanPearson(train, 0.5, 5, 1.5, "x", 8);
        } finally {
            Scope.exit();
        }
    }
    ...
    public void testTweedieDispersionPExceed2P5Phi1() {
        ...
        try {
            ...
            assertMLBetterThanPearson(train, 1, 5, 0.1, "x", 8);
            assertMLBetterThanPearson(train, 1, 5, 1.5, "x", 8);
        } finally {
            Scope.exit();
        }
    }
    ...
    public void assertMLBetterThanPearson(Frame train, double trueDispersion, double variance_power, double initDisp,
        ...
        try {
            ...
            params._logll_length = logLength;
    ...
    public void manuallyGenerateInfoCols(double[] manualInfoCols, TweedieMLDispersionOnly mlDisp,
        ...
        if (!Double.isNaN(resp)) {
            ...
            double sumDWV2 = resp == 0 ? 0 : calDWV2(p, logZ, alpha, (int) manualInfoCols[7], (int) manualInfoCols[8], phi, manualInfoCols[3]);
            manualInfoCols[12] = sumDWV2; // without 1/y or 1/(PI*y)
            double dWOverW = resp == 0 ? 0 : sumDWV2/Math.abs(manualInfoCols[11]);
           ...
           double d2WVOverdPhi = resp==0 ? 0 : (manualInfoCols[11]*sumD2WV-sumDWV2*sumDWV2)/(manualInfoCols[11]*manualInfoCols[11]);
           // d2ll
    ...
    public double calDWV2(double vPower, double logZ, double alpha,  int DjkL, int DjkU,
                         double phi, double logDWV2Max) {
        double sumDWV = 0;
        if (vPower < 2) {
            for (int index=DjkL; index <= DjkU; index++)
                sumDWV += Math.exp(calDLogWV(vPower, logZ, alpha, index)-logDWV2Max); // * (alpha - 1) / phi;
            sumDWV = Math.exp(Math.log(sumDWV+logDWV2Max))*(alpha-1)/phi;    
        } else {
            for (int index=DjkL; index<=DjkU; index++)
                sumDWV += Math.exp(calDLogWV(vPower, logZ, alpha, index)-logDWV2Max)*Math.pow(-1, index)*Math.sin(-index*Math.PI*alpha);//*(alpha-1)/phi;
            if (sumDWV > 0)
                sumDWV = Math.exp(Math.log(sumDWV)+logDWV2Max)*(alpha-1)/phi;
            else
                sumDWV = sumDWV*Math.exp(logDWV2Max)*(alpha-1)/phi;
        }
        return sumDWV;
    }
