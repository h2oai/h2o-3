...
//    1.31814009  0.82918839  0.63285077  0.02949062  0.00000000  0.83011321
    String [] cfs1 = new String [] {"AGE", "DPROS.b",    "DPROS.c",     "DPROS.d",  "DCAPS.b",  "PSA",      "VOL", "GLEASON"};
    double [] vals = new double [] {0.0,    0.93030614,   1.31814009,    0.82918839, 0.63285077, 0.02949062, 0.0,    0.83011321};
    GLMParameters params = new GLMParameters(Family.binomial);
    params._response_column = "CAPSULE";
    params._ignored_columns = new String[]{"ID",};
    params._train = _prostateTrain._key;
    ...
    for(Solver s:new Solver[]{Solver.AUTO,Solver.IRLSM,Solver.L_BFGS}) {
      ...
      try {
        ...
        System.out.println("metrics = " + model._output._training_metrics);
        for (int i = 0; i < cfs1.length; ++i)
          assertEquals(vals[i], coefs.get(cfs1[i]), 1e-1 * vals[i]);
