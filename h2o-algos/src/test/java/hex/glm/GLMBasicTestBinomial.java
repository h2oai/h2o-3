...
//    Residual Deviance: 305.1 	AIC: 315.1
    ...
    try {
      for (Solver s : new Solver[]{Solver.AUTO, Solver.IRLSM, Solver.L_BFGS, Solver.COORDINATE_DESCENT}){//, Solver.COORDINATE_DESCENT}) {
        ...
        try {
          ...
          for (int i = 0; i < cfs1.length; ++i)
            assertEquals(vals[i], coefs.get(cfs1[i]), s == Solver.COORDINATE_DESCENT?5e-3:1e-4);
          ...
          assertEquals(76.8525, GLMTest.residualDevianceTest(model),s == Solver.COORDINATE_DESCENT?1e-3:1e-4);
          // test scoring
          try {
...
//    Residual Deviance: 302.9 	AIC: 318.9
    ...
    for(Solver s:new Solver[]{Solver.AUTO,Solver.IRLSM,Solver.L_BFGS, Solver.COORDINATE_DESCENT}) {
      ...
      try {
        ...
        for (int i = 0; i < cfs1.length; ++i)
          assertEquals(vals[i], coefs.get(cfs1[i]), s == Solver.COORDINATE_DESCENT? 1e-2:1e-4);
        ...
        // [1]80.92923
        assertEquals(80.92923, GLMTest.residualDevianceTest(model), s == Solver.COORDINATE_DESCENT? 1e-2:1e-4);
      ...
      for (Solver s : new Solver[]{Solver.AUTO, Solver.IRLSM, Solver.L_BFGS, Solver.COORDINATE_DESCENT}) {
        ...
        try {
          ...
          for (int i = 0; i < cfs1.length; ++i) {
            ...
            assertEquals(vals[i], coefs.get(cfs1[i]), s == Solver.COORDINATE_DESCENT?1e-3:1e-4);//dec
          }
