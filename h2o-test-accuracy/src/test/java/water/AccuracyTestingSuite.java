...
public class AccuracyTestingSuite {
																	...
																	@org.testng.annotations.Optional("") String testCasesFilterString)
	{
		...
		try {
			resultsDir = new File(AccuracyTestingUtil.find_test_file_static(logDir).getCanonicalFile().toString() +
				"/results");
			h2oLogsDir = new File(AccuracyTestingUtil.find_test_file_static(logDir).getCanonicalFile().toString() +
				"/results/h2ologs");
		} catch (IOException e) {
