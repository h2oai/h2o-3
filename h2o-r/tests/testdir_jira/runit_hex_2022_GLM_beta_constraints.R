...
test.GLM.betaConstraints <- function(conn){
    ...
    run_glm <- function(  family_type = "gaussian",
        ...
        print(glm_constraints.h2o@model$training_metrics$null_deviance)
        print("Glmnet null deviance:")
        print(glm_constraints.r$nulldev)
        checkEqualsNumeric(glm_constraints.h2o@model$training_metrics$null_deviance,
                           glm_constraints.r$nulldev,
                           tolerance = 0.05)

        # Check residual deviances
        print("H2O resid deviance:")
        print(glm_constraints.h2o@model$training_metrics$residual_deviance)
        ...
        checkEqualsNumeric(glm_constraints.h2o@model$training_metrics$residual_deviance,
                           glm_constraints_resid,
                           tolerance = 0.05)

        # Check objective values (gaussian)
        if(family_type == "gaussian"){
            h2o_obj_val <- (glm_constraints.h2o@model$training_metrics$residual_deviance / nrow(prostate.hex)) + (1 - alpha) * 1e-05 *
