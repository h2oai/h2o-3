...
test.pub.501 <- function(conn) {
  ...
  tr <- h2o.importFile(conn, normalizePath(locate("smalldata/gbm_test/ecology_model.csv")), key = "tr")
  train <- h2o.importFile(conn, normalizePath(locate("smalldata/covtype/covtype.20k.data")), key = "train")
