...
test <- function(conn) {
  ...
  airline.test.hex = h2o.importFile(conn, locate("smalldata/airlines/allyears2k_headers.zip"), destination_frame="airline.test.hex", header=TRUE)
  print("Reading UUIDs into H2O")
  uuid.hex = h2o.importFile(conn, locate("smalldata/airlines/airlineUUID.csv"), destination_frame="uuid.hex", header=TRUE)
