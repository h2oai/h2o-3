...
test.basic.minmax <- function() {
  ...
  expect_that(as.double(iris1_min), equals(min(iris[,1])))
  expect_that(as.double(iris1_max), equals(max(iris[,1])))
  ...
  expect_that(as.double(irisall_min), equals(min(iris[,-5])))
  expect_that(as.double(irisall_max), equals(max(iris[,-5])))
  
  Log.info("Shuffle order of differently typed arguments to min and max")
  expect_that(as.double(min(hex[, 1], 0, 2.5)), equals(min(hex[,1], 0, 2.5)))
  expect_that(as.double(min(hex[,-5], 4,  -5)), equals(min(hex[,-5], -5, 4)))
  expect_that(as.double(max(hex[, 1], 5,   3)), equals(max(hex[,1], 3, 5)))
  expect_that(as.double(max(hex[,-5], 10,  3)), equals(max(hex[,-5], 10, -3)))

  Log.info("min and max corretness")
  df <- data.frame(c(1,-0.1,0))
  expect_that(as.double(min(as.h2o( df))), equals(min(df)))
  expect_that(as.double(max(as.h2o( df))), equals(max(df)))
