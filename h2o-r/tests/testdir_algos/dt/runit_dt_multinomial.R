...
test_dt_multinomial <- function() {
  ...
  # Train the model
  dt <- h2o.decisionTree(max_depth = 3, x = predictors, y = response_col, training_frame = data)
  
  # Print model summary
  print(dt)
}

doTest("Decision tree: multinomial classification", test.compare.classification.with.r)
