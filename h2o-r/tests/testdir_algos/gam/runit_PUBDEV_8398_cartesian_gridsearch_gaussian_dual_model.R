...
test.model.gam.random.gridsearch.dual.modes <- function() {
    ...
    gam_grid1 = h2o.grid("gam", grid_id="GAMModel1", x=xL, y=yR, training_frame=trainGaussian, family='gaussian',
                         hyper_params=hyper_parameters, search_criteria=search_criteria, seed=1)
    gam_grid2 = h2o.grid("gam", grid_id="GAMModel2", x=xL, y=yR, training_frame=trainGaussian, family='gaussian',
                         hyper_params=hyper_parameters2, search_criteria=search_criteria, seed=1)
...
doTest("General Additive Model test dual model specification with Gaussian family for cartesian gridsearch with I-spline", test.model.gam.random.gridsearch.dual.modes)
