...
test.threshold <- function() {
    ...
    print("Get performance with the new threshold")
    reseted_model = h2o.getModel(prostate_glm@model_id)
    perf_new = h2o.performance(reseted_model, prostate)
    print(perf_new)

    expect_equal(old_threshold, perf_old@metrics$threshold)
    expect_equal(new_threshold, perf_new@metrics$threshold)
...
doTest("Test reset model threshold", test.threshold)
