...
glm_control_variables_multinomial <- function() {
    ...
    prostate_glm <- h2o.glm(family = "ordinal",
                            y = response,
                            training_frame = df,
    ...
    expect_true(varimp[varimp$variable == "C2.0", 2] == 0)
    expect_true(varimp[varimp$variable == "C2.1", 2] == 0)
    expect_true(varimp[varimp$variable == "C2.2", 2] == 0)

    # in unrestricted model control variables can have larger importance than zero
    expect_true(varimp_unrestricted[varimp_unrestricted$variable == "C2.0", 2] >= 0)
