...
test.glm.interactions7 <- function() {
  ...
  df <- as.h2o(df)
  h2o_glm1 <- h2o.glm(y="Petal.Width", x=c("Species","Species_Sepal.Length.versicolor", "Species_Sepal.Length.virginica", "Sepal.Length","Sepal.Width", "Petal.Length"), training_frame=df, standardize=FALSE)

  h2o_glm2 <- h2o.glm(y="Petal.Width", x=c("Species","Sepal.Length","Sepal.Width", "Petal.Length"), interactions=c("Species", "Sepal.Length"), standardize=FALSE, training_frame=as.h2o(iris))
...
doTest("Testing model accessors for GLM", test.glm.interactions7)
