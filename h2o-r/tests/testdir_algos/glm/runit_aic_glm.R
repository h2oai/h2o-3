...
.assert_equal <- function(h2o_model, r_model, coef_tolerance = 1e-6, aic_tolerance = 1e-6, dk_params = 0, loglik_tolerance = 1e-6, r_llh) {
    ...
    if (missing(r_llh)) {
        ...
        # Calculate AIC from R model
        if (inherits(r_model, "glm")) {
            r_aic <- AIC(r_model)
        } else {
            # For tweedie models
...
test_glm_aic_gaussian_no_regularization <- function() {
    ...
    .assert_equal(glm_no_reg, r_glm_no_reg, dk_params = 1, aic_tolerance = 0.2)
    ...
    .assert_equal(glm_no_reg, r_glm_no_reg, dk_params = 1, aic_tolerance = 1e-5)
}
...
test_glm_aic_negative_binomial_no_regularization <- function() {
    ...
    theta <- glm_no_reg_theta_est@model$output$dispersion

    # Note: R's MASS::glm.nb can be used for negative binomial
    library(MASS)
    r_glm_no_reg <- glm.nb(GLEASON ~ ., data = train_df)

    .assert_equal(glm_no_reg_theta_est, r_glm_no_reg,
        coef_tolerance = 1e-2, aic_tolerance = 2e-2, dk_params = 1
    )
}
...
test_glm_aic_tweedie_no_regularization <- function() {
    ...
    .assert_equal(glm_no_reg, r_glm_no_reg, coef_tolerance = 1e-3, aic_tolerance = 1e-4)
}
