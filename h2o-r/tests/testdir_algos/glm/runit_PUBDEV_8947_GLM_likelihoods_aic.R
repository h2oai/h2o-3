...
test.glm.aic.likelihood <- function() {
 ...
 # browser()
  ...
  model.h2o.gamma.inverse <- h2o.glm(calc_like=TRUE, x=myX, y=myY, training_frame=h2o.data, family="gamma", link="inverse",alpha=0.5, lambda=0, nfolds=0)
  model.R.gamma.inverse <- glm(formula=R.formula, data=R.data[,c(1:5,7:9)], family=Gamma(link=inverse), na.action=na.omit)
  perf <- h2o.performance(model.h2o.gamma.inverse)
  print("GLM Gamma")
  print("H2O AIC")
  print(h2o.aic(perf))
  print("H2O log likelihood")
  ...
  print("R loglikelihood")
  print(logLik(model.R.gamma.inverse))
  
  # checking for binomial
  myY <- "CAPSULE"
  myX <- c("AGE","RACE","DCAPS","PSA","VOL","DPROS","GLEASON")
  R.formula <- (R.data[,"CAPSULE"]~.)
  model.h2o.binomial.logit <- h2o.glm(calc_like=TRUE, x=myX, y=myY, training_frame=h2o.data, family="binomial", link="logit",alpha=0.5, lambda=0, nfolds=0)
  model.R.binomial.logit <- glm(formula=R.formula, data=R.data[,4:10], family=binomial(link=logit), na.action=na.omit)
  perf <- h2o.performance(model.h2o.binomial.logit)
  print("GLM binomial")
  print("H2O AIC")
  print(h2o.aic(perf))
  print("H2O log likelihood")
  ...
  myX = c("ID","AGE","RACE","CAPSULE","DCAPS","PSA","VOL","DPROS")
  R.formula = (R.data[,"GLEASON"]~.) 
  model.h2o.poisson.log <- h2o.glm(calc_like=TRUE, x=myX, y=myY, training_frame=h2o.data, family="poisson", link="log",alpha=0.5, lambda=0, nfolds=0)
  model.R.poisson.log <- glm(formula=R.formula, data=R.data[,2:9], family=poisson(link=log), na.action=na.omit)
  perf <- h2o.performance(model.h2o.poisson.log)
  print("GLM Poisson")
  print("H2O AIC")
  print(h2o.aic(perf))
  print("H2O log likelihood")
