...
.expand_factors <- function(df){
  ...
  stopifnot("only factors are expected to be non numeric" = non_numeric_columns==factors)
...
.predict_helper <- function(df, coeffs, link_function, control_variables){
  temp_result <- rep_len((if ("Intercept" %in% names(coeffs)) coeffs[["Intercept"]] else 0.0), nrow(df))
...
.almost_equal <- function(expected, actual){
  ...
  if (max(diff) > 1e-10) {
    message(sprintf("Maximum difference %f, average difference %f, median difference %f.", max(diff), mean(diff), median(diff)))
  }
  max(diff) < 1e-8
}

.predict_with_and_without_control_variables <- function(y, training_frame, test_frame, control_variables, other_args=list(), seed=42) {
  ...
  link_fns <- list(
    "identity" = identity,
    "logit" = function(eta) ifelse(eta < 0, 1/(1+exp(-eta)), exp(eta)/(1+exp(eta))),
    "log" = function(eta) exp(eta),
    "inverse" = function(eta) 1/eta,
    "tweedie" = function(eta) {
      var_power <- if ("tweedie_variance_power" %in% names(other_args)) other_args[["tweedie_variance_power"]] else 0.0
      # documentation says link power should be 1 - var_power but it doesn't seem to be automatically set so using 1 as default
      link_power <- if ("tweedie_link_power" %in% names(other_args)) other_args[["tweedie_link_power"]] else 1
      if (link_power != 0){
        eta^(1/link_power)
      } else {
        exp(eta)
  ...
  model <- do.call(h2o.glm, c(list(y=y, training_frame=training_frame, seed=seed), other_args))
  ...
  link <- if ("link" %in% names(other_args)) other_args[["link"]] else "family_default"
  
  if (link == "family_default") {
    family <- if ("family" %in% names(other_args)) other_args[["family"]] else "AUTO"
    if (family == "AUTO")
      family <- if (is.factor(training_frame[[y]])) "binomial" else "gaussian"
    link <- links_for_families[[family]]
  }
  
  link_function <- link_fns[[link]]
  
  without_control_variables <- .predict_helper(df, coeffs, link_function, NULL)
  ...
  stopifnot("Predictions without control variables don't match"=.almost_equal(h2o_res[, ncol(h2o_res)], without_control_variables))
  
  with_control_variables <- .predict_helper(df, coeffs, link_function, control_variables = control_variables)
  
  list(predictions = with_control_variables, unrestricted_predictions = without_control_variables)
}


verify_control_variables_implementation <- function(y, training_frame, test_frame, control_variables, other_args=list(), seed=42){
  preds <- .predict_with_and_without_control_variables(y=y, training_frame = training_frame, test_frame=test_frame, control_variables=control_variables, other_args=other_args, seed=seed)
  
  model_constr <- do.call(h2o.glm, c(list(y=y, training_frame=training_frame, seed=seed, control_variables=control_variables), other_args))
...
#   print(summary(diff))
#   stopifnot("Predictions with control variables don't match"=.almost_equal(h2o_constr_res[, ncol(h2o_constr_res)], preds$predictions))
...
.sanity_check <- function(train_df, test_df, continuous_response, positive_continuous_response, whole_positive_number_response, binary_response, control_variables) {
  for (standardize in c(TRUE, FALSE)) {
    ...
    verify_control_variables_implementation(
      ...
      other_args = list(family = "gaussian", standardize = standardize)
    )
    ...
    for (fam in pos_continuous_families){
      ...
      verify_control_variables_implementation(
        ...
        test_frame = test_df,
        control_variables = control_variables,
        other_args = list(family = fam, standardize = standardize)
        )
    }
    
    for (lp in c(0.0, 0.5, 1, 1.5)){
      cat("tweedie_link_power=", lp, "\n")
      verify_control_variables_implementation(
        y = positive_continuous_response, 
        training_frame = train_df,
        test_frame = test_df,
        control_variables = control_variables,
        other_args = list(family="tweedie", tweedie_link_power = lp, standardize = standardize)
      )
    }
    
    for (tvp in c(0.0, 0.5, 1, 1.5, 2)){
      cat("tweedie_variance_power=", tvp, "\n")
      verify_control_variables_implementation(
        y = positive_continuous_response, 
        training_frame = train_df,
        test_frame = test_df,
        control_variables = control_variables,
        other_args = list(family="tweedie", tweedie_variance_power = tvp, standardize = standardize)
      )
    }
    
    for (tvp in c(0.0, 0.5, 1, 1.5, 2)){
      for (lp in c(0.0, 0.5, 1, 1.5)){
        cat("tweedie_variance_power=", tvp, ";\ttweedie_link_power=", lp, "\n")
        verify_control_variables_implementation(
          y = positive_continuous_response, 
          training_frame = train_df,
          test_frame = test_df,
          control_variables = control_variables,
          other_args = list(family="tweedie", tweedie_variance_power = tvp, tweedie_link_power = lp, standardize = standardize)
        )
      }
    }
    ...
    verify_control_variables_implementation(
      ...
      test_frame = test_df,
      control_variables = control_variables,
      other_args = list(standardize = standardize)
    )
    ...
    verify_control_variables_implementation(
      ...
      test_frame = test_df,
      control_variables = control_variables,
      other_args = list(family="poisson", standardize = standardize)
    )
    ...
    verify_control_variables_implementation(
      ...
      test_frame = test_df,
      control_variables = control_variables,
      other_args = list(family="negativebinomial", standardize = standardize)
    )
  }
}
...
test.control_variables_work_correctly <- function(){
    ...
    .sanity_check(
      ...
      control_variables=c("Petal.Width", "Petal.Length", "Species")
    )
}
