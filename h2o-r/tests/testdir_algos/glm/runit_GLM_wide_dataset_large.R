...
test <- function(conn) {
    print("Reading in Arcene training data for binomial modeling.")
        arcene.train = h2o.uploadFile(conn, locate("smalldata/arcene/arcene_train.data"), destination_frame="arcene.train")
        arcene.label = h2o.uploadFile(conn, locate("smalldata/arcene/arcene_train_labels.labels"), destination_frame="arcene.label")
    ...
    print("Reading in Arcene validation data.")
        arcene.valid = h2o.uploadFile(conn, locate("smalldata/arcene/arcene_valid.data"), destination_frame="arcene.valid", header=FALSE)
        arcene.label = h2o.uploadFile(conn, locate("smalldata/arcene/arcene_valid_labels.labels"), destination_frame="arcene.label", header=FALSE)
    ...
    print("Run model on 3250 columns of Arcene with strong rules off.")
            time.noSR.3250 <- system.time(model.noSR.3250 <- h2o.glm(x=c(1:3250), y="arcene.train.label", training_frame=arcene.train.full, family="binomial", lambda_search=FALSE, alpha=1, nfolds=0, use_all_factor_levels=TRUE))
