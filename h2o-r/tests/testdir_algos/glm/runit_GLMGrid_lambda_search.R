...
test.GLMGrid.lambda.search <- function(conn) {
  ...
  prostate.bestlambda = h2o.glm(x = 3:9, y = 2, data = prostate.hex, family = "binomial", alpha = c(0.25, 0.5), nlambda = 5, lambda_search = TRUE, nfolds = 2)
  ...
  prostate.search = h2o.glm(x = 3:9, y = 2, data = prostate.hex, family = "binomial", alpha = c(0.25, 0.5), nlambda = 5, lambda_search = TRUE, return_all_lambda = TRUE, nfolds = 2)
