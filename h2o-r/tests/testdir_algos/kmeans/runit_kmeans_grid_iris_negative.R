...
check.kmeans.grid.cars.negative <- function(conn) {
  ...
  cars_kmeans_grid <- h2o.grid("kmeans", grid_id="kmeans_grid_iris_test", x=1:4, k=3, training_frame=iris, hyper_params=grid_space)
  print(cars_kmeans_grid)
  ...
  expect_equal(length(cars_kmeans_grid@model_ids), 4)
  ...
  Log.info(paste0("Constructing the grid of kmeans models with non-gridable parameter build_tree_one_node"))
  expect_error(cars_kmeans_grid <- h2o.grid("kmeans", grid_id="kmeans_grid_iris_test", x=1:4, k=3, training_frame=iris, hyper_params=grid_space))

  testEnd()
}

doTest("K-Means Grid Search using bad parameters", check.kmeans.grid.cars.negative)
