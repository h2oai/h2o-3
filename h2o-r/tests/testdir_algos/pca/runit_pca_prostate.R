...
test.pca.prostate <- function(conn) {
  ...
  Log.info("PCA on columns 3 to 9 with k = 3, transform = 'STANDARDIZE', method = 'Power'")
  fitPCA <- h2o.prcomp(training_frame = prostate.hex, x = 3:9, k = 3, transform = "STANDARDIZE", method = "Power")
