...
test.grid.resume <- function() {
    ...
    baseline_grid <- h2o.grid("extendedisolationforest",
                              grid_id = "extendedisofor_grid_test",
                              x = 1:4,
                              y = 5,
    ...
    # Grid search with validation frame
    train <- h2o.importFile(locate("smalldata/anomaly/ecg_discord_train.csv"))
    test <- h2o.importFile(locate("smalldata/anomaly/ecg_discord_test.csv"))

    model <- h2o.extendedIsolationForest(training_frame = train, 
                                         seed = 1234,
                                         ntrees = 10,
                                         sample_size = 10)
    ...
    validated_grid <- h2o.grid("extendedisolationforest",
                               grid_id = "extendedisofor_grid_test_validation",
                               x = 1:4,
                               y = 5,
