...
test.h.input.validation <- function() {
  ...
  tryCatch({
    h2o.h(gbm.model, prostate.hex, NULL)
  ...
  tryCatch({
    h2o.h(gbm.model, prostate.hex, c())
  ...
  tryCatch({
    h2o.h(gbm.model, prostate.hex, c('AGE', 'NONEXISTENT_COLUMN'))
  ...
  tryCatch({
    h2o.h(gbm.model, prostate.hex, c('AGE', 'GLEASON'))
  ...
  h_val <- h2o.h(gbm.model, prostate.hex, c('AGE', 'PSA'))
  Log.info(paste("H statistic value:", h_val))
  expect_true(h_val >= 0.0, info = "H statistic should be non-negative")
  
  Log.info("All input validation tests passed for GBM!")
}

doTest("H statistic input validation test", test.h.input.validation)
