...
test.gbm.effective_parameters <- function() {
    ...
    model1 <- h2o.gbm(seed=1234, stopping_rounds=3, x=predictors,y=response_col,training_frame=train,validation_frame=valid)
    model2 <- h2o.gbm(seed=1234, stopping_rounds=3, x=predictors,y=response_col,training_frame=train,validation_frame=valid, distribution="bernoulli", stopping_metric="logloss", histogram_type="UniformAdaptive",
