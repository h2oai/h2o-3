...
check.gbm.grid.cars.negative <- function(conn) {
  ...
  if ( non_gridable_parameter == 2 ) { grid_space$seed <- c(1, 100, 12345) }

  Log.info("Constructing the grid of gbm models with some non-gridable parameters. Expecting failure...")
  if ( validation_scheme == 1 ) {
