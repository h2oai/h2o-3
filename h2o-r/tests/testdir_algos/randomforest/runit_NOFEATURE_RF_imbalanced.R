...
test.rf.imbalanced <- function(conn) {
  ...
  hh_imbalanced=h2o.randomForest(x=c1:54,y=55,ntrees=50,training_frame=covtype,
                                 balance_classes=F,nfolds=10)
  print(hh_imbalanced)
  hh_balanced=h2o.randomForest(x=c1:54,y=55,ntrees=50,training_frame=covtype,
                               balance_classes=T,nfolds=10)
  print(hh_balanced)
