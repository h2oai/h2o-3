...
test.RF.checkpoint_on_iris <- function() {
  ...
  ##
  # Build an initial RF model
  iris.rf.initial <- h2o.randomForest(y = 5, x = 1:4, training_frame = train.hex,
                               validation_frame = test.hex, seed = 42,
                               ntrees = ntrees.initial)
  # Continue building the initial model by appending more trees
  iris.rf.cont <- h2o.randomForest(y = 5, x = 1:4, training_frame = train.hex,
                               validation_frame = test.hex, seed = 42,
                               checkpoint = iris.rf.initial@parameters$model_id,
                               ntrees = ntrees.cont)
  # Build the same model from the scratch
  iris.rf.total <- h2o.randomForest(y = 5, x = 1:4, training_frame = train.hex,
                                 validation_frame = test.hex, seed = 42,
                                 ntrees = ntrees.total)

  # Verify that model built from checkpoint and model built from scratch are same
  expect_equal(iris.rf.cont@parameters$ntrees, iris.rf.total@parameters$ntrees)
  ...
  a <- iris.rf.cont@model$traininig_metrics
  b <- iris.rf.total@model$training_metrics
  # FIXME PLEASE PLEASE PLEASE expect_mm_equal(a, b)

  # Validation metrics should be same
  a <- iris.rf.cont@model$validation_metrics
...
h2oTest.doTest("RF test checkpoint on iris", test.RF.checkpoint_on_iris)
