...
##
# Copy example from HGLM package and compare results from our implementation.  This test should be 
# enabled after we implemented the version using hlikelihood.
##

test.HGLMData1 <- function() {
  tol = 2e-2
  h2odata <- h2o.importFile(locate("smalldata/hglm_test/semiconductor.csv"))
  h2odata$Device <- h2o.asfactor(h2odata$Device)
  yresp <- "y"
  xlist <- c("x1", "x3", "x5", "x6")
  z <- c(1)
  hglm_model <- h2o.hglm(x=xlist, y=yresp, training_frame=h2odata, random_columns=xlist, group_column="Device")
