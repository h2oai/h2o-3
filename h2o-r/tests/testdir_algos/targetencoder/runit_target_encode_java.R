...
test.targetencoder <- function() {
    ...
    # Test that target_encode_fit is also accepting te column as a string(not array with single element)
    test.that.target_encode_fit.is.also.accepting.te.column.as.a.string <- function() {
        data <- getTitanicData()
        column <- "embarked"
        te_cols <- list(column)
    ...
    test.holdout_type.validation <- function() {
        data <- getTitanicData()
        column <- "embarked"
        te_cols <- list(column)
        encoding_map <- h2o.target_encode_fit(data, te_cols, "survived")
    ...
    test.indexes.to.names.conversion <- function() {
        data <- getTitanicData()
    ...
    test.warning.will.be.shown.when.noise.is.used.with.none.strategy <- function() {
    ...
    test.error.is.being.thrown.when.encoding.map.does.not.contain.fold.column.but.there.is.an.attempt.to.apply.holdout_type.kfold <- function() {
    ...
    test.kfold.column.name.is.provided.for.holdout_type.kfold <- function() {
    ...
    test.that.setting.blending.parameters.to.values.outside.of.the.valid.range.will.throw.errors <- function() {
    ...
    test.that.using.default.values.of.optional.parameters.does.not.lead.to.errors <- function() {
    ...
    makeSuite(
        test,
        test.that.target_encode_fit.is.also.accepting.te.column.as.a.string,
        test.holdout_type.validation,
        test.indexes.to.names.conversion,
        test.noise.parameter,
        test.warning.will.be.shown.when.noise.is.used.with.none.strategy,
        test.error.is.being.thrown.when.encoding.map.does.not.contain.fold.column.but.there.is.an.attempt.to.apply.holdout_type.kfold,
        test.kfold.column.name.is.provided.for.holdout_type.kfold,
        test.that.setting.blending.parameters.to.values.outside.of.the.valid.range.will.throw.errors,
        test.that.using.default.values.of.optional.parameters.does.not.lead.to.errors,
    )
}

doSuite("Test target encoding exposed from Java", test.targetencoder(), time_monitor=TRUE)
