...
test.glrm.prostate_miss <- function() {
  ...
  h2oTest.logInfo("Importing prostate_cat.csv data and saving for validation...")
  prostate.full <- h2o.uploadFile(h2oTest.locate("smalldata/prostate/prostate_cat.csv"), destination_frame= "prostate.hex", na.strings = rep("NA", 8))
  ...
  for(i in 1:length(missing_frac)) {
    ...
    h2oTest.logInfo(paste("Copying data and inserting ", 100 * f, "% missing entries:\n", sep = ""))
    ...
    h2oTest.logInfo(paste("H2O GLRM with ", 100 * f, "% missing entries:\n", sep = ""))
    ...
    h2oTest.checkGLRMPredErr(prostate.glrm, prostate.miss, prostate.full, tolerance = 1e-5)
...
h2oTest.doTest("GLRM Test: Prostate Data with Missing Values Inserted", test.glrm.prostate_miss)
