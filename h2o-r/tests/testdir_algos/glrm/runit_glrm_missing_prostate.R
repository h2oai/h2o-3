...
test.glrm.prostate_miss <- function(conn) {
  ...
  Log.info("Importing prostate.csv data and saving for validation...")
  ...
  for(i in 1:length(missing_frac)) {
    ...
    prostate.glrm <- h2o.glrm(training_frame = prostate.miss, validation_frame = prostate.full, k = 8, init = "SVD", max_iterations = 2000, 
                              gamma_x = 0.5, gamma_y = 0.5, regularization_x = "L1", regularization_y = "L1", min_step_size = 1e-6, seed = SEED)
