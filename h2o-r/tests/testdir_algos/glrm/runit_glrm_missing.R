...
test.glrm.missing <- function(conn) {
  ...
  for(i in 1:length(missing_frac)) {
    ...
    arrests.glrm <- h2o.glrm(training_frame = arrests.hex, k = 4, gamma = 0, init = "PlusPlus", seed = seed)
    obj_val[i] <- arrests.glrm@model$objective
    iters[i] <- arrests.glrm@model$iterations
  }
  print(data.frame(Fraction = missing_frac, Objective = obj_val, Iterations = iters))
  testEnd()
}
