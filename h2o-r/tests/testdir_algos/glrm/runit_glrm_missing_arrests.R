...
test.glrm.arrests_miss <- function() {
  ...
  h2oTest.logInfo("Importing USArrests.csv data and saving for validation...\n")
  arrests.full <- h2o.uploadFile(h2oTest.locate("smalldata/pca_test/USArrests.csv"))
  ...
  for(i in 1:length(missing_frac)) {
    ...
    h2oTest.checkGLRMPredErr(arrests.glrm, arrests.miss, arrests.full, tolerance = 1e-6)
...
h2oTest.doTest("GLRM Test: USArrests Data with Missing Entries Inserted", test.glrm.arrests_miss)
