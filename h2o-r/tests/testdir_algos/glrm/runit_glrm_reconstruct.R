...
test.glrm.reconstruct <- function() {
  ...
  for(t in c("NONE", "DEMEAN", "DESCALE", "STANDARDIZE")) {
    rank <- sample(1:7, 1)
    h2oTest.logInfo(paste("H2O GLRM with rank k = ", rank, ", transform = '", t, "', and impute_original = TRUE", sep = ""))
...
h2oTest.doTest("GLRM Test: Reconstruction of Original and Standardized Iris", test.glrm.reconstruct)
