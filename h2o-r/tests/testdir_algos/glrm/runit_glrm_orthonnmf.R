...
test.glrm.orthonnmf <- function() {
  ...
  h2oTest.logInfo("Run GLRM with orthogonal non-negative regularization on X, non-negative regularization on Y")
  ...
  h2oTest.logInfo("Check that X and Y matrices are non-negative")
  fitX.mat <- as.matrix(fitX)
  expect_true(all(fitY >= 0))
  expect_true(all(fitX.mat >= 0))
  
  h2oTest.logInfo("Check that columns of X are orthogonal")
  ...
  h2oTest.logInfo("Run GLRM with orthogonal non-negative regularization on both X and Y")
  ...
  h2oTest.logInfo("Check that rows of Y are orthogonal")
  YYt <- fitY %*% t(fitY)
...
h2oTest.doTest("GLRM Test: Orthogonal Non-negative Matrix Factorization", test.glrm.orthonnmf)
