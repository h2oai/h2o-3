...
testMaxRGLM <- function() {
  ...
  bestPredictorNames <- h2o.get_best_model_predictors(maxrglm_model)
  ...
  expect_true(abs(max(bestR2)-bestR2Value[2]) < 1e-6)
  print("Model wth best R2 value have predictors:")
  print(bestPredictorNames[[2]])
}

doTest("MaxRGLM: Gaussian data", testMaxRGLM)
