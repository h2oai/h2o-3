...
check.deeplearning_imbalanced <- function() {
  ...
  checkTrue(nrow(biases3) == 1, "wrong dimensionality!")
    ...
    df <- as.h2o(iris)
    dl1 <- h2o.deeplearning(1:4,5,df,hidden=c(10,10),export_weights_and_biases = TRUE)
    ...
    dl3 <- h2o.deeplearning(1:4,5,training_frame=df,hidden=c(10,10),initial_weights=list(w1,NULL,w3),initial_biases=list(b1,b2,NULL), epochs=10)
    ll3 <- h2o.logloss(h2o.performance(dl3,df))
    checkTrue(ll3 < ll1)

    ## make another model with partially set user-modified weights/biases
    dl4 <- h2o.deeplearning(1:4,5,training_frame=df,hidden=c(10,10),initial_weights=list(w1*1.1,w2*0.9,sqrt(w3)),initial_biases=list(b1,b2,NULL), epochs=10)
