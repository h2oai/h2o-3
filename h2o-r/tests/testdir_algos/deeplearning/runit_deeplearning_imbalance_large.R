...
check.deeplearning_imbalanced <- function(conn) {
  ...
  hh_imbalanced<-h2o.deeplearning(x=c(1:54),y=55,l1=1e-5,activation="Rectifier",loss="CrossEntropy",hidden=c(200,200),epochs=1,training_frame=covtype,balance_classes=F, do_classification = TRUE, reproducible=T, seed=1234)
  print(hh_imbalanced)
  hh_balanced<-h2o.deeplearning(x=c(1:54),y=55,l1=1e-5,activation="Rectifier",loss="CrossEntropy",hidden=c(200,200),epochs=1,training_frame=covtype,balance_classes=T, do_classification = TRUE, reproducible=T, seed=1234)
  print(hh_balanced)
