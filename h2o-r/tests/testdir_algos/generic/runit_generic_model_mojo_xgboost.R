...
test.model.generic.gbm <- function() {
    ...
    original_model <- h2o.xgboost(x=cols, y = "IsDepDelayed", training_frame = data, validation_frame = data, nfolds = 3, ntrees = 1)
    ...
    original_model <- h2o.xgboost(x=cols, y = "Distance", training_frame = data, validation_frame = data, nfolds = 3, ntrees = 1)
    print(original_model)
    mojo_original_name <- h2o.download_mojo(model = original_model, path = tempdir())
    mojo_original_path <- paste0(tempdir(),"/",mojo_original_name)
    ...
    # Multinomial
    cols <- c("Origin", "Distance")
    original_model <- h2o.xgboost(x=cols, y = "Dest", training_frame = data, validation_frame = data, nfolds = 3, ntrees = 1)
    print(original_model)
    mojo_original_name <- h2o.download_mojo(model = original_model, path = tempdir())
    mojo_original_path <- paste0(tempdir(),"/",mojo_original_name)
