...
compare_output <- function(original_output, generic_output){
    # Remove the difference in metrics name - generic metrics have different naming
    original_output <- gsub("H2OBinomialMetrics: gbm", "", original_output)
    generic_output <- gsub("H2OBinomialMetrics: generic", "", generic_output)
    
    removed_original <- c()
    for (i in 1:length(original_output)) {
        line <- original_output[i]
        
        if(grepl("Extract .+ frame", line)){ # There are no frames to extract in generic
            removed_original <- append(removed_original, i)
        } else if(grepl("H2OBinomialModel: gbm", line)){
            removed_original <- append(removed_original, i)
        } else if(grepl("Model ID", line)){
            removed_original <- append(removed_original, i)
        } else if(grepl("H2OBinomialMetrics: gbm", line)){
            removed_original <- append(removed_original, i)
        }
    }
    ...
    for (i in 1:length(generic_output)) {
        ...
        if(grepl("H2OBinomialModel: generic", line)){
            removed_generic <- append(removed_generic, i)
        } else if(grepl("Model ID", line)){
            removed_generic <- append(removed_generic, i)
        } else if(grepl("H2OBinomialMetrics: generic", line)){
            removed_generic <- append(removed_generic, i)
        }
    }
