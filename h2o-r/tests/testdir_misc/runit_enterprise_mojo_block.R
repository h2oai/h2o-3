...
# The H2O-3 Secure paywall (marketing demo) blocks getting a model *out* of
...
test.enterprise_mojo_block <- function() {
  ...
  expect_blocked <- function(expr, op_text) {
    ...
    expect_true(grepl("requires H2O-3 Secure", msg, fixed = TRUE))
...
doTest("Secure: MOJO export/import/upload blocked in the OSS R client",
