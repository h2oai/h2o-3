...
test.separator <- function(){
  ...
  tab.test = h2o.importFile(path = normalizePath(h2oTest.locate(path.tab)), destination_frame = "tab.hex",sep = "\t")
  expect_that(dim(tab.test), equals(c(3,3)))
  #Tab delimited with no argument
  tab.test.noarg = h2o.importFile(path = normalizePath(h2oTest.locate(path.tab)), destination_frame = "tab.hex",sep = "")
  expect_that(dim(tab.test.noarg), equals(c(3,3)))
  #--------------------------------------------------------------------------------------------------------------------
  #Pipe delimited with argument
  pipe.test = h2o.importFile(path = normalizePath(h2oTest.locate(path.pipe)), destination_frame = "pipe.hex",sep = "|")
  expect_that(dim(pipe.test), equals(c(3,3)))
  #Pipe delimited with no argument
  pipe.test.noarg = h2o.importFile(path = normalizePath(h2oTest.locate(path.pipe)), destination_frame = "pipe.hex",sep = "")
  expect_that(dim(pipe.test.noarg), equals(c(3,3)))
  #--------------------------------------------------------------------------------------------------------------------
  #Hive delimited with argument
  hive.test = h2o.importFile(path = normalizePath(h2oTest.locate(path.hive)), destination_frame = "hive.hex",sep = "\001")
  expect_that(dim(hive.test), equals(c(3,3)))
  #Hive delimited wih no argument
  hive.test.noarg = h2o.importFile(path = normalizePath(h2oTest.locate(path.hive)), destination_frame = "hive.hex",sep = "")
  expect_that(dim(hive.test.noarg), equals(c(3,3)))
  #--------------------------------------------------------------------------------------------------------------------
  #Semi colon delimited with argument
  semi.test = h2o.importFile(path = normalizePath(h2oTest.locate(path.semi)), destination_frame = "semi.hex",sep = ";")
  expect_that(dim(semi.test), equals(c(3,3)))
  #Semi colon delimited with no argument
  semi.test.noarg = h2o.importFile(path = normalizePath(h2oTest.locate(path.semi)), destination_frame = "semi.hex",sep = "")
  expect_that(dim(semi.test.noarg), equals(c(3,3)))
  #--------------------------------------------------------------------------------------------------------------------
  #Caret delimited with argument
  caret.test = h2o.importFile(path = normalizePath(h2oTest.locate(path.caret)), destination_frame = "caret.hex",sep = "^")
  ...
  #caret.test.noarg = h2o.importFile(path = normalizePath(h2oTest.locate(path.caret)), destination_frame = "caret.hex",sep = "")
  #expect_that(dim(caret.test.noarg), equals(c(3,3)))
  #print(caret.test.noarg)
  #--------------------------------------------------------------------------------------------------------------------
  #Asterisk delimited with argument
  asterisk.test = h2o.importFile(path = normalizePath(h2oTest.locate(path.asterisk)), destination_frame = "asterisk.hex",sep = "*")
  ...
  #asterisk.test.noarg = h2o.importFile(path = normalizePath(h2oTest.locate(path.asterisk)), destination_frame = "asterisk.hex",sep = "")
  #expect_that(dim(asterisk.test.noarg), equals(c(3,3)))
  #print(asterisk.test.noarg)
}

h2oTest.doTest("Separator Test", test.separator)
