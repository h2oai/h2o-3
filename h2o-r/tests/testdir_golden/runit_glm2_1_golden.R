...
cuseH2O <- h2o.uploadFile(path = h2oTest.locate("smalldata/logreg/cuseexpanded.csv"), destination_frame="cuseH2O")
cuseR <- read.csv(h2oTest.locate("smalldata/logreg/cuseexpanded.csv"), header=T)

h2oTest.logInfo("Test H2O treatment of FACTORS AS PREDICTORS")
...
h2oTest.doTest("GLM Test: Golden GLM2 - Factors as predictors", test.glm2binregression.golden)
