...
err <- as.data.frame(fith2o@model$scoring_history$"Training MSE")
REPMSE <- err[4,]
...
Log.info(paste("Length of H2O MSE Vec: ", length(fith2o@model$scoring_history$"Training MSE"),      "\t\t", "Expected Length   : ", 4))
...
expect_equal(length(fith2o@model$scoring_history$"Training MSE"), 4) # 3 errs per for each subforest + one error for empty forest.
