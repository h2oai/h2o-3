...
# mojo implementation when standardize is set to FALSE.  A random neural network is generated and trained
...
test.deeplearning.mojo <- function() {
    #----------------------------------------------------------------------
    # Run the test
    #----------------------------------------------------------------------
    e <- tryCatch({
        ...
        modelAndDir<-buildModelSaveMojo(params) # build the model and save mojo

        filename = sprintf("%s/in.csv", modelAndDir$dirName) # save the test dataset into a in.csv file.
        h2o.downloadCSV(test_frame[,params$x], filename)
