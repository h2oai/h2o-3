...
h2oTest.compareCats <- function(pred, actual) {
...
h2oTest.transformData <- function(df, transform = "NONE") {
...
h2oTest.checkError <- function(dataR, imputeR, metricsH2O, transform = "NONE", impute_original = FALSE, tolerance = 1e-6) {
  NUM <- function(x) { x[,sapply(x, is.numeric)] }
  FAC <- function(x) { x[,sapply(x, is.factor)]  }
  
  # Transform data in R to match what H2O processed and compute errors
  dataR_trans <- h2oTest.transformData(dataR, transform = ifelse(impute_original, "NONE", transform))
  numerrR <- sum((NUM(dataR_trans) - NUM(imputeR))^2, na.rm = TRUE)
  caterrR <- h2oTest.compareCats(FAC(imputeR), FAC(dataR_trans))
...
h2oTest.checkTrainErr <- function(fitH2O, trainR, imputeR, tolerance = 1e-6) {
  h2oTest.checkError(trainR, imputeR, fitH2O@model$training_metrics@metrics, fitH2O@allparameters$transform, fitH2O@allparameters$impute_original, tolerance)
}

h2oTest.checkValidErr <- function(fitH2O, testR, imputeR, tolerance = 1e-6) {
  h2oTest.checkError(testR, imputeR, fitH2O@model$validation_metrics@metrics, fitH2O@allparameters$transform, fitH2O@allparameters$impute_original, tolerance)
}

h2oTest.checkGLRMPredErr <- function(fitH2O, trainH2O, validH2O = NULL, tolerance = 1e-6) {
  ...
  h2oTest.checkTrainErr(fitH2O, trainR, pred.df, tolerance)
  if(!is.null(validH2O) && !is.null(fitH2O@model$validation_metrics)) {
    validR <- as.data.frame(validH2O)
    h2oTest.checkValidErr(fitH2O, validR, pred.df, tolerance)
  }
  return(pred)
}
