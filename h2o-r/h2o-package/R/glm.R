...
#' @param id (Optional) The unique id assigned to the resulting model. If none is given, an id will automatically be generated.
...
#' }
#' @export
h2o.glm <- function(x, y, training_frame, id, validation_frame,
                    max_iterations = 50,
                    beta_epsilon = 0,
...
h2o.makeGLMModel <- function(model,beta) {
   ...
   res = .h2o.__remoteSend(model@conn, method="POST", .h2o.__GLMMakeModel, id=model@id, names = paste("[",paste(paste("\"",names(beta),"\"",sep=""), collapse=","),"]",sep=""), beta = paste("[",paste(as.vector(beta),collapse=","),"]",sep=""))
   m <- h2o.getModel(id=res$id$name)
   m@model$coefficients <- m@model$coefficients_table[,2]
   names(m@model$coefficients) <- m@model$coefficients_table[,1]
   m
}
...
#' @export
h2o.startGLMJob <- function(x, y, training_frame, id, validation_frame,
