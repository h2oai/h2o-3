...
#' @param max_mem_size (Optional) A \code{character} string specifying the maximum size, in bytes, of the memory allocation pool to H2O. This value must a multiple of 1024 greater than 2MB. Append the letter m or M to indicate megabytes, or g or G to indicate gigabytes.  This value is only used when R starts H2O. **Note:** If ``max_mem_size`` is not defined, then the amount of memory that H2O allocates will be determined by the default memory of Java Virtual Machine (JVM). This amount is dependent on the Java version, but it will generally be 25% of the machine's physical memory.
#' @param min_mem_size (Optional) A \code{character} string specifying the minimum size, in bytes, of the memory allocation pool to H2O. This value must a multiple of 1024 greater than 2MB. Append the letter m or M to indicate megabytes, or g or G to indicate gigabytes.  This value is only used when R starts H2O. **Note:** If ``min_mem_size`` is not defined, then the amount of memory that H2O allocates will be determined by the default memory of Java Virtual Machine (JVM). This amount is dependent on the Java version, but it will generally be 25% of the machine's physical memory.
