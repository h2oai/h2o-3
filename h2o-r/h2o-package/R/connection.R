...
h2o.init <- function(ip = "127.0.0.1", port = 54321, startH2O = TRUE, forceDL = FALSE, Xmx,
  ...
  if(!is.character(ip) || length(ip) != 1L || is.na(ip) || !nzchar(ip))                         stop("`ip` must be a non-empty character string")
  if(!is.numeric(port) || length(port) != 1L || is.na(port) || port < 0 || port > 65536)        stop("`port` must be an integer ranging from 0 to 65536")
  if(!is.logical(startH2O) || length(startH2O) != 1L || is.na(startH2O))                        stop("`startH2O` must be TRUE or FALSE")
  if(!is.logical(forceDL) || length(forceDL) != 1L || is.na(forceDL))                           stop("`forceDL` must be TRUE or FALSE")
  if(!missing(Xmx) && !(is.character(Xmx) && length(Xmx) == 1L && !is.na(Xmx) && nzchar(Xmx)))  stop("`Xmx` must be missing or a non-empty character string")
  if(!missing(Xmx) && !regexpr("^[1-9][0-9]*[gGmM]$", Xmx))                                     stop("`Xmx` option must be like 1g or 1024m")
  if(!is.numeric(nthreads) || length(nthreads) != 1L || is.na(nthreads) || nthreads < -2)       stop("`nthreads` must an integer value greater than or equal to -2")
  if(!is.null(max_mem_size) && !(is.character(max_mem_size) && length(max_mem_size) == 1L && !is.na(max_mem_size) && nzchar(max_mem_size))) stop("`max_mem_size` must be NULL or a non-empty character string")
  if(!is.null(max_mem_size) && !regexpr("^[1-9][0-9]*[gGmM]$", max_mem_size))                   stop("`max_mem_size` option must be like 1g or 1024m")
  if(!is.null(min_mem_size) && !(is.character(min_mem_size) && length(min_mem_size) == 1L && !is.na(min_mem_size) && nzchar(min_mem_size))) stop("`min_mem_size` must be NULL or a non-empty character string")
  if(!is.null(min_mem_size) && !regexpr("^[1-9][0-9]*[gGmM]$", min_mem_size))                   stop("`min_mem_size` option must be like 1g or 1024m")
  if(!is.logical(beta) || length(beta) != 1L || is.na(beta))                                    stop("`beta` must be TRUE or FALSE")
  if(!is.logical(assertion) || length(assertion) != 1L || is.na(assertion))                     stop("`assertion` must be TRUE or FALSE")
  if(!is.null(license) && !is.character(license))                                               stop("`license` must be of class character")
  if(!is.character(ice_root) || length(ice_root) != 1L || is.na(ice_root) || !nzchar(ice_root)) stop("`ice_root` must be a non-empty character string")
  if(!is.logical(strict_version_check) || length(strict_version_check) != 1L || is.na(strict_version_check))  stop("`strict_version_check` must be TRUE or FALSE")
  if(!is.logical(https) || length(https) != 1L || is.na(https))                                 stop("`https` must be TRUE or FALSE")
  if(!is.logical(insecure) || length(insecure) != 1L || is.na(insecure))                        stop("`insecure` must be TRUE or FALSE")
  if(!is.character(username) && !is.na(username))                                               stop("`username` must be a character string")
  if(!is.character(password) && !is.na(password))                                               stop("`password` must be a character string")
