...
.h2o.checkParam <- function(paramDef, paramValue) {
  ...
  if (scalar) { # scalar == TRUE
    if (type == "H2OModel")
        type <-  "character"
    if (name == "seed") {
	  if(is.character(paramValue) && suppressWarnings(is.na(bit64::as.integer64((paramValue)))))
	    e <- paste0("\"seed\" must be of type long or string long, but got string which cannot be converted to long.\n")
	  else if(is.numeric(paramValue) && nchar(paramValue) > 16){ 
	    warning("The seed has maximum number of digits which can be used in R as a number. The R cannot handle correctly larger numbers than 16 digits. 
	    If you need used larger number than integer, pass seed parameter as string number. Otherwise, the seed could be inconsistent.
