    ...
    if (type == "H2OModel")
      ...
      if (name == "seed") {
        if(is.character(paramValue) && !.is.int64(paramValue))
          ...
          } else if(is.na(suppressWarnings(as.integer(paramValue)))) {
              warning("The seed value is larger than Integer. R can handle only 32-bit integer without loss. 
               If you need to use a larger number than the integer, pass seed parameter as the string number. Otherwise, the seed could be inconsistent.
