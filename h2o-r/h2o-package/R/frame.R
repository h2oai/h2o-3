...
use.package <- function(package, 
  ...
  stopifnot(is.character(package), length(package)==1L,
   ...
   if (package == "data.table" && use && suppressWarnings(require("data.table", quietly = TRUE))) { # not sure if this is needed.  Keeping it for now.
     if ((!suppressWarnings(require("bit64", quietly = TRUE))) || (packageVersion("bit64") < as.package_version("0.9.7"))) {
