...
#' Obtain a list of columns that are specified by `type`
#'
#' @rdname columns_by_type
#' @param object H2OFrame object
...
#' }
#' @export
h2o.columns_by_type <- function(object,type="numeric",...){
  if(!is.H2OFrame(object)){
    stop("h2o.filter_type only operates on H2OFrames.")
  }
  if(!is.character(type)){
    stop("`type` variable should be of type character.")
  }
  .eval.scalar(.newExpr("columnsByType", object,.quote(type))) + 1
}
