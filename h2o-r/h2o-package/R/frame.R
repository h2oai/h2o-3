...
setMethod("mean", "H2OFrame", function(x, trim = 0, na.rm = FALSE, ...) {
  ...
  if (trim < 0) trim <- 0
  if (trim > .5) trim <- .5
  ast <- .h2o.nary_op("mean", x, trim, na.rm, ...)
  .force.eval(ast@ast)
})
...
setMethod("var", "H2OFrame",
  ...
  if(!missing(use)) {
    ...
    use <- "everything"
  ast <- .h2o.nary_op("var", x, y, na.rm, use)
  .force.eval(ast@ast)
})
...
function(x, center = TRUE, scale = TRUE) {
  ast <- .h2o.nary_op("scale", x, center, scale)
  .force.eval(ast@ast)
}
