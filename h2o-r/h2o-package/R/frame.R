...
h2o.splitFrame <- function(data, ratios = 0.75, destination_frames) {
  ...
  if (!missing(destination_frames))
    params$dest_keys <- .collapse.char(destination_frames)
...
#    warning(data@id, " has greater than ", .MAX_INSPECT_COL_VIEW, " columns. This may take awhile...")
...
#
#  res = .h2o.__remoteSend(data@conn, .h2o.__PAGE_SUMMARY2, source=data@id, max_ncols=.Machine$integer.max)
...
as.Date.H2OFrame <- function(x, format, ...) {
  ...
  expr = paste("as.Date(", paste(x@id, deparse(eval(format, envir = parent.frame())), sep = ","), ")", sep = "")
...
h2o.getTimezone <- function() {
  ...
  fr <- .newH2OFrame("H2OFrame", id = .key.make("getTimeZone"), mutable = mutable)
...
h2o.listTimezones <- function(conn=h2o.getConnection()) {
  ...
  mutable <- new("H2OFrameMutableState", ast = ast)
  fr <- .newH2OFrame("H2OFrame", id = .key.make("listTimeZones"), mutable = mutable)
  ret <- as.data.frame(fr)
  h2o.rm(fr@id)
  ret
}
...
#  .newH2OFrame("H2OFrame", id=res$dest_key, logic=FALSE)
#}
...
setMethod("[", "H2OFrame", function(x, i, j, ..., drop = TRUE) {
  ...
  if (missingJ) {
    ...
    ast <- new("ASTNode", root = new("ASTApply", op = "cols"), children = list(ast, cols))
  }
  ...
  .newH2OFrame("H2OFrame",  id = .key.make("subset"), mutable = mutable)
})
...
setMethod("[<-", "H2OFrame", function(x, i, j, ..., value) {
  ...
  res <- .h2o.replace.frame(conn = x@conn, ast = ast, id = x@id)
...
setMethod("colnames<-", signature(x="H2OFrame", value="character"),
  function(x, value) {
    ...
    .h2o.replace.frame(conn = x@conn, ast = ast, id = x@id)
})
...
h2o.levels <- function(x, i) {
  ...
  res <- .h2o.__remoteSend(x@conn, .h2o.__COL_DOMAIN(x@id, colnames(x)[col_idx]), method="GET")
  res$domain[[1]]
}
...
setMethod("summary", "H2OFrame", function(object, factors=6L, ...) {
  ...
  cols <- sapply(cnames, function(col) {
    col.rest <- .h2o.__remoteSend(object@conn, .h2o.__COL_SUMMARY(object@id, col), method = "GET")
...
#      if( any(!(vars2 %in% vars)) )             stop("ORDER BY columns must be used in the group by.")
  ...
  .newH2OFrame("H2OFrame", id = .key.make("group_by"), mutable = mutable)
}
...
#  .newH2OFrame('H2OFrame', id=res$dest_key)
#})
#
...
##     warning(object@id, " has greater than ", .MAX_INSPECT_COL_VIEW, " columns. This may take awhile...")
##   res <- .h2o.__remoteSend(.h2o.__PAGE_SUMMARY2, source=object@id, max_ncols=.Machine$integer.max)
