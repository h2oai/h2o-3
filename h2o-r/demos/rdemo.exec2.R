...
print("Convert RACE column from numeric to factor variable")
prostate.hex$RACE <- as.factor(prostate.hex$RACE)
print(prostate.hex)
expect_true(is.factor(prostate.hex$RACE))
print(summary(prostate.hex))

print("Display count of AGE column levels")
print("Note: Currently only working on a single integer or factor column")
...
print("Get quantiles of PSA column")
prostate.qs <- quantile(prostate.hex$PSA)
print(prostate.qs)

print("Extract outliers based on 5% and 95% quantile cutoffs")
...
print("Trim outliers from dataset")
...
print("Construct test and train sets using runif")
...
print("For large datasets, need to sample using h2o.runif")
...
print("Generate GBM predictions on test set")
prostate.pred <- predict(prostate.gbm, prostate.test)
summary(prostate.pred)
head(prostate.pred)
tail(prostate.pred)

print("Create new boolean column based on 25% quantile cutoff")
...
print("Run GLM2 with y = new boolean column, x = AGE, RACE, VOL, GLEASON")
