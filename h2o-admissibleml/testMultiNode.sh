...
function cleanup () {
  kill -9 ${PID_11} ${PID_12} ${PID_13} 1> /dev/null 2>&1
  wait 1> /dev/null 2>&1
...
# Setup JVM_CLASSPATH to include all compiled classes and jars
JVM_CLASSPATH="build/classes/java/main${SEP}build/classes/java/test${SEP}build/resources/main${SEP}../h2o-core/build/classes/java/main${SEP}../h2o-core/build/classes/java/test${SEP}../h2o-core/build/resources/main${SEP}../h2o-core/build/libs/*${SEP}../lib/*"

# Print classpath to debug
echo "Classpath used:"
echo "$JVM_CLASSPATH" | tr "$SEP" '\n'
...
(cd src/test/java && /usr/bin/find . -name '*.java' | cut -c3- | sed 's/\.java$//' | sed 's/\//./g') | /usr/bin/sort > $OUTDIR/tests.txt

# Output ignored and do-only tests
...
($JVM $TEST_SSL \
  ...
  $JACOCO_FLAG $JUNIT_RUNNER $(cat $OUTDIR/tests.txt) 2>&1; echo $? > $OUTDIR/status.1) 1> $OUTDIR/out.1 2>&1 & PID_1=$!

# Wait for test completion
wait $PID_1 1> /dev/null 2>&1

# Show top 10 longest tests
grep EXECUTION $OUTDIR/out.* | sed -e "s/.*TEST \(.*\) EXECUTION TIME: \(.*\) (Wall.*/\2 \1/" | sort -gr | head -n 10 >> $OUTDIR/out.0

# Final cleanup and exit with result
cleanup
