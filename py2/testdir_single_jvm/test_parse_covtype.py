...
def parseFrameCheck(frames_result, multiplyExpected):
    ...
    for i,c in enumerate(columns):
        ...
        assert zeros == expectedZeros[i] * multiplyExpected # files are concats of covtype. so multiply expected
        assert label == "C%s" % (i+1)
        assert stype == "int"
        assert missing == 0
...
class Basic(unittest.TestCase):
    ...
    def test_parse_covtype(self):
        ...
        tryList = [
            ('covtype.data', 10, 1),
            ('covtype20x.data', 200, 20),
        ]

        for (csvFilename, timeoutSecs, multiplyExpected) in tryList:
