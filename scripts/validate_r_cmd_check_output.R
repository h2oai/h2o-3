...
whitelist = list(
  "CRAN incoming feasibility" = c("Maintainer:","Checking URLs requires 'libcurl' support in the R build"),
  "package dependencies" = "No repository set, so cyclic dependency check skipped",
  "installed package size" = c("installed size is","sub-directories of 1Mb or more","java .*Mb")
  # "" = c("")
)
...
rcheck_id <- 1L # subset if there are many .Rcheck dirs in wd, it will take first
rcheck_dir <- Sys.glob("*.Rcheck")[rcheck_id]
...
res <- tools:::check_packages_in_dir_results(getwd())[[rcheck_id]]
if (length(res)) {
  if (res$status %in% c("FAIL","ERROR","WARN")) {
    status <- 1
    log_file <- Sys.glob("*.Rcheck/00check.log")[rcheck_id]
    ...
    details <- tools:::check_packages_in_dir_details(getwd())
