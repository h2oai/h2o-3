...
//    ],
    [
      stageName: 'Graal GBM benchmark', executionScript: 'h2o-3/scripts/jenkins/groovy/benchmarkStage.groovy',
      timeoutValue: 120, target: 'benchmark-gbm-graal', component: pipelineContext.getBuildConfig().COMPONENT_ANY,
      additionalTestPackages: [pipelineContext.getBuildConfig().COMPONENT_R],
      customData: [algorithm: 'gbm-graal'], makefilePath: pipelineContext.getBuildConfig().BENCHMARK_MAKEFILE_PATH,
      image: pipelineContext.getBuildConfig().GRAAL_IMAGE_NAME,
      nodeLabel: pipelineContext.getBuildConfig().getBenchmarkNodeLabel(),
    ]
  ]
