...
public class RefCntTest extends TestUtil {
  ...
  public void testBasic() {
    ...
    Vec.VectorGroup vg = crimes.vecs().group();
    ...
    Assert.assertTrue(crimes.vecs("Date").isTime(0));
    Rapids.exec("(tmp= py_1 (:= chicagoCrimes10k.hex (+ (cols_py chicagoCrimes10k.hex \"Date\") 1) 2 []))",session);
    Assert.assertTrue(crimes.vecs("Date").isTime(0));// User named frame is unchanged
    Frame py_1 = DKV.getGet(Key.make("py_1"));
    Assert.assertTrue(py_1.vecs("Date").isNumeric(0)); // tmp= py_1 holds the changed column
    Assert.assertTrue(py_1.vecs("Date").mean(0) > 1300000000L); // msec since epoch is generally >1.3b msec
    ...
    py_3.vecs().getRollups(); // Verify we can compute rollups on all cols; will crash if some cols are deleted
    ...
    py_4.vecs().getRollups(); // Verify we can compute rollups on all cols; will crash if some cols are deleted
    ...
    py_5.vecs().getRollups(); // Verify we can compute rollups on all cols; will crash if some cols are deleted
    ...
    py_6.vecs().getRollups(); // Verify we can compute rollups on all cols; will crash if some cols are deleted
    ...
    py_7.vecs().getRollups(); // Verify we can compute rollups on all cols; will crash if some cols are deleted
    ...
    py_8.vecs().getRollups(); // Verify we can compute rollups on all cols; will crash if some cols are deleted
    ...
    py_9.vecs().getRollups(); // Verify we can compute rollups on all cols; will crash if some cols are deleted
    ...
    py_10.vecs().getRollups(); // Verify we can compute rollups on all cols; will crash if some cols are deleted
