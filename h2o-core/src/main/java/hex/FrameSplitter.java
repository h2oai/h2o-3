...
public class FrameSplitter extends H2OCountedCompleter<FrameSplitter> {
  ...
  @Override public void compute2() {
    ...
    final VecAry[] templates = makeTemplates(dataset, ratios);
    ...
    final VecAry datasetVecs = dataset.vecs();
    splits = new Frame[nsplits];
    for (int s=0; s<nsplits; s++) {
      Frame split = new Frame(destKeys[s], dataset._names, templates[s] );
      split.delete_and_lock(jobKey);
      splits[s] = split;
    }
    setPendingCount(nsplits);
    for (int s=0; s<nsplits; s++)
      new FrameSplitTask(this,datasetVecs, ratios, s).dfork(splits[s].vecs());
  ...
  private VecAry[] makeTemplates(Frame dataset, double[] ratios) {
    VecAry vecs = dataset.vecs();
    final long[][] espcPerSplit = computeEspcPerSplit(vecs.espc(), vecs.numRows(), ratios);
    final int num = dataset.numCols(); // number of columns in input frame
    final int nsplits = espcPerSplit.length; // number of splits
    final String[][] domains = vecs.domains(); // domains
    final byte[] types = new byte[num];

    for (int j = 0; j < vecs.len(); ++j) types[j] = vecs.type(j);

    VecAry[] t = new VecAry[nsplits]; // resulting vectors for all
    for (int i=0; i<nsplits; i++) {
      // vectors for j-th split
      Key vkey = Vec.newKey();
      int rowLayout = AVec.ESPC.rowLayout(vkey,espcPerSplit[i]);
  ...
  private static class FrameSplitTask extends MRTask<FrameSplitTask> {
    final VecAry _srcVecs; // a source frame given by list of its columns
    ...
    public FrameSplitTask(H2OCountedCompleter completer, VecAry srcVecs, double[] ratios, int partIdx) {
      super(completer);
      _srcVecs = srcVecs;
      _ratios  = ratios;
      _partIdx = partIdx;
    ...
    @Override protected void setupLocal() {
      ...
      long[] partSizes = partitione(_srcVecs.numRows(), _ratios);
      long pnrows = 0;
      for (int p=0; p<_partIdx; p++) pnrows += partSizes[p];
      long[] espc = _srcVecs.espc();
    ...
    @Override public void map(Chunk[] cs) { // Output chunks
      ...
      Chunk [] srcChunks = _srcVecs.getChunks(cinidx);
