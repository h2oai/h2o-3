...
//        assert (java.util.Arrays.deepEquals(cm.domain,mdomain));
  ...
  public SB toJava( SB sb ) {
    ...
    sb.nl();
    sb.p("public class ").p(modelName).p(" extends water.genmodel.GeneratedModel {").nl();
  ...
  private SB toJavaDOMAINS( SB sb, SB fileContextSB ) {
    ...
    for (int i=0; i<_output._domains.length; i++) {
      ...
      sb.nl();
      fileContextSB.i().p("// The class representing column ").p(_output._names[i]).nl();
      JCodeGen.toClassWithArray(fileContextSB, null, colInfoClazz, dom);
    }
    return sb.i().p("};").nl().di(1);
  }
