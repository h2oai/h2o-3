...
//        assert (java.util.Arrays.deepEquals(cm.domain,mdomain));
  ...
  public SB toJava( SB sb ) {
    ...
    sb.p("// ").p(H2O.ABV.projectVersion().toString()).nl();
    sb.p("//").nl();
  ...
  private CtClass makeCtClass() throws CannotCompileException {
    ...
    CtClass clz = pool.makeClass(JCodeGen.toJavaId(_key.toString()));
    try {
      CtClass genmodel_clz = pool.get("hex.genmodel.GenModel");
      clz.setSuperclass(genmodel_clz);
...
....
  ...
  protected SB toJavaSuper( String modelName, SB sb ) {
    return sb.nl().p("public "+modelName+"() { super(NAMES,DOMAINS); }").nl();
  }
  ...
  public void testJavaScoring( Frame fr, Frame model_predictions ) {
    try {
      System.out.println(toJava());
      CtClass cc = makeCtClass();
      Class clz = cc.toClass(Model.class.getClassLoader());
      Object modelo = clz.newInstance();
      GenModel model = (GenModel)modelo;
      throw H2O.unimpl();
    }
