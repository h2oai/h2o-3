...
public class CreateFrame extends Iced {
  ...
  public CreateFrame(Key<Frame> key) {
    _job = new Job<>(key, Frame.class.getName(), "CreateFrame");
  }
  ...
  public Job<Frame> execImpl() {
    ...
    if (integer_fraction + binary_fraction + categorical_fraction + time_fraction + string_fraction
        > 1.00000001)
      throw new IllegalArgumentException(
          "Integer, binary, categorical, time and string fractions must add up to <= 1.");
    ...
    if (response_factors > Model.Parameters.MAX_SUPPORTED_LEVELS)
      throw new IllegalArgumentException(
          "Response factors must be <= " + Model.Parameters.MAX_SUPPORTED_LEVELS + ".");
    if (factors > 1000000)
    ...
    if (byte_estimate > cluster_free_mem)
      throw new IllegalArgumentException(
          "Frame is expected to require "
              ...
              + ", won't fit into H2O's free memory of "
              + cluster_free_mem);

    if (!randomize) {
      if (integer_fraction != 0
          || categorical_fraction != 0
          || time_fraction != 0
          || string_fraction != 0)
        throw new IllegalArgumentException(
            "Cannot have integer, categorical or time fractions > 0 unless randomize=true.");
    } else {
      if (value != 0)
        throw new IllegalArgumentException(
