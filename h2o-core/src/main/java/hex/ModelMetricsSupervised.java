...
public class ModelMetricsSupervised extends ModelMetrics {
  ...
  abstract public static class MetricBuilderSupervised<T extends MetricBuilderSupervised<T>> extends MetricBuilder<T> {
    ...
    public MetricBuilderSupervised(int nclasses, String[] domain) {
      assert domain==null || domain.length >= nclasses: "Domain is null ("+domain == null+") or domain.length ("+domain.length+") is less than nclasses ("+nclasses+")"; // Domain can be larger than the number of classes, if the score set includes "junk" levels
      _nclasses = nclasses;
      _domain = domain;
