...
public abstract class Cumu extends Iced<Cumu> {
  ...
  public Frame run(Frame f, double axis) {
    ...
    if (f.numCols() == 1) {
      if (axis == 0.0) {
        ...
        return new Frame(k, null, new Vec[]{cumuVec});
      } else {
        return new Frame(f);
      }
    }
      ...
      if (axis == 0.0) {  // down the column implementation
        ...
        CumuTaskWholeFrame t = new CumuTaskWholeFrame(f.anyVec().nChunks(), init(), f.numCols());
...
protected class CumuTaskAxis1 extends MRTask<CumuTaskAxis1> {
...
protected class CumuTaskWholeFrame extends MRTask<CumuTaskWholeFrame> {
  final int _nchks;   // IN
