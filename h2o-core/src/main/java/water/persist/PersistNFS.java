...
public final class PersistNFS extends Persist {
  ...
  private static File getFileForKey(Key k) {
    byte[] kb = k.bytes();
    final int off = k.isChunkKey() ? water.fvec.Vec.KEY_PREFIX_LEN : 0;
    assert new String(kb, off, KEY_PREFIX_LENGTH).equals(KEY_PREFIX) : "Not an NFS key: " + k;
    String s = new String(kb, KEY_PREFIX_LENGTH + off, kb.length - (KEY_PREFIX_LENGTH + off));
    return new File(s);
  }
