...
class ParseHandler extends Handler {
  ...
  public ParseV3 parse(int version, ParseV3 parse) {
    ...
    // TODO: add JobBase:
    parse.job = (JobV3)Schema.schema(version, Job.class).fillFromImpl(ParseDataset.parse(parse.destination_key.key(), srcs, parse.delete_on_done, setup, parse.blocking));
    if( parse.blocking ) {
