...
public class FVecParseWriter extends Iced implements StreamParseWriter {
  ...
  private void recordMissingColumnError(int colIdx, Object value) {
    if ((_maxMissingCol == -1) || (_maxMissingCol + 1 == colIdx)) // we want to report values in the right order (or not at all)
      _missingColVals = (_missingColVals == null ? new StringBuilder() : _missingColVals.append(", ")).append(value);
