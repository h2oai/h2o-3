...
public class VecUtils {
  ...
  public static Vec copyOver(Vec src, byte type, long[] domain) {
  ...
  /**
   * Replace an integer column (such as Enum storage) with the values given by a double array
   */
  public static class ReplaceTask extends MRTask<ReplaceTask> {
    private double[] _vals;
    double _prior = Double.NaN;
    public ReplaceTask(double[] values) { _vals = values; } //training
    public ReplaceTask(double[] values, double prior) { _vals = values; _prior = prior; } //testing
    @Override
    public void map(Chunk c) {
      for (int i=0;i<c._len;++i) {
        if (c.isNA(i)) continue; //NA stays NA
        if (c.at8(i) < _vals.length) {
          c.set(i, _vals[(int) c.at8(i)]);
        } else {
          assert(_prior!=Double.NaN);
          c.set(i, _prior);
        }
      }
    }
