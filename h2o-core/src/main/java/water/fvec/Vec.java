...
public class Vec extends Keyed<Vec> {
  ...
  static void bulk_remove( final Key[] keys, final int ncs ) {
    ...
    for (Key key : keys) setMutating(chunkKey(key, -2));
    // Bulk dumb local remove - no JMM, no ordering, no safety.
    // Remove Vecs everywhere first - important! (in {@link RollupsStats} we install new RollupStats only if Vec still exists)
    new MRTask() {
