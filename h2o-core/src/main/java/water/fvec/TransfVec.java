 ...
 *
 * <p>The mapping is defined by a simple hash map composed of two arrays.  The
 * first array contains values. Index of values is index into the second array
 * {@link #_indexes} which contains final value (i.e., index to domain
 * array).</p>
 *
 * <p>If {@link #_indexes} array is null, then index of found value is used directly.</p>
 *
 * <p>To avoid virtual calls or additional null check for {@link #_indexes} the
 * vector returns two different implementations of the underlying chunk
 * depending if {@link #_indexes} is <code>null</code> or not.</p>
 */
