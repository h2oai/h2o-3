...
class BinaryMerge extends DTask<BinaryMerge> {
  ...
  private void createChunksInDKV() {
    ...
    chunksPopulateRetFirst(numColsInResult, numLeftCols, perNodeLeftLoc, grrrsLeft, perNodeRightLoc, grrrsRite, frameLikeChunks, frameLikeChunks4Strings, frameLikeChunksLongs);
    _timings[10] += ((t1=System.nanoTime()) - t0) / 1e9; t0=t1;

    // compress all chunks and store them
    chunksCompressAndStore(nbatch, numColsInResult, frameLikeChunks, frameLikeChunks4Strings, frameLikeChunksLongs);
  ...
  // Now loop through _ret1st and _retLen and populate
  private void chunksPopulateRetFirst(final int numColsInResult, final int numLeftCols, final long perNodeLeftLoc[],
                                      final GetRawRemoteRows grrrsLeft[][], final long perNodeRightLoc[],
                                      final GetRawRemoteRows grrrsRite[][], final double[][][] frameLikeChunks,
