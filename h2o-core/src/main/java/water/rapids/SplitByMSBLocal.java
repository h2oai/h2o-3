...
class SplitByMSBLocal extends MRTask<SplitByMSBLocal> {
  ...
  SplitByMSBLocal(boolean isLeft, long base[], int shift, int keySize, int batchSize, int bytesUsed[], int[] col, Key linkTwoMRTask, int[][] id_maps, boolean[] isNotDouble, BigInteger[] baseD, boolean[] isCategorical) {
    _isLeft = isLeft;
    ...
    _shift = shift;
    _batchSize=batchSize; _bytesUsed=bytesUsed; _col=col; _base=base; _isNotDouble=isNotDouble; _isCategorical=isCategorical;
    _baseD=baseD;
    _keySize = keySize;
    ...
    for (int r=0; r<chk[0]._len; r++) {    // tight, branch free and cache efficient (surprisingly)
      ...
      if (!chk[0].isNA(r)) {
        ...
        if (_isLeft && _id_maps[0]!=null) {
          ...
          if (_isNotDouble[0] || _isCategorical[0]) {
      ...
      for (int c=1; c<chk.length; c++) {  // TO DO: left align subsequent
        ...
        if (_isLeft && _id_maps[c] != null) thisx = _id_maps[c][(int)thisx] + 1;
          ...
          if (_isNotDouble[c] || _isCategorical[c]) {
