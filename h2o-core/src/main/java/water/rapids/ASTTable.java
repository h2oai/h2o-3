...
class ASTTable extends ASTPrim {
  @Override public String[] args() { return new String[]{"X", "Y"}; }
...
class ASTUnique extends ASTPrim {
  ...
  @Override public String str() { return "unique"; }
  ...
  @Override Val apply(Env env, Env.StackHelp stk, AST asts[]) {
    ...
    if( fr.anyVec().isCategorical() ) {
      ...
      IcedHashMap<ASTGroup.GKX,String> gs = ASTGroup.findGroups(fr);
      final ASTGroup.GK0[] gkxs = ASTGroup.sortGroups(gs);
      v = Vec.makeZero(gkxs.length);
      new MRTask() {
        @Override public void map(Chunk c) {
          int start = (int)c.start();
          for( int i=0; i < c._len; i++ ) c.set(i, gkxs[i + start]._d0);
        }
      }.doAll(v);
    }
