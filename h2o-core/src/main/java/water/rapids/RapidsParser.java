...
public class RapidsParser {
    ...
    while (_x < _str.length()) {
      switch (firstNonWhiteSpaceChar()) {
        ...
        case '}':
          ...
          break;
        case '[':
          pushOnStack(parseList());
          break;
        case '0': case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '8': case '9':
          pushOnStack(new AstNum(parseNumber()));
          break;
        case '-':
          if (peek(1) >= '0' && peek(1) <= '9') {
            pushOnStack(new AstNum(parseNumber()));
          } else {
            pushOnStack(new AstId(parseToken()));
          }
          break;
        case '\"': case '\'':
          pushOnStack(new AstStr(parseString()));
          break;
        case ' ':
  ...
  private void pushOnStack(AstRoot rapids) {
    stack.push(rapids);
    if (!counter.isEmpty()) {
  ...
  private void parseFunctionDefinitionOpen() {
    ...
    pushOnStack(new AstStr("")); // 1-based ID list
  ...
  private void parseFunctionDefinitionClose() {
    ...
    pushOnStack(new AstFunction(argNames, body));
  }
