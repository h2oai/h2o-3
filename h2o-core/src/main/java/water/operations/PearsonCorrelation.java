...
public class PearsonCorrelation {
    ...
    public static double pearsonCorrelation(Frame frx, Frame fry, Mode mode) {
        ...
        if (NACount != 0) {
            ...
            if (mode.equals(Mode.Everything)) return Double.NaN;
        }

        return (ss / (ncols - NACount - 1)) / denom; //Pearson's Correlation Coefficient
    }
    ...
    public static Mode convertMode(String use) {
        Mode mode;
        switch (use) {
            case "everything":
    ...
    public static Frame array(Frame frx, Frame fry, Mode mode) {
        Vec[] vecxs = frx.vecs();
        ...
        if (mode.equals(Mode.Everything) || mode.equals(Mode.AllObs)) {
            ...
            // FIXME this functions is not well defined, sometims it return a frame sometimes a double
//            if (ncolx == 1 && ncoly == 1) {
//                // return (cvs[0].getResult()._covs[0] / (fry.numRows() - 1)) / denom[0][0]);
//            }
            ...
            for (int y = 0; y < ncoly; y++) {
                res[y] =
                        Vec.makeVec(
                                ArrayUtils.div(
                                        ArrayUtils.div(cvs[y].getResult()._covs, (fry.numRows() - 1)), denom[y]),
                                keys[y]);
            }

            return new Frame(fry._names, res);
            ...
            Frame frxy_naomit =
                    ...
                    }.doAll(new Frame(frx).add(fry).types(), new Frame(frx).add(fry))
                            .outputFrame(new Frame(frx).add(fry).names(), new Frame(frx).add(fry).domains());
            ...
            for (int y = 0; y < ncoly_naomit; y++) {
                //Get covariance between x and y
                cvs[y] =
                        new CoVarTask(vecys_naomit[y].mean(), xmeans)
                                .dfork(new Frame(vecys_naomit[y]).add(frxy_naomit.subframe(0, ncolx)));
                //Get sigma of y vecs
...
//                //  return (cvs[0].getResult()._covs[0] / (frxy_naomit.numRows() - 1)) / denom[0][0];
//            }
            ...
            for (int y = 0; y < ncoly_naomit; y++) {
                res[y] =
                        Vec.makeVec(
                                ArrayUtils.div(
                                        ArrayUtils.div(cvs[y].getResult()._covs, (frxy_naomit.numRows() - 1)),
                                        denom[y]),
                                keys[y]);
            }

            return new Frame(frxy_naomit.subframe(ncolx, frxy_naomit.vecs().length)._names, res);
        }
    }
